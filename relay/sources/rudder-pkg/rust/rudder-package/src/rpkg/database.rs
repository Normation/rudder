...
impl Database {
  pub fn read(path: &str) -> Result<Database> {
      let data = std::fs::read_to_string(path).with_context(|| format!("Failed to read the installed plugin database in {}", path))?;
...
mod tests {
  ...
  fn test_plugin_database_parsing() {
    let data = read_to_string("./tests/plugin_database_parsing.json").expect("Unable to parse file './tests/pluginèdatabase_parsing.json'");
  ...
  fn test_adding_a_plugin_to_db() {
    ...
    let target_path = dir.path().join("target.json").into_os_string().into_string().unwrap();
    let _ = Database::write(&target_path.clone(), a);
    let reference: serde_json::Value = serde_json::from_str(&read_to_string("./tests/database/plugin_database_update_sample.json.expected").unwrap()).unwrap();
    let generated: serde_json::Value = serde_json::from_str(&read_to_string(target_path).unwrap()).unwrap();
    assert_json_eq!(
      reference,
      generated
    );
