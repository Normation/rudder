...
pub fn watch(path: WatchedDirectory, cfg: CatchupConfig, tx: mpsc::Sender<ReceivedFile>) {
    ...
    // Nothing ever joins these tasks, so make sure the error they may stop on is visible
    tokio::spawn(log_stop(
        "catchup listing",
        list_files(path.clone(), cfg, tx.clone()),
    ));
    tokio::spawn(log_stop("inotify watch", watch_files(path, tx)));
}

async fn log_stop(name: &'static str, task: impl Future<Output = Result<(), Error>>) {
    match task.await {
        Ok(()) => error!("{} stopped", name),
        Err(e) => error!("{} stopped: {}", name, e),
    }
}
