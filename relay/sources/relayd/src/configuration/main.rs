...
use anyhow::{anyhow, Context, Error, bail};
...
impl Configuration {
    ...
    fn validate(self) -> Result<Self, Error> {
        ...
        if (matches!(self.processing.reporting.output, ReportingOutputSelect::Upstream) ||
            matches!(self.processing.inventory.output , InventoryOutputSelect::Upstream)) &&
        ...
        if matches!(self.processing.reporting.output, ReportingOutputSelect::Database) &&
...
impl Default for Configuration {
    fn default() -> Self { toml::from_str::<Self>("").unwrap() }
}
