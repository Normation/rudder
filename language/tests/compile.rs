...
/// takes an rd file and a cf file, parses the first, compiles,
...
/// input is rd -> state_checkdef.rd
/// output is .rd.cf -> state_checkdef.rd.cf
/// example of files that should succeed: s_errors.rd s_errors.rd.cf
// TODO: compare both result and generated output (.rd.cf) in separated tests
...
fn real_files(filename: &str) {
    ...
    test_real_file(filename, &rudderc::generator::Format::CFEngine);
...
fn generated_files(filename: &str, content: &str) {
    test_generated_file(filename, content, &rudderc::generator::Format::CFEngine);
