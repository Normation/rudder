...
impl<'src> IR2<'src> {
    ...
    fn binding_check(&self, statement: &Statement) -> Result<()> {
        match statement {
            ...
            Statement::Case(_name, cases) => map_results(cases.iter(), |(_c, st)| {
    ...
    fn enum_expression_check(&self, context: &VarContext, statement: &Statement) -> Result<()> {
        match statement {
            Statement::Case(case, cases) => {
                ...
                fix_results(cases.iter().map(|(_cond, st)| {
                    self.enum_expression_check(context, st)
                }))
            }
            _ => Ok(()),
        }
        ...
        match statement {
            Statement::Case(keyword, cases) => {
                if first_level {
                    ...
                    fix_results(cases.iter().map(|(_cond, st)| {
                        self.cases_check(variables, st, false)
                    }))?;
                } else {
                    fail!(
                        keyword,
