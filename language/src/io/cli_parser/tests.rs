...
mod tests {
    ...
    fn cli() {
        ...
        assert_eq!(
            ...
            opt_new_r("rudderc technique read -c tools/rudderc-dev.conf -i tests/techniques/6.1.rc5/technique.rd -o tests/techniques/6.1.rc5/technique.ee.cf -l warn -b --stdin --stdout"),
            Ok(CLI::Technique(Technique::Read {
                options: Options {
                    config_file: PathBuf::from("tools/rudderc-dev.conf"),
                    input: Some(PathBuf::from("tests/techniques/6.1.rc5/technique.rd")),
                    stdin: true,
                    stdout: true,
        ...
        // expected subcommand
        assert!(opt_new_r("rudderc -c tools/rudderc-dev.conf -i tests/techniques/6.1.rc5/technique.rd -o tests/techniques/6.1.rc5/technique.ee.cf -l warn -b --stdin --stdout").is_err());
        ...
        assert_eq!(
            opt_new_r("rudderc compile --format rd"),
            Err("InvalidValue".into())
        );
    }
