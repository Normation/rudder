...
impl LogOutput {
    ...
    /// or rudder language own error logging format
    ...
    fn print_to_file(&self, files: &mut Vec<CommandResult>, command: Command) {
        ...
        // else
        // note there might be no file to print. ie technique generate
        ...
        for file in files.iter_mut() {
            if let (Some(dest), Some(new_content)) = (&file.destination, &file.content) {
                if let Some(content) = filemap.get_mut(dest) {
                    *content = format!("{}\n\n{}", content, new_content);
                } else if file.format != Format::JSON && file.format != Format::Markdown {
                    filemap.insert(dest, format!("# generated by rudderc\n{}", new_content));
                } else {
                    filemap.insert(dest, new_content.to_owned());
                }
                file.content = None;
            } else {
