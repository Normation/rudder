...
impl<'a> VarContext<'a> {
    pub fn new_global() -> VarContext<'static> {
        VarContext { variables: HashMap::new(), global_context: None }
    }

    pub fn new_local(global: &'a VarContext<'a>) -> VarContext<'a> {
        VarContext { variables: HashMap::new(), global_context: Some(global) }
    }

    pub fn new_enum_variable(&mut self, name: PToken<'a>, enum1: PToken<'a>, value: Option<PToken<'a>>) -> Result<()> {
        if self.variables.contains_key(&name) {
            fail!(name, "Variable {} already defined {}", name, self.variables.entry(name).key());
        }
        // Do not allow a local name to hide a global name
        match self.global_context {
            None => {},
            Some(gc) => if gc.variables.contains_key(&name) {
                fail!(name, "Variable {} hides global variable {}", name, self.variables.entry(name).key());
            }
        }
...
mod tests {
    ...
    fn test_context() {
        ...
        assert!(gc.new_enum_variable(ident("var1"),ident("enum1"),None).is_ok());
        assert!(gc.new_enum_variable(ident("var2"),ident("enum1"),Some(ident("debian"))).is_ok());
        let mut c = VarContext::new_local(&gc);
        assert!(c.new_enum_variable(ident("var3"),ident("enum2"),None).is_ok());
        assert!(c.new_enum_variable(ident("var4"),ident("enum1"),Some(ident("ubuntu"))).is_ok());
        
        assert_eq!(c.get_variable(ident("var3")), Some(&VarKind::Enum(ident("enum2"),None)));
        assert_eq!(c.get_variable(ident("var2")), Some(&VarKind::Enum(ident("enum1"),Some(ident("debian")))));
        assert_eq!(gc.get_variable(ident("var1")), Some(&VarKind::Enum(ident("enum1"),None)));
