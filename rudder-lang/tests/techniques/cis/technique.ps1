function Cis {
  ...
  $local_classes = Merge-ClassContext $local_classes $(Service-Enabled -ServiceRegex "${service}" -componentName "Service enabled at boot" -reportId $reportId -techniqueName $techniqueName -auditOnly:$auditOnly).get_item("classes")

  $class = "any.(skip_item_${report_data.canonified_directive_id}_true)"
  if (Evaluate-Class $class $local_classes $system_classes) {
    $local_classes = Merge-ClassContext $local_classes $(Service-Started -ServiceRegex "${service}" -componentName "Service started" -reportId $reportId -techniqueName $techniqueName -auditOnly:$auditOnly).get_item("classes")
  } else {
