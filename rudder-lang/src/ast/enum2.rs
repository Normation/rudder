...
impl<'src> EnumTree<'src> {
    // create a new enumtree with a single level
    pub fn new(name: Token<'src>, elders: Vec<Token<'src>>, incomplete: bool) -> EnumTree<'src> {
    ...
    // This subtree is the munimal enum tree use by an expression
...
impl<'src> EnumList<'src> {
    ...
    // Update when parser will be modified
    pub fn add_enum(&mut self, e: PEnum<'src>) -> Result<()> {
        ...
        let tree = EnumTree::new(e.name, e.items, false);
    ...
    // Update when parser will be modified
    pub fn extend_enum(&mut self, e: PEnum<'src>) -> Result<()> {
        // find tree name
        let treename = match self.elements.get(&e.name) {
            None => fail!(e.name, "Sub enum {} doesn't have any parent", e.name),
            Some(t) => *t,
        };
        // find tree
...
mod tests {
    ...
    #[test]
    fn test_tree() {
        let mut tree = EnumTree::new("T".into(), vec!["a".into(), "b".into(), "c".into()], true);
