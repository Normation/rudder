...
if [ ! -f /opt/rudder/etc/ssl/agent.cert ]; then
  mkdir -p /data/ssl
  openssl req -new -sha256 -key /data/ppkeys/localhost.priv -out /data/ssl/agent.cert -x509 -days 3650 -extensions agent_cert -config /opt/rudder/etc/ssl/openssl-agent.cnf -subj "/UID=${uuid}"
fi
