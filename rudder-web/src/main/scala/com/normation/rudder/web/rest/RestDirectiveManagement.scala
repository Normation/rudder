...
class RestDiretiveManagement (
  ...
  /*  case Get("add" :: name :: Nil, req) => { val directive = Directive(DirectiveId(uuidGen.newUuid),name,0)
    ...
    case Put("clone" :: id :: Nil, req) => {
      ...
      val actor = RestUtils.getActor(req)
      val directiveId = DirectiveId(id)
      readDirective.getDirective(directiveId) match {
        case Full(directive) => 
          val activetech = readDirective.getActiveTechnique(directive.id)
    ...
    case Get(id :: Nil, req) => {
      ...
      readDirective.getDirective(DirectiveId(id)) match {
        case Full(x) =>
  ...
  def ChangeDirectiveStatus(id : DirectiveId, modId : ModificationId, actor : EventActor, status : Boolean)={
    ...
    readDirective.getDirective(id) match {
      case Full(directive) =>
        if (directive.isEnabled==status){
          ...
          writeDirective.saveDirective(
              readDirective.getActiveTechnique(directive.id).get.id,
  ...
  def toJSON (directive : Directive) : JValue = {
