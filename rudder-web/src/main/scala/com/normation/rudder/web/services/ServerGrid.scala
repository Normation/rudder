 ...
 */
class ServerGrid(getServerAndMachine:LDAPFullInventoryRepository) extends Loggable {
  ...
  private def details(jsonArg:String) : JsCmd = {
    ...
    ( for {
      ...
      sm <- getServerAndMachine.get(NodeId(arg.id),status)
    } yield (sm,arg.jsid) ) match {
      case Full((sm,jsid)) => 
        SetHtml(jsid, DisplayServer.showPannedContent(sm)) &
        DisplayServer.jsInit(sm.node.main.id, sm.node.softwareIds, "", Some("node_tabs"))
      case e:EmptyBox => 
