...
import com.normation.rudder.domain.policies.{PolicyInstanceTarget, ConfigurationRule}
...
import com.normation.rudder.web.components.ShowServerDetailsFromNode
import com.normation.rudder.web.components.SearchServerComponent
...
class SearchServers extends StatefulSnippet with Loggable {
  val lock = new Object
  ...
  private[this] def setNodeGroupCategoryForm(query:Option[Query]) : SearchServerComponent = {    
    ...
    searchServerComponent.set(Full(sc))
    sc
  }
  ...
  private def searchServers = chooseTemplate("query","SearchServers",serverPortletTemplateFile)
  
  // the container for the server selected
//  var currentSelectedServer = Option.empty[NodeInfo]
  ...
  private[this] def parseJsArg(): JsCmd = {
    def displayDetails(nodeId:String) = {
      SetHtml("serverDetails", (new ShowServerDetailsFromNode(new NodeId(nodeId))).display)
    }
    ...
    def executeQuery(query:String) : JsCmd = {
      ...
      q match {
        ...
        case Full(q)    => 
          Replace("SearchServers", sc.buildQuery()) &
  ...
  private[this] def showPopup() : JsCmd = {
    setCreationPopup(searchServerComponent.is.open_!.getQuery(),
   ...
   * @return
   */
  private def showServerDetails(s:String) : JsCmd = {
    val arr = s.split("\\|")
    val nodeId = arr(1)
    SetHtml("serverDetails", (new ShowServerDetailsFromNode(new NodeId(nodeId))).display) &
