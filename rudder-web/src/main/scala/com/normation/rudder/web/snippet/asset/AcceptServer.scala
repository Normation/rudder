...
import com.normation.rudder.services.servers.NewServerManager
 ...
 */
class AcceptServer {
  val logger = LoggerFactory.getLogger(classOf[AcceptServer])
  val newServerManager = inject[NewServerManager]
  val ldap = inject[LDAPConnectionProvider]
  val rudderDit = inject[RudderDit]
  val serverGrid = inject[ServerGrid]
  val serverSummaryService = inject[ServerSummaryService]
  ...
  val pendingServerDit = inject[InventoryDit]("pendingServersDit")
  ...
  def list(html:NodeSeq) :  NodeSeq =  {
    ...
    def refuseServers(listServer : Seq[NodeId]) : Unit = {
      //TODO : manage error message    
      S.clearCurrentNotices
      newServerManager.refuse(listServer, CurrentUser.getActor).zip(listServer).foreach {case (box,id) =>  box match {
        case e:EmptyBox => 
          ...
          case Full(servers) => 
            bind("servergrid",template, 
              ...
              "accept" -> SHtml.submit("Accept", { 
                () => { addServers(listServer) }
                S.redirectTo(S.uri)
              })
              ,
              "refuse" -> SHtml.submit("Refuse", { 
                () => refuseServers(listServer)
                 S.redirectTo(S.uri)
              }, ("class", "red")),
