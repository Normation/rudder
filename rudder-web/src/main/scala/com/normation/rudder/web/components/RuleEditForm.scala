...
class RuleEditForm(
  ...
  private[this] def  showRuleDetails() : NodeSeq = {
    ...
    (
      ...
      "#nameField" #>    <div>{crName.displayNameHtml.getOrElse("Could not fetch rule name")} {updatedrule.map(_.name).openOr("could not fetch rule name")} <br/><div class="rudderID"><b>Rudder ID :</b> {rule.id.value} </div></div> &
