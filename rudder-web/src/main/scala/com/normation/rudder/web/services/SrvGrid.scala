...
class SrvGrid {
  ...
  def displayAndInit(
    ...
    Script(initJs(tableId, callback))
  }
   ...
   *
   */
  def initJs(tableId:String,callback : String => JsCmd) : JsCmd = {
    ...
    JsRaw("""
      ...
      """.replaceAll("#table_var#",jsVarNameForId(tableId))
        ...
        JsRaw("""
          ...
          """.format(tableId).replaceAll("#table_var#",jsVarNameForId(tableId))
        ) &
