...
import com.normation.rudder.repository.{GroupCategoryRepository, NodeGroupRepository}
import bootstrap.liftweb.LiftSpringApplicationContext.inject
import com.normation.rudder.domain.queries.Query
import com.normation.rudder.services.policies.{DependencyAndDeletionService, PolicyInstanceTargetService}
import com.normation.rudder.domain.policies.{PolicyInstanceTargetInfo, GroupTarget, PolicyInstanceTarget}
...
import com.normation.rudder.web.services.{LogDisplayer, ReportDisplayer, DisplayServer}
...
object ShowServerDetailsFromNode {
...
class ShowServerDetailsFromNode(
  nodeId:NodeId  
) extends DispatchSnippet with Loggable {
  import ShowServerDetailsFromNode._
  ...
  private[this] val acceptedInventoryDit = inject[InventoryDit]("acceptedServersDit")
  ...
  private[this] val targetService = inject[PolicyInstanceTargetService]
  ...
  def display() : NodeSeq = {
    nodeInfoService.getNodeInfo(nodeId) match {
      ...
      case Full(server) => // currentSelectedServer = Some(server)
   ...
   * @return
   */
  private def bindServer(node : NodeInfo, inventory: FullInventory) : NodeSeq = {
      ...
      bind("server", serverDetailsTemplate,
             "jsTree" ->
              ...
              "nodeDetails" -> DisplayServer.showNodeDetails(inventory, Some(node.creationDate)),
              "inventory" -> DisplayServer.show(inventory, false),
              "extraHeader" -> DisplayServer.showExtraHeader(inventory),
              "extraContent" -> DisplayServer.showExtraContent(inventory),
              "reports" -> reportDisplayer.displayReports(node),
              "logs" -> logDisplayer.display(node.id))
  }
    
  private def findTargets(node : NodeInfo): Box[Seq[PolicyInstanceTarget]] = {
    for {
      allTargets <- targetService.findTargets(Seq(node.id)) //allTargets: Map[NodeId, Set[PolicyInstanceTarget]]
  ...
  ********************************************/
  ...
  private[this] var targets :  Seq[PolicyInstanceTarget] = Seq()
