  ...
  private def displayPolicyServerInfos(sm:FullInventory) : NodeSeq = {
    nodeInfoService.getNodeInfo(sm.node.main.policyServerId) match {
      ...
      case Full(Some(policyServerDetails)) =>
        <span><b>Rudder Policy Server: </b><a href={JsInitContextLinkUtil.baseNodeLink(policyServerDetails.id)} onclick="location.reload()">{policyServerDetails.hostname}</a></span>
      case Full(None) =>
