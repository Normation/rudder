  ...
  def merge(
    ...
    val mods = if (removeMissing) { // compare all values
      ...
      if (forceKeepMissingAttributes.isEmpty) {
        ...
        val attrs =
          (origin.attributes.map(_.getName).toSet -- forceKeepMissingAttributes) ++ targetEntry.attributes.toSeq.map(_.getName)
        com.unboundid.ldap.sdk.Entry.diff(
          origin.backed,
          target.backed,
          true,
          false,
