...
object LDAPIOResult {
  ...
  def effect[A](effect: => A):            IO[LDAPRudderError.BackendException, A] = {
    IOResult.effect(effect).mapError(err => LDAPRudderError.BackendException(err.msg, err.cause))
  }
  def effectNonBlocking[A](effect: => A): IO[LDAPRudderError.BackendException, A] = {
    IOResult.effectNonBlocking(effect).mapError(err => LDAPRudderError.BackendException(err.msg, err.cause))
  }
