...
class TestCertificate extends Specification with Loggable {
  ...
  val reportSaver = new ReportSaver[Seq[LDIFChangeRecord]] {
    override def save(report: InventoryReport): Box[Seq[LDIFChangeRecord]] = {
  ...
  val fullInventoryRepo = new FullInventoryRepository[Seq[LDIFChangeRecord]] {
     ...
     */
    override def get(id: NodeId): Box[FullInventory] = repository.get(id) match {
      case None    => Failure(s"no such node in repo: ${id.value}")
      case Some(x) => Full(x)
    }

    override def exists(id: NodeId): Box[Boolean] = Full(repository.isDefinedAt(id))

    override def get(id: NodeId, inventoryStatus: InventoryStatus): Box[FullInventory] = get(id)
    override def save(serverAndMachine: FullInventory): Box[Seq[LDIFChangeRecord]] = {
  ...
  "when a node is not in repository, it is ok to not have a signature with cfkey" in {
    ...
    val res = processor.saveInventory(
        () => Resource.getAsStream("linux-cfe-sign.ocs")
      , "linux-cfe-sign"
      , None
    )

    (res.openOrThrowException(s"test error: ${res}")  must beAnInstanceOf[InventoryProcessStatus.Accepted]) and
  ...
  "when a node is in repository with a registered key, it is NOK to miss signature" in {
    ...
    (res.openOrThrowException(s"test error: ${res}")  must beAnInstanceOf[InventoryProcessStatus.MissingSignature])
  }
  ...
  "when certificate 'subject' doesn't match node ID, we got an error" in {
    ...
    (res must beLike {
      case f: Failure => f.messageChain must beMatching(".*subject doesn't contain same node ID in 'UID' attribute as inventory node ID.*")
    })
 }
