...
class TestSignatureService extends Specification with Loggable {
    ...
    ZIO.attempt {
      ...
      if (null == url) {
        throw new NullPointerException(
          s"Resource with relative path '${path}' is null (missing resource? Spelling? Permissions?)"
        )
      }
      url.openStream()
  ...
  private[this] object TestInventoryDigestServiceV1 extends ParseInventoryDigestFileV1 with GetKey with CheckInventoryDigest {
    ...
    def getKey(receivedInventory: Inventory): IOResult[(SecurityToken, KeyStatus)] = {
      for {
        cfengineKey <- ZIO
                         .fromOption(receivedInventory.node.agents.headOption)
                         .mapError(_ => InventoryError.Inconsistency("There is no public key in inventory"))
  ...
  "a signed inventory" should {
    "Be ok if checked with correct signature" in {
      ZioRuntime.unsafeRun(for {
        ...
        check      <-
          ZIO.acquireReleaseWith(getInputStream("fusion-inventories/signed_inventory.ocs"))(is => effectUioUnit(is.close))(is => {
            for {
    ...
    "Be wrong if checked with wrong signature" in {
      ZioRuntime.unsafeRun(for {
        ...
        check        <- ZIO.acquireReleaseWith(getInputStream("fusion-inventories/node-with-server-role-attribute.ocs"))(is =>
