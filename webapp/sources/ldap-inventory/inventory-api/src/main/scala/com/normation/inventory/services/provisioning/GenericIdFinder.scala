...
class NodeInventoryDNFinderService(actions: Seq[NamedNodeInventoryDNFinderAction]) extends NodeInventoryDNFinderAction {
  ...
  override def tryWith(entity: NodeInventory): IOResult[Option[(NodeId, InventoryStatus)]] = {
    ZIO
      .foldLeft(actions)(Option.empty[(NodeId, InventoryStatus)]) {
          ...
          next.action.tryWith(entity).flatMap {
            case Some((id, s)) =>
              InventoryProcessingLogger
                .debug(s"Node Id '${id.value}' found in DIT '${s.name}' with id finder '${next.name}'") *> Some((id, s)).succeed
            case None          =>
              InventoryProcessingLogger
                .trace(s"Node id '${entity.main.id.value}' not found with findder '${next.name}'") *> None.succeed
          }
      }
      .flatMap {
...
class MachineDNFinderService(actions: Seq[NamedMachineDNFinderAction]) extends MachineDNFinderAction with Loggable {
  ...
  override def tryWith(entity: MachineInventory): IOResult[Option[(MachineUuid, InventoryStatus)]] = {
    ZIO
      .foldLeft(actions)(Option.empty[(MachineUuid, InventoryStatus)]) {
        case (Some(found), next) => Some(found).succeed
        case (None, next)        =>
          ...
          next.action.tryWith(entity).flatMap {
            case Some((id, s)) =>
              InventoryProcessingLogger.debug(
                s"Machine Id '${id.value}' found in DIT '${s.name}' with id finder '${next.name}'"
              ) *> Some((id, s)).succeed
            case None          =>
              InventoryProcessingLogger
                .trace(s"Machine id '${entity.id.value}' not found with findder '${next.name}'") *> None.succeed
          }
      }
      .flatMap {
