...
class FileHistoryLogRepository[ID: ClassTag, T](
  ...
  private def root: IOResult[File] = {
    for {
      ...
      isCreated <- ZIO.when(!dir.exists) {
                     Task.effect(dir.mkdirs).mapError(e => InventoryError.System(s"Error creating '${rootDir}': ${e.getMessage}"))
                   }
    } yield {
      dir
    }
  ...
  def getIds: IOResult[Seq[ID]] = {
    for {
      r   <- root
      res <- Task
               .effect(r.listFiles.collect { case (f) if (f.isDirectory) => converter.filenameToId(f.getName) })
  ...
  def versions(id: ID): IOResult[Seq[DateTime]] = {
    for {
      ...
      res <- if (ok) {
               Task
                 .effect(i.listFiles.toSeq.map(f => sToV(f.getName)).filter(_.isDefined).map(_.get).sortWith(_.compareTo(_) > 0))
