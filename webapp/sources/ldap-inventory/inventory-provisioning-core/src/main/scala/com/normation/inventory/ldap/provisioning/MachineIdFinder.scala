...
class UseExistingMachineIdFinder(
  ...
  override def tryWith(entity: MachineInventory): IOResult[Option[(MachineUuid, InventoryStatus)]] = {
    for {
      con   <- ldap
      entry <- con
                 .searchSub(rootDN, AND(IS(OC_MACHINE), EQ(A_MACHINE_UUID, entity.id.value)), "1.1")
                 .map(_.headOption)
                 .notOptional(s"No machine entry found for id '${entity.id.value}'") // TODO: error if more than one !! #555
        ...
        for {
          ...
          res     <- (if (entries.size >= 1) {
                     ...
                     */
                        ...
                        ZIO.foreach(entries)(e => InventoryProcessingLogger.info(s"-> ${e(A_MACHINE_UUID).get}")) *>
