    ...
    (inventory.node.main.status, inventory.machine.status) match {
      case (AcceptedInventory, PendingInventory) =>
        ...
        InventoryProcessingLogger.debug(
          s"Found machine '${inventory.machine.id.value}' in pending DIT but that machine is the container of the accepted node '${inventory.node.main.id.value}'. Moving machine to accpeted"
        ) *>
        (for {
  ...
  override def apply(inventory: Inventory, records: Seq[LDIFChangeRecord]): IOResult[Seq[LDIFChangeRecord]] = {
    ...
    (inventory.node.main.status, inventory.machine.status) match {
      case (RemovedInventory, RemovedInventory) =>
        InventoryProcessingLogger.debug(
          "Found node '%s' and machine '%s' in removed DIT but we received an inventory for it, moving them into pending".format(
      ...
      case (RemovedInventory, _) =>
        InventoryProcessingLogger.debug(
          "Found node '%s' ain removed DIT but we received an inventory for it, moving it into pending and leaving the container alone"
            .format(inventory.node.main.id)
        ) *>
        (for {
