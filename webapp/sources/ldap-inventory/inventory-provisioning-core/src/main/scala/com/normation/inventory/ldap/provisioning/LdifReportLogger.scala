...
class DefaultLDIFInventoryLogger(val LDIFLogDir: String = DefaultLDIFInventoryLogger.defaultLogDir) extends LDIFInventoryLogger {
  ...
  def log(
    ...
    ZIO.when(logger.logEffect.isTraceEnabled) {
      IO.bracket(IO.effect(new LDIFWriter(LDIFFile)))(writer =>
        IO.effect(writer.close).catchAll(ex => logger.debug("LDIF log for inventory processing: " + LDIFFile.getAbsolutePath))
      ) { writer =>
        Task.effect {
