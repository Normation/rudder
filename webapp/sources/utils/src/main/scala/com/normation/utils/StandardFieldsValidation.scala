...
object NodeIdRegex extends RegexValidation {
  ...
  def checkNodeId(uuid: String) : PureResult[String] = {
    if(NodeIdRegex.isValid(uuid)) {
      ...
      Left(Inconsistency(s"""The node ID '${uuid}' is not valid. It should be lesser than 50 chars and contains alpha-numeric char or dash and no dash in first position (ie matches that regex: ${NodeIdRegex.stringPattern})"""))
    }
  }
...
object HostnameRegex extends RegexValidation {
  ...
  def checkHostname(hostname: String) : PureResult[String] = {
    if(HostnameRegex.isValid(hostname)) {
      ...
      Left(Inconsistency(s"""The node ID '${hostname}' is not valid. It should only contains alpha-numeric char or dash and no dash in first position (ie matches that regex: ${HostnameRegex.stringPattern})"""))
    }
  }
}
