...
class Serializer[T](techniques: (String, T => String)*) {
  ...
  def apply(prop: String, value: T): Option[String] = propReg.get(prop) map { f => f(value) }
...
class Unserializer[T](techniques: (String, String => Box[T])*) {
  ...
  def apply(prop: String, value: String): Box[T] = propReg.get(prop) flatMap { f => f(value) }
...
object FilePermsTranslator extends Translator[FilePerms](
      ...
      new Unserializer("self" -> { x => FilePerms(x) })
    )

object SelectFieldTranslator extends Translator[Seq[String]](
      new Serializer("self" -> { s => s.mkString(",") }),
      new Unserializer("self" -> { s => Some(s.split(",").toSeq) })
    )
...
object FileTranslator extends Translator[File](
      ...
      new Unserializer("self" -> { x => if (isEmpty(x)) Empty else Full(new File(x)) })
    )
