  ...
  def getConfigParameters(): IOResult[Seq[RudderWebProperty]] = {
    for {
      ...
      properties <- ZIO.foreach(entries) { entry =>
                      mapper
                        .entry2RudderConfig(entry)
                        .toIO
                        .chainError(
                          s"Error when transforming LDAP entry into an application parameter. Entry: ${entry.toLDIFString()}"
                        )
                    }
    } yield {
      properties
      ...
      res                    <- {
        if (needSave) {
          ...
          for {
            ...
            result   <-
              con.save(propEntry).chainError(s"Error when saving parameter entry in repository: ${propEntry.toLDIFString()}")
            eventLog <-
              modifyGlobalPropertyInfo match {
                case Some(info) =>
                  val modId = ModificationId(uuidGen.newUuid)
                  eventLogRepository.saveModifyGlobalProperty(
                    modId,
                    info.actor,
                    oldProperty,
                    property,
