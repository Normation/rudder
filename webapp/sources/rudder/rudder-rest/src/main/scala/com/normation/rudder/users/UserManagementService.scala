...
object UserManagementIO {
  ...
  def replaceXml(
    ...
    (originalDoc \\ AUTHENTICATION_ROOT_ELT).headOption match {
      ...
      case _             =>
        Left(Unexpected(s"User file has wrong formatting : ${debugFileName}"))
    }
  }
...
object UserManagementService {
  ...
  def addUserXmlRewriteRule(
      newUser:         User,
      isPreHashed:     Boolean,
      passwordEncoder: (String, CharSequence) => String
            ...
            case e: Elem if (e.label == UserManagementIO.AUTHENTICATION_ROOT_ELT) =>
              val hashedUser = {
                if (isPreHashed) {
                  ...
                  newUser.copy(password = passwordEncoder((origXml \ "@hash").text, newUser.password))
                }
              }
  ...
  def updateUserXmlRewriteRule(
    ...
    if (!userExists(id, origXml)) {
      Left(Inconsistency(s"User with id '${id}' does not exist and can't be update"))
    } else {
      Right(new RewriteRule {
        override def transform(n: Node): NodeSeq = n match {
          case user: Elem if (user \ "@name").text == id =>
            ...
            val newPassword = if (password.isEmpty) {
              ...
              else passwordEncoder((origXml \ "@hash").text, password)
            }
    ...
    for {
      ...
      _    <- UserManagementIO.transformUserFile(
                ...
                addUserXmlRewriteRule(
                  newUser,
                  isPreHashed,
                  (hash: String, pwd: CharSequence) => getHashEncoderOrDefault(hash).encode(pwd)
                )
              )
    ...
    for {
      ...
      _     <- ZIO.when(!(userService.authConfig.users.keySet.contains(id))) {
                 ...
                 UserManagementIO.transformUserFile(
                   file,
                   addUserXmlRewriteRule(User(id, "", Set.empty, None), isPreHashed = true, (_, _) => "")
                 )
               }
      _     <- UserManagementIO.transformUserFile(
                 file,
