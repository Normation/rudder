  ...
  def getTechniqueWithVersion(
    ...
    (optTechniqueName, opTechniqueVersion) match {
      ...
      case (Some(techniqueName), None)          => // get last version of technique, with last revision in that case
        IOResult
          .effect(techniqueRepository.getLastTechniqueByName(techniqueName))
    ...
    source match {
      case Some(cloneId) =>
        for {
          ...
          technique    <-
            configRepository.getTechnique(techId).notOptional(s"Technique with ID '${techId.debugString}' was not found")
