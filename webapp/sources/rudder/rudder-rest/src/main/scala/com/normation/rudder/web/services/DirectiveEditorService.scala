...
class DirectiveEditorServiceImpl(techniqueRepository: ConfigurationRepository, section2FieldService: Section2FieldService)
    extends DirectiveEditorService {
  ...
  override def get(
      techniqueId:   TechniqueId,
      directiveId:   DirectiveUid,
      withVarValues: Map[String, Seq[String]] = Map()
  ): Box[DirectiveEditor] = {
    ...
    for {
      ...
      pol    <-
        techniqueRepository
          ...
          .notOptional(s"Error when looking for technique with ID '${techniqueId.debugString}'. Check technique name and version")
          .toBox
