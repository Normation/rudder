...
class GenericConfigService(
  ...
  implicit private[this] def toRuddercTargets(p: RudderWebProperty):   Set[RuddercTarget] = {
    ...
    ruddercTargetsDecoder.decodeJson(p.value).map(_.values) match {
      ...
      case Left(err)      =>
        logEffect.warn(
          s"Error when trying the read rudderc targets from settings (using default: ${RuddercTarget.defaultTargets.map(_.name).mkString(", ")}): ${err}"
        )
        RuddercTarget.defaultTargets
    }
  }
  implicit private[this] def serRuddercTargets(x: Set[RuddercTarget]): String             = {
  ...
  def set_rudder_generation_rudderc_enabled_targets(targets: Set[RuddercTarget]): IOResult[Unit]               =
    save("rudder_generation_rudderc_enabled_targets", targets)
}
