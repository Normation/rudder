...
final case class ByRuleValueCompliance(
  ...
  def status: ReportType = ReportType.getWorseType(nodes.flatMap(_.values).map(_.status))
...
final case class ByRuleByNodeByDirectiveByValueCompliance(
  ...
  override def status: ReportType = ReportType.getWorseType(values.map(_.status))
}
