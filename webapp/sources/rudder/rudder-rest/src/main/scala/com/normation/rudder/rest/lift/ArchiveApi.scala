...
class SaveArchiveServicebyRepo(
  ...
  def saveRuleCategory(eventMetadata: EventMetadata, r: RuleCategoryArchive):    IOResult[Unit] = {
    for {
      root        <- roRuleCategoryRepos.getRootCategory()
      parents      = r.parentCategories
      created     <- Ref.make(Set.empty[RuleCategoryId])
      // naive version
      // _       <- ZIO.foreach(r.children)(c => {
      //              if (root.contains(c.id)) {
      //                woRuleCategoryRepos.updateAndMove(
      //                  c.transformInto[RuleCategory],
      //                  parents(c).id, // every child category has a parent
      //                  eventMetadata.modId,
      //                  eventMetadata.actor,
      //                  eventMetadata.msg
      //                )
      //              } else {
      //                created.get.flatMap(existing => {
      //                  ZIO.unless(existing.contains(c.id))(
      ...
      //              }
      //            })
      childMap     = r.childCategories
      newRoot      = r.toRuleCategory
      movedMap     = childMap.filter {
                       case (k, _) =>
                         val newParents = newRoot.findParents(k)
                         val oldParents = root.findParents(k)
                         if (newParents != oldParents) println(s"new parents : ${newParents}, old parents : ${oldParents}")
                         root.contains(k) && newRoot.findParents(k) != root.findParents(k)
                     }
      _           <- ZIO.foreachDiscard(movedMap) {
                       case (id, cat) =>
                         val parent = parents(cat)
                         // create the parent and all great-parents, or else creating the child will not succeed
                         def recCreateParents(c: RuleCategoryArchive): IOResult[Unit] = {
                           ZIO
                             .unless(c.id == root.id || root.contains(c.id)) {
                               val create = !root.contains(c.id) && newRoot.contains(c.id)
                               // create starting from the greatest parent
                               recCreateParents(parents(c)) *>
                               (if (create) {
                                  woRuleCategoryRepos.create(
                                    c.toRuleCategory,
                                    parents(c).id,
                                  ...
                                  ApplicationLoggerPure.Archive.trace(
                                    s"Created rule category parent from archive: '${cat.name}' (${id.value})"
                                  )
                                } else {
                                  woRuleCategoryRepos.updateAndMove(
                                    c.toRuleCategory,
                                    ...
                                    s"Moved rule category parent from archive: '${cat.name}' (${id.value}) to ${parents(c).id}"
                                  )
                                })
                             }
                             .unit
                         }
                         recCreateParents(parent)
                     }
      toCreateMap <- created.get.map(existing => childMap.filterNot { case (k, _) => existing.contains(k) || root.contains(k) })
      _           <- ZIO.foreachDiscard(toCreateMap) {
                       case (id, cat) =>
                         val parent   = parents(cat)
                         ...
                         ApplicationLoggerPure.Archive.trace(s"Updated rule category from archive: '${cat.name}' (${id.value})")
                     }
    } yield ()
  }
