  ...
  object Import extends LiftApiModule0 {
    ...
    def process0(version: ApiVersion, path: ApiPath, req: Req, params: DefaultParams, authzToken: AuthzToken): LiftResponse = {
      ...
      def parseArchive(archive: FileParamHolder): IOResult[PolicyArchive] = {
        ...
        IOResult
          .effect(archive.fileStream)
          .bracket(is => effectUioUnit(is.close()))(is => ZipUtils.getZipEntries(originalFilename, is))
...
class ZipArchiveBuilderService(
  ...
  def getJsonZippableContent(json: String): (InputStream => IOResult[Any]) => IOResult[Any] = {
    (use: InputStream => IOResult[Any]) =>
      IOResult.effect(new ByteArrayInputStream(json.getBytes(StandardCharsets.UTF_8))).bracket(is => effectUioUnit(is.close)) {
        is => use(is)
      }
  }
  ...
  def getTechnique(
      ...
      techniques:  RefM[Map[TechniqueId, (Chunk[TechniqueCategoryName], Technique)]]
  ): IOResult[(Chunk[TechniqueCategoryName], Technique)] = {
    techniques.modify(cache => {
...
class SaveArchiveServicebyRepo(
  ...
  def saveTechnique(eventMetadata: EventMetadata, t: TechniqueArchive): IOResult[Unit] = {
    ...
    for {
      ...
      existing <-
        IOResult.effect(if (techniqueDir.exists) {
      ...
      _ <- ZIO.foreach_(updated) { u =>
             if (u.state == ResourceFileState.Deleted) {
               IOResult.effect(File(techniqueDir.pathAsString).delete())
             } else ZIO.unit
           }
