...
object JRuleCategories {
  ...
  implicit lazy val transformer:        Transformer[RuleCategory, JRuleCategories]        = {
    Transformer
      ...
      .withFieldRenamed(_.childs, _.children)
      .buildTransformer
  }
...
object RuleCategoryArchive {
  implicit lazy val transformer: Transformer[RuleCategoryArchive, RuleCategory] = {
    Transformer
      .define[RuleCategoryArchive, RuleCategory]
      .withFieldRenamed(_.children, _.childs)
      .withFieldConst(_.isSystem, false)
      .buildTransformer
  }
}
...
class SaveArchiveServicebyRepo(
  ...
  def saveRuleCategory(eventMetadata: EventMetadata, r: RuleCategoryArchive):    IOResult[Unit] = {
    for {
      ...
      _      <- ZIO.foreachDiscard(r.childCategories) {
                  case (id, cat) =>
                    val parent   = parents(cat) // every child category has a parent
                    val parentId = r.id
                    // create the parent, or creating the child will not succeed
                    ZIO.unless(root.contains(parentId)) {
                      woRuleCategoryRepos.create(
                        parent.transformInto[RuleCategory],
                        parentId,
                        eventMetadata.modId,
                        eventMetadata.actor,
                        eventMetadata.msg
                    ...
                    (if (root.contains(id)) {
                       woRuleCategoryRepos.updateAndMove(
                         cat.transformInto[RuleCategory],
                         parentId,
                         eventMetadata.modId,
                         eventMetadata.actor,
                         eventMetadata.msg
                       ) *>
                       ApplicationLoggerPure.Archive.debug(s"Updated rule category from archive: '${cat.name}' (${id.value})")
                     } else {
                       woRuleCategoryRepos.create(
                         cat.transformInto[RuleCategory],
                         parentId,
                       ...
                       ApplicationLoggerPure.Archive.debug(s"Created rule category from archive: '${cat.name}' (${id.value})")
                     })
                }
    } yield ()
  }
