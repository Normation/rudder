...
/// utility functions ///
  ...
  def saveNode(nodeDetails: Rest.NodeDetails, eventActor: EventActor): IO[CreationError, NodeId] = {
    ...
    implicit val mod = ChangeContext(ModificationId(uuidGen.newUuid), eventActor, None)

    for {
      _         <- NodeLoggerPure.PendingNode.debug(
                     s"API creation of new node '${nodeDetails.hostname}' [${nodeDetails.id}] with target status '${nodeDetails.status}'"
                   )
      validated <- toCreationError(Validation.toNodeTemplate(nodeDetails))
      _         <- NodeLoggerPure.PendingNode.debug(s"Data for node '${nodeDetails.hostname}' [${nodeDetails.id}] validated")
      _         <- checkUuid(validated.inventory.node.main.id)
      created   <- saveInventory(validated.inventory)
      _         <- NodeLoggerPure.PendingNode.debug(
                     s"Node inventory for '${nodeDetails.hostname}' [${nodeDetails.id}] saved in pending, checking for acceptation"
                   )
      nodeSetup <- accept(validated)
      nodeId    <- saveRudderNode(validated.inventory.node.main.id, nodeSetup)
      _         <- NodeLoggerPure.PendingNode.info(
                     s"New node '${nodeDetails.hostname}' [${nodeDetails.id}] with target status '${nodeDetails.status}' create by API"
                   )
    } yield {
      nodeId
    }
  ...
  def checkUuid(nodeId: NodeId): IO[CreationError, Unit] = {
    ...
    def error(status: InventoryStatus) = Inconsistency(
      s"A node with id '${nodeId.value}' already exists with status '${status.name}'"
    ).fail
    (nodeFactRepository
      .getStatus(nodeId)
      .flatMap { s =>
        s match {
          case PendingInventory | AcceptedInventory => error(s)
  ...
  def accept(template: NodeTemplate)(implicit mod: ChangeContext): IO[CreationError, NodeSetup] = {
    ...
    template match {
      case AcceptedNodeTemplate(_, properties, policyMode, state) =>
        NodeLoggerPure.PendingNode.debug(
          s"Accepting API created node '${template.inventory.node.main.hostname}' [${id.value}]"
        ) *>
        newNodeManager
          .accept(id, mod.modId, mod.actor)
          .toIO
  ...
  def mergeNodeSetup(node: NodeFact, changes: NodeSetup): NodeFact = {
    import com.softwaremill.quicklens._
    ...
    node
      ...
      .modify(_.properties)
      .setTo(Chunk.fromIterable(properties))
  }
   ...
   */
  def saveRudderNode(id: NodeId, setup: NodeSetup)(implicit mod: ChangeContext): IO[CreationError, NodeId] = {
    nodeFactRepository
      .update(id, n => mergeNodeSetup(n, setup))
      .map(_ => id)
