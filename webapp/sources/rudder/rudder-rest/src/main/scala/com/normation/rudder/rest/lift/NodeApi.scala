        ...
        val array = ids.map { id =>
          val status = {
            if (accepted.contains(id)) AcceptedInventory.name
              ...
              "deleted" // RemovedInventory would output "removed" which is inconsistant with other API
            else "unknown"
          }
        ...
        case eb: EmptyBox =>
          JsonResponse(
            JObject(
              JField(
                "error",
                (eb ?~! s"An error occurred while fetching verions of '${software}' software for nodes").messageChain
              )
            )
          )
      }
...
//    val url = s"http://localhost/rudder/relay-api/remote-run/nodes/${nodeId.value}"
  ...
  def runNode[A](nodeId: NodeId, classes: List[String]): OutputStream => Unit = {
    ...
    def errorMessageWithHint(s: String) =
      s"Error occured when contacting internal remote-run API to apply classes on Node '${nodeId.value}': ${s}"
        ...
        opt <- IOResult.effect {
                 request.exec {
                   case (status, headers, is) =>
                     NodeLogger.debug(
                       s"Processing remore-run on ${nodeId.value}: HTTP status ${status}"
