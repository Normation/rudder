...
class SystemApiService11(
  ...
  private[this] def restoreByDatetime(
    ...
    (for {
      valideDate <- IOResult.effect(s"The given archive id is not a valid archive tag: ${datetime}")(
  ...
  private[this] def getZip(
    ...
    (ZIO
      .bracket(IOResult.effect(new RevWalk(repo.db)))(rw => effectUioUnit(rw.dispose)) { rw =>
        for {
