...
class SettingsApi(
  ...
  case object RestRudderVerifyCertificates extends RestBooleanSetting {
    ...
    val startPolicyGeneration = true
    def get: IOResult[Boolean]                                       = configService.rudder_verify_certificates()
    def set: (Boolean, EventActor, Option[String]) => IOResult[Unit] = {
      (value: Boolean, actor: EventActor, reason: Option[String]) =>
