...
class SettingsApi(
  ...
  final case object RestReportingMode               extends RestSetting[ComplianceModeName]         {
    ...
    def set                       = (value: ComplianceModeName, actor: EventActor, reason: Option[String]) =>
  ...
  case object RestRudderVerifyCertificates extends RestBooleanSetting {
    ...
    def set                   = (value: Boolean, actor: EventActor, reason: Option[String]) =>
      configService.set_rudder_verify_certificates(value, actor, reason)
  }
  ...
  final case object RestRuddercTargets extends RestSetting[Set[RuddercTarget]] {
    ...
    override def parseJson(json: JValue):           Box[Set[RuddercTarget]] = {
      def err(x: JValue) = Left(
        s"Can not parse '${prettyRender(x)}' as a rudderc target, accepted values are: '${RuddercTarget.all.map(_.name).mkString("', '")}'"
      ).toIO
      (json match {
        case JArray(values) =>
          values.accumulate {
