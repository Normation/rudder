...
trait TraitTestApiFromYamlFiles extends Specification {
  ...
  def readYamlFiles(baseDir: String, tmpDir: File, only: String => Boolean): IOResult[List[(String, List[AnyRef])]] = {
    ...
    def copyTransform(orig: Path, destDir: File): IOResult[(String, Managed[RudderError, InputStream])] = {
      ...
      for {
        f <- IOResult
               .effect(Resource.asString(orig.toString).map(s => dest.write(transform(name, s))))
    ...
    for {
      ...
      baseIs     = baseFiles.map { name =>
                     (
                       name,
                       Managed.make(IOResult.effect(Resource.getAsStream(baseDir + "/" + name)))(is => effectUioUnit(is.close()))
                     )
                   }
