                   ...
                   semaphore.withPermit(
                     for {
                       ...
                       _        <- vars.flatten.accumulate {
                                     case (name, value, errorMsg) =>
                                       IOResult.effectNonBlocking(errorMsg)(template.setAttribute(name, value))
                                   }
                       // return the actual template with replaced variable in case of success
                       result   <-
                         IOResult.effectNonBlocking("An error occured when converting template to string")(template.toString())
              ...
              x     <- c.get(content) match {
                         ...
                         case None          =>
                           for {
                             ...
                             _ <- p.complete(for {
                                    parsed  <- IOResult
                                                 .effect(s"Error when trying to parse template '${templateName}'") {
