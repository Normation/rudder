...
class TestMigrateSystemTechniques7_0 extends Specification {
  ...
  val testEnv = new TestTechniqueRepo(
    ...
    optGitRevisionProvider =
      Some((repo: GitRepositoryProvider) => new LDAPGitRevisionProvider(ldap, rudderDit, repo, "refs/heads/master"))
  )
  ...
  private[this] lazy val ldapEntityMapper: LDAPEntityMapper =
    new LDAPEntityMapper(rudderDit, nodeDitImpl, acceptedNodesDitImpl, queryParser, inventoryMapper)
  ...
  testEnv.techniqueRepository.registerCallback(
...
//  org.slf4j.LoggerFactory.getLogger("com.normation.rudder.services.policies").asInstanceOf[ch.qos.logback.classic.Logger].setLevel(ch.qos.logback.classic.Level.TRACE)
  ...
  "The in memory LDAP directory" should {
    ...
    "have 6 more entries for new actives directives after tech lib reload" in {
      ...
      FileUtils.moveDirectory(
        new File(testEnv.configurationRepositoryRoot, "new-techniques"),
        new File(testEnv.configurationRepositoryRoot, "techniques")
      )
  ...
  "Group name of 'all-nodes-with-cfengine-agent' should have been modified" in {
    (Option(ldap.server.getEntry(s"nodeGroupId=all-nodes-with-cfengine-agent,${groupDn}"))
      .map(_.getAttribute(A_NAME).getValue))
  ...
  "Group name of 'hasPolicyServer-root' should have been modified" in {
    (Option(ldap.server.getEntry(s"nodeGroupId=hasPolicyServer-root,${groupDn}"))
      .map(_.getAttribute(A_NAME).getValue))
  ...
  "Group description of 'all-nodes-with-cfengine-agent' should have been modified" in {
    (Option(ldap.server.getEntry(s"nodeGroupId=all-nodes-with-cfengine-agent,${groupDn}"))
      .map(_.getAttribute(A_DESCRIPTION).getValue))
  ...
  "When initialized with 6.2 data, migration is needed" in {
    ...
    (Option(ldap.server.getEntry(s"ruleId=hasPolicyServer-root,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
    must containTheSameElementsAs(List("common-root")))
    (Option(ldap.server.getEntry(s"ruleId=hasPolicyServer-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
  ...
  def checkNewEntries(prefix: String) = { // root policy server
    Try(
      ldap.server.assertEntriesExist(
        ...
        s"ruleId=${prefix}policy-server-root,${ruleDn}", // server relay
    ...
    (Option(ldap.server.getEntry(s"ruleId=${prefix}hasPolicyServer-root,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
    must containTheSameElementsAs(List("common-hasPolicyServer-root"))) and
    (Option(ldap.server.getEntry(s"ruleId=${prefix}hasPolicyServer-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
    must containTheSameElementsAs(List("common-hasPolicyServer-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2"))) and
    (Option(ldap.server.getEntry(s"ruleId=${prefix}policy-server-root,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
    ...
    (Option(ldap.server.getEntry(s"ruleId=${prefix}policy-server-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2,${ruleDn}"))
      .map(_.getAttribute("directiveId").getValues.toList)
      .getOrElse(Nil)
    must containTheSameElementsAs(
      List(
        "server-common-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2",
        "rudder-service-apache-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2",
        "rudder-service-relayd-ba7e3ca5-a967-40d8-aa97-41a3ff450fd2"
      )
    ))
  }
  ...
  "We can create new config objects root policy server and relay1" in {
    ...
    checkMigrate.migrateConfigs6_x_7_0
      .createNewConfigsAll(List(NodeId("root"), NodeId("ba7e3ca5-a967-40d8-aa97-41a3ff450fd2")))
      .runNow
  ...
  "We can finish migration and clean up and all new entries STILL exists and rules are migrated" in {
    ...
    checkMigrate.migrateConfigs6_x_7_0
      .finalMoveAndCleanAll(List(NodeId("root"), NodeId("ba7e3ca5-a967-40d8-aa97-41a3ff450fd2")))
      .runNow
    ...
    checkNewEntries("") and
    (Option(ldap.server.getEntry(s"ruleId=44444444-02fd-43d0-aab7-28460a91347b,${ruleDn}"))
      .map(_.getAttribute("ruleTarget").getValues.toList)
      .getOrElse(Nil)
    must containTheSameElementsAs(
      List("""{"include":{"or":["special:all_exceptPolicyServers"]},"exclude":{"or":["special:all_policyServers"]}}""")
    )) and
    (Option(ldap.server.getEntry(s"ruleId=55555555-02fd-43d0-aab7-28460a91347b,${ruleDn}"))
      .map(_.getAttribute("ruleTarget").getValues.toList)
      .getOrElse(Nil)
