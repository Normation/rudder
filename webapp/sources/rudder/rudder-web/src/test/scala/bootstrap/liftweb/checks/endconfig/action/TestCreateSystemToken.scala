...
import bootstrap.liftweb.checks.endconfig.action.CreateSystemToken
...
class TestCreateSystemToken extends Specification {
  ...
  "When writing the system tokens, we" should {
    "generate proper header and token files" in File.temporaryDirectory("rudder-test-system-token-") { tmpDir =>
      ...
      tokenFile.exists must beTrue
      tokenFile.contentAsString must beEqualTo(token)
      tokenFile.permissionsAsString must beEqualTo("rw-------")

      val tokenHeaderFile = tmpDir / CreateSystemToken.tokenHeaderFile
      tokenHeaderFile.exists must beTrue
      tokenHeaderFile.contentAsString must beEqualTo(s"x-api-token: ${token}")
      tokenHeaderFile.permissionsAsString must beEqualTo("rw-------")
    }
  }
}
