...
view model =
    let
        selectedMode =
            model.selectedSettings.policyMode
        ...
        policyModeLabel mode =
            case mode of
                ...
                Default ->
                    span [] [ text "Global mode", span [ class ("mode" ++ policyModeClass model.globalPolicyMode) ] [] ]

                _ ->
                    span [ class ("node mode" ++ policyModeClass mode) ] [ text (policyModeStr mode) ]
                ...
                _ ->
                    label
                        ...
                        [ if mode == Default then
                            span [] [ text "Global mode", span [ class ("mode" ++ policyModeClass model.globalPolicyMode) ] [ text (String.toLower (policyModeStr model.globalPolicyMode)) ] ]

                          else
                            text (policyModeStr mode)
                        ]
        ...
        btnSave saving disable modalSettings =
            let
                ...
                msg =
                    modalSettings |> Maybe.map OpenModal |> Maybe.withDefault (StartSaving Nothing)
            in
            button
                [ class
                    ("btn btn-success btn-save btn-settings"
                        ++ (if saving then
                                " saving"
                            ...
                            , div [ class "form-group" ]
                                [ div [ class "btn-group yesno" ]
                                    [ label
                                        [ class
                                            ("btn btn-default"
                                                ++ (if model.selectedSettings.overridable then
                                                        " active"
                                        ...
                                        , onClick (SelectOverrideMode True)
                                        ]
                                        [ text "Yes" ]
                                    , label
                                        [ class
                                        ...
                                        , onClick (SelectOverrideMode False)
                                        ]
                                        [ text "No" ]
                                    ]
...
explainPolicyMode mode =
    case mode of
        Default ->
            Just "This may be overridden on a per-directive basis."

        Audit ->
            Just "Directives will never be enforced on this node, regardless of their policy mode."

        Enforce ->
            Just "All directives will apply necessary changes on this node, except directives with an 'Audit' override setting."
