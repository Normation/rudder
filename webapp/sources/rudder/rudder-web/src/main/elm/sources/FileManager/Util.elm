...
set index item list =
    toList <| Array.set index item <| fromList list
...
swap index1 index2 list =
    toList <| arraySwap index1 index2 <| fromList list
...
arraySwap index1 index2 array =
    ...
    case ( maybe1, maybe2 ) of
        ( Just item1, Just item2 ) ->
            Array.set index1 item2 <| Array.set index2 item1 array
...
button atts childs =
    Html.button (type_ "button" :: atts) childs
            ...
            case err of
                ...
                Detailed.BadStatus metadata body ->
                    case Json.Decode.decodeString decoder body of
                        Ok { error } ->
                            formatApiNameMessage (Maybe.withDefault "Unknown error" error)

                        Err _ ->
                            formatApiNameMessage "Unable to parse server response"
