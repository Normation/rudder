...
import Html.Attributes exposing (attribute, checked, class, property, title, type_)
...
getAllComplianceValues complianceDetails =
    let
        ...
        valSuccessNotApplicable =
            getValueCompliance complianceDetails.successNotApplicable

        -- 0
        valSuccessAlreadyOK =
            getValueCompliance complianceDetails.successAlreadyOK

        -- 0
        valSuccessRepaired =
            getValueCompliance complianceDetails.successRepaired

        -- 0
        valAuditCompliant =
            getValueCompliance complianceDetails.auditCompliant

        -- 0
        valAuditNotApplicable =
            getValueCompliance complianceDetails.auditNotApplicable

        -- 0
        valAuditNonCompliant =
            getValueCompliance complianceDetails.auditNonCompliant

        -- 1
        valError =
            getValueCompliance complianceDetails.error

        -- 2
        valAuditError =
            getValueCompliance complianceDetails.auditError

        -- 2
        valUnexpectedUnknownComponent =
            getValueCompliance complianceDetails.unexpectedUnknownComponent

        -- 3
        valUnexpectedMissingComponent =
            getValueCompliance complianceDetails.unexpectedMissingComponent

        -- 3
        valBadPolicyMode =
            getValueCompliance complianceDetails.badPolicyMode

        -- 3
        valApplying =
            getValueCompliance complianceDetails.applying

        -- 4
        valReportsDisabled =
            getValueCompliance complianceDetails.reportsDisabled

        -- 5
        valNoReport =
            getValueCompliance complianceDetails.noReport

        -- 6
        okStatus =
            valSuccessNotApplicable + valSuccessAlreadyOK + valSuccessRepaired + valAuditCompliant + valAuditNotApplicable

        nonCompliant =
            valAuditNonCompliant

        error =
            valError + valAuditError

        unexpected =
            valUnexpectedUnknownComponent + valUnexpectedMissingComponent + valBadPolicyMode
        ...
        roundedCompliance =
            compliances
                |> Dict.map
                    (\k a ->
                        if List.member k getNumbersToUpdate then
                            { a | floor = a.floor + 1 }
...
sumPercent compliance =
    ...
    in
    [ compliance.successAlreadyOK
    , compliance.applying
    , compliance.auditCompliant
    , compliance.auditError
    , compliance.auditNonCompliant
    , compliance.auditNotApplicable
    , compliance.badPolicyMode
    , compliance.error
    , compliance.noReport
    , compliance.reportsDisabled
    , compliance.successNotApplicable
    , compliance.successRepaired
    , compliance.unexpectedMissingComponent
    , compliance.unexpectedUnknownComponent
    ]
        |> List.map getValue
        |> List.sum
...
mergeCompliance c1 c2 =
    let
        ...
        toMaybeFloat =
            \fun -> sumMaybeFloat (fun c1) (fun c2)
    in
    ComplianceDetails
...
filterCompliance complianceDetails complianceFilters =
    let
        dict s =
            case s of
                "successAlreadyOK" ->
                    \v -> { v | successAlreadyOK = Nothing }

                "auditCompliant" ->
                    \v -> { v | auditCompliant = Nothing }

                "successRepaired" ->
                    \v -> { v | successRepaired = Nothing }

                "successNotApplicable" ->
                    \v -> { v | successNotApplicable = Nothing }

                "auditNotApplicable" ->
                    \v -> { v | auditNotApplicable = Nothing }

                "auditNonCompliant" ->
                    \v -> { v | auditNonCompliant = Nothing }

                "error" ->
                    \v -> { v | error = Nothing }

                "auditError" ->
                    \v -> { v | auditError = Nothing }

                "unexpectedMissingComponent" ->
                    \v -> { v | unexpectedMissingComponent = Nothing }

                "unexpectedUnknownComponent" ->
                    \v -> { v | unexpectedUnknownComponent = Nothing }

                "badPolicyMode" ->
                    \v -> { v | badPolicyMode = Nothing }

                "applying" ->
                    \v -> { v | applying = Nothing }

                "reportsDisabled" ->
                    \v -> { v | reportsDisabled = Nothing }

                "noReport" ->
                    \v -> { v | noReport = Nothing }
        ...
        statuses =
            if List.isEmpty complianceFilters.selectedStatus then
                []

            else if complianceFilters.showOnlyStatus then
        ...
        || (filteredDict
                |> Dict.Extra.any
                    (\k val ->
                        let
                            isSelected =
                                List.member k complianceFilters.selectedStatus
                        in
...
filterDetailsByCompliance filter =
    \i ->
        checkFilterCompliance i.complianceDetails filter


filterValueByCompliance filter =
    \i ->
        let
            isSelected =
                List.member i.status filter.selectedStatus
        in
        List.isEmpty filter.selectedStatus
            || (if filter.showOnlyStatus then
                    isSelected
...
filterReports filter =
    \r ->
        let
            isSelected =
                List.member r.status filter.selectedStatus
        in
        List.isEmpty filter.selectedStatus
            || (if filter.showOnlyStatus then
                    isSelected
...
filterReportsByCompliance filter =
    \i ->
        List.isEmpty filter.selectedStatus
            || (i.reports
    ...
    if complianceFilters.showComplianceFilters then
        let
            ...
            statusDropdown status substatus =
                let
                    allSelected =
                        substatus |> List.all (\s -> List.member s selectedStatus)

                    anySelect =
                        substatus |> List.any (\s -> List.member s selectedStatus)

                    indeterminate =
                        property "indeterminate"
                            (if not allSelected && anySelect then
                    ...
                    newSelection =
                        if allSelected then
                            selectedStatus |> List.filter (\s -> List.Extra.notMember s substatus)

                        else
                            List.Extra.unique (List.append selectedStatus substatus)
                in
                        ...
                        (substatus
                            |> List.map
                                (\s ->
                                    let
                                        isSelected =
                                            List.member s selectedStatus

                                        subSelection =
                                            if isSelected then
                                                List.Extra.remove s selectedStatus
            ...
            , div [ class "form-group" ]
                [ div [ class "btn-group " ]
                    [ label
                        ...
                        , onClick (updateAction { complianceFilters | showOnlyStatus = True })
                        ]
                        [ text "Show only" ]
                    , label
                        [ class
                        ...
                        , onClick (updateAction { complianceFilters | showOnlyStatus = False })
                        ]
                        [ text "Hide" ]
                    ]
                , div [ class "btn-group" ]
                ...
                (complianceFilters.selectedStatus
                    |> List.map
                        (\s ->
                            let
                                className =
                                    case Dict.Extra.find (\status substatus -> List.member s substatus) complianceStatusGroups of
                                        Just ( st, sbst ) ->
                                            st
...
buildTooltipContent title content =
    let
        ...
        contentTag =
            "</h4><div class='tooltip-inner-content'><pre>"
...
badgePolicyMode globalPolicyMode policyMode =
    let
        ...
        msg =
            case mode of
                "enforce" ->
                    "<div style='margin-bottom:5px;'>This rule is in <b>enforce</b> mode.</div>" ++ defaultMsg

                "audit" ->
                    "<div style='margin-bottom:5px;'>This rule is in <b>audit</b> mode.</div>" ++ defaultMsg
