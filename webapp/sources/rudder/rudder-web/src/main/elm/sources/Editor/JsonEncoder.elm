...
encodeTechniqueConstraint constraint =
    let
        allowEmpty =
            constraint.allowEmpty |> Maybe.map (\v -> [ ( "allow_empty_string", bool v ) ]) |> Maybe.withDefault []

        allowWp =
            constraint.allowWhiteSpace |> Maybe.map (\v -> [ ( "allow_whitespace_string", bool v ) ]) |> Maybe.withDefault []

        max =
            constraint.maxLength |> Maybe.map (\v -> [ ( "max_length", int v ) ]) |> Maybe.withDefault []

        min =
            constraint.minLength |> Maybe.map (\v -> [ ( "min_length", int v ) ]) |> Maybe.withDefault []

        regex =
            constraint.matchRegex |> Maybe.map (\v -> [ ( "regex", string v ) ]) |> Maybe.withDefault []

        notRegex =
            constraint.notMatchRegex |> Maybe.map (\v -> [ ( "not_regex", string v ) ]) |> Maybe.withDefault []

        select =
            constraint.select |> Maybe.map (\v -> [ ( "select", list encodeSelectOption v ) ]) |> Maybe.withDefault []
    in
...
encodeTechniqueParameters param =
    let
        ...
        constraint =
            if param.constraints == defaultConstraint then
                []
...
appendPolicyMode pm base =
    case pm of
        ...
        Just mode ->
            object (( "policyMode", encodePolicyMode mode ) :: base)
