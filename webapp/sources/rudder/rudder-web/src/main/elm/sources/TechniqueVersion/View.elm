...
import Html.Attributes exposing (attribute, checked, class, disabled, for, id, title, type_, value)
...
view model =
    div [ id "techniqueVersion" ]
        ...
        , if model.ui.hasWriteRights then
            let
                createAction =
                    case last model.techniques of
                        Just t ->
                            Create t.version

                        Nothing ->
                            Ignore "Unknown technique version"
            in
            div [ class "space-top" ]
                [ button [ type_ "button", id "addButton", class "btn btn-success new-icon", onClick createAction ]
                    -- ng-click "techniques[techniques.length-1].action()"
