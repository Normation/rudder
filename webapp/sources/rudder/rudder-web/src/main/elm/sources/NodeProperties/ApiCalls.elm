...
deleteProperty property model =
    let
        successMsg =
            "property '" ++ property.name ++ "' has been removed"
...
findPropertyUsage propertyName model =
    let
        queryFilter =
            "is:Directive,Technique in:dir_param_value,technique_method_value "

        property =
            "${node.properties[" ++ propertyName ++ "]"

        param =
            string "value" (queryFilter ++ property)

        paramLimit =
            int "limit" 1000

        urlTest =
            Url.Builder.relative (model.contextPath :: "secure" :: "api" :: "quicksearch" :: []) [ param, paramLimit ]
