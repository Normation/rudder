...
handleEnvMsg msg model =
    case msg of
        Open () ->
            ( { model | open = True, dir = [ "/" ] }, Cmd.none )

        Close ->
            ( { model | open = False, selected = [] }, close [] )

        Accept ->
            ( { model | open = False, selected = [] }, close <| reverse <| map ((++) (getDirPath model.dir) << .name) model.selected )
        ...
        MouseDown maybe pos1 ctrl ->
            ( { model
                ...
                , selected =
                    case maybe of
                        Just file ->
                            if member file model.selected then
                                if ctrl then
                                    filter ((/=) file) model.selected
        ...
        GetBounds result ->
            case result of
                Ok elements ->
                    ( { model | bounds = map .element elements }, Cmd.none )
            ...
            if buttons == 2 then
                ( { model
                    ...
                    , showContextMenu =
                        case maybe of
                            Just file ->
                                not <| getDirPath model.dir == model.clipboardDir && member file model.clipboardFiles
                ...
                ( { model
                    ...
                    , selected =
                        if model.showBound then
                            map second <| filter (touchesBound model.bound << first) <| zip model.bounds model.files

                        else
                            case maybe of
                                Just file ->
                                    if model.drag || model.ctrl then
                                        model.selected
                    ...
                    , load =
                        case maybe of
                            Just file ->
                                if model.drag && (file.type_ == "dir") && (not << member file) model.selected then
                                    True
                ...
                , case maybe of
                    Just file ->
                        if model.drag && (file.type_ == "dir") && (not << member file) model.selected then
                            move model.api (getDirPath model.dir) model.selected <| "/" ++ file.name ++ "/"
            ...
            case result of
                Ok files ->
                    let
                        ...
                        newTree =
                            case Dict.get folder model.tree of
                                Just ti ->
                                    Dict.insert folder (TreeItem folder ti.parents childs) model.tree

                                Nothing ->
                                    let
                                        values =
                                            Dict.values model.tree

                                        parents =
                                            case List.Extra.find (\ti -> List.member folder ti.childs) values of
                                                Just p ->
                                                    List.append [ p.name ] p.parents
                        ...
                        openedRows =
                            Dict.insert folder ( folder, Asc ) tableFilters.openedRows

                        newFilters =
                            { tableFilters | openedRows = openedRows }
                    in
        ...
        Refresh result ->
            case result of
                Ok () ->
                    ( model, listDirectory model.api model.dir )
        ...
        GotContent result ->
            case model.dialogState of
                Edit f _ ->
                    case result of
                        Ok content ->
                            ( { model | dialogState = Edit f content }, Cmd.none )
