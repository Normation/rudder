...
techniqueList model techniques =
    let
        strFilter =
            String.toLower (String.trim model.techniqueFilter.filter)

        filteredTechniques =
            List.sortWith (\t1 t2 -> N.compare t1.name t2.name) (List.filter (\t -> String.contains strFilter (String.toLower t.name) || String.contains strFilter (String.toLower t.id.value)) techniques)
    ...
    div [ class "template-sidebar sidebar-left col-techniques", onClick OpenTechniques ]
        [ div [ class "sidebar-header" ]
            [ div [ class "header-title" ]
                ...
                , div [ class "header-buttons", hidden (not model.hasWriteRights) ]
                    [ -- Need to add technique-write rights
...
allMethodCalls call =
    case call of
        ...
        Block _ b ->
            List.concatMap allMethodCalls b.calls
...
draftsItem model draft =
    let
        ...
        hasDeprecatedMethod =
            List.any (\m -> Maybe.Extra.isJust m.deprecated) (List.concatMap (\c -> Maybe.Extra.toList (Dict.get c.methodName.value model.methods)) (List.concatMap allMethodCalls draft.technique.elems))
    in
    li [ class "jstree-node jstree-leaf" ]
        ...
        , a [ class ("jstree-anchor " ++ activeClass), onClick (SelectDraft draft.id) ]
            ...
            , span [ class "treeGroupName" ]
                [ text
                    (if String.isEmpty draft.technique.name then
                        "<unamed draft>"

                     else
                        draft.technique.name
                    )
                ]
...
techniqueItem model technique =
    let
        directiveUsed =
            List.filter (\d -> d.techniqueName == technique.id.value) model.directives

        isDirectiveExist =
            not (List.isEmpty directiveUsed)

        techniqueIsUsed =
            if isDirectiveExist then
                "used"
        ...
        techniqueIsUsedMsg =
            if isDirectiveExist then
                String.Extra.pluralize "directive" "directives" (List.length directiveUsed) ++ " based on this technique"

            else
                "No directives based on this technique"

        techniqueMethods =
            List.concatMap allMethodCalls technique.elems

        unknownMethods =
            List.filter (\c -> Maybe.Extra.isNothing (Dict.get c.methodName.value model.methods)) techniqueMethods
        ...
        hasDeprecatedMethod =
            List.any (\m -> Maybe.Extra.isJust m.deprecated) (List.concatMap (\c -> Maybe.Extra.toList (Dict.get c.methodName.value model.methods)) (List.concatMap allMethodCalls technique.elems))

        hasUnknownMethod =
            if List.isEmpty techniqueMethods then
                False

            else if List.isEmpty unknownMethods then
                False
