...
nextGroupIds model =
    let
        allGroupIds =
            treeIds model.groupsTree |> List.map .value

        knownGroupIds =
            model.groupsCompliance |> Dict.keys

        groupById =
            model.groupsTree |> getAllElems |> List.map (\g -> ( g.id.value, g )) |> Dict.fromList

        getGroupNameById id =
            case Dict.get id groupById of
                Just g ->
                    g.name
        ...
        diff =
            Set.diff (Set.fromList allGroupIds) (Set.fromList knownGroupIds) |> Set.toList
