...
buildComplianceBar filters complianceDetails =
    let
        filteredCompliance =
            filterCompliance complianceDetails filters
        ...
        displayCompliance compliance className =
            if compliance.value > 0 then
                let
                    ...
                    realPercent =
                        compliance.value / sumPercent filteredCompliance * 100

                    realRounded =
                        Basics.floor realPercent

                    complianceTxt =
                        if realRounded < 3 then
                            ""

                        else
                            String.fromInt realRounded ++ "%"
                in
        ...
        allComplianceValues =
            getAllComplianceValues filteredCompliance
    in
