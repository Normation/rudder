...
import Html.Attributes exposing (attribute, checked, class, disabled, for, href, id, placeholder, style, tabindex, type_, value)
...
view model =
    let
        ...
        allMissingCategoriesId =
            List.map .id (getAllMissingCats model.rulesTree)
        ...
        ruleTreeElem item =
            let
                ...
                classFocus =
                    case model.mode of
                        RuleForm rDetails ->
                            if rDetails.rule.id.value == item.id.value then
                                " focused "
        ...
        ruleTreeCategory item =
            let
                ...
                childsList =
                    ul [ class "jstree-children" ] (List.concat [ categories, rules, missingCat ])
                ...
                classFocus =
                    case model.mode of
                        CategoryForm cDetails ->
                            if cDetails.category.id == item.id then
                                " focused "
                ...
                missingCatClass =
                    if List.member item.id allMissingCategoriesId then
                        " missing-cat "
        ...
        treeFilters =
            ruleFilters.treeFilters

        newTag =
            ruleFilters.treeFilters.newTag
