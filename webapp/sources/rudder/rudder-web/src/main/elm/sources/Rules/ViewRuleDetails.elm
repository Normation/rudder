...
import Rules.ViewUtils exposing (btnSave, countRecentChanges, getGroupsNbResourcesBadge, getNbResourcesBadge, getRuleNbGroups, getRuleNbNodes)
import String
...
editionTemplate model details =
    let
        ...
        ( ruleTitle, isNewRule ) =
            case originRule of
                Nothing ->
                    ( span [ style "opacity" "0.4" ] [ text "New rule" ], True )
        ...
        ( classDisabled, badgeDisabled ) =
            if Maybe.withDefault True (Maybe.map .enabled originRule) then
                ( "", text "" )
        ...
        ( topButtons, badgePolicy ) =
            case originRule of
                Just or ->
                    let
                        ( txtDisabled, iconDisabled, editMsg ) =
                            if rule.enabled then
                                ( "Disable", "fa fa-ban", OpenDisablePopup )

                            else
                                ( "Enable", "fa fa-check-circle", OpenEnablePopup )
                    in
        ...
        getDiffList listA listB =
            ...
            if selectedLength == originLength then
                let
                    diff =
                        List.length (List.Extra.findIndices (isNotMember listB) listA)
                in
                ( diff, diff )
                ...
                let
                    diff =
                        List.length (List.Extra.findIndices (isNotMember listA) listB)

                    lengthDiff =
                        originLength - selectedLength
                in
                ( diff, lengthDiff + diff )

        ( diffDirectivesPos, diffDirectivesNeg ) =
            getDiffList (Maybe.Extra.unwrap [] .directives originRule) rule.directives
        ...
        getDiffGroups origineExcludedGroup origineIncludedGroup targetExcludedGroup targetIncludedGroup =
            let
                ( diffPos, diffNeg ) =
                    getDiffList (origineExcludedGroup ++ origineIncludedGroup) (targetExcludedGroup ++ targetIncludedGroup)
                ...
                totalSwap =
                    fromIncldToExcld + fromExldToIncld
            in
            ( diffPos + totalSwap, diffNeg + totalSwap )

        ( originRuleGroupsExcld, originRuleGroupsIncld ) =
            ( getTargetExcludedIds (Maybe.Extra.unwrap [] .targets originRule), getTargetIncludedIds (Maybe.Extra.unwrap [] .targets originRule) )

        ( diffGroupsPos, diffGroupsNeg ) =
            getDiffGroups originRuleGroupsExcld originRuleGroupsIncld (getTargetExcludedIds rule.targets) (getTargetIncludedIds rule.targets)

        nbDirectives =
            case originRule of
                Just oR ->
                    Maybe.withDefault 0 details.numberOfDirectives
            ...
            [ ul [ class "nav nav-underline" ]
                [ li [ class "nav-item" ]
                    [ button
                        ...
                        , onClick (UpdateRuleForm { details | tab = Information })
                        ]
                        [ text "Information" ]
                    ]
                , li [ class "nav-item" ]
                    [ button
                        ...
                        , class
                            ("nav-link"
                                ++ (if details.tab == Directives then
                                        " active"
                        ...
                        , onClick (UpdateRuleForm { details | tab = Directives })
                        ]
                        [ text "Directives"
                        , span [ class "badge badge-secondary badge-resources" ]
                            [ getNbResourcesBadge nbDirectives "This rule does not apply any directive"
                            , if diffDirectivesPos /= 0 then
                                span [ class "nb-resources new" ] [ text (String.fromInt diffDirectivesPos) ]

                              else
                                text ""
                            , if diffDirectivesNeg /= 0 then
                                span [ class "nb-resources del" ] [ text (String.fromInt diffDirectivesNeg) ]
                ...
                , li [ class "nav-item" ]
                    [ button
                        ...
                        , onClick (UpdateRuleForm { details | tab = Groups })
                        ]
                        [ text "Groups"
                        , span [ class "badge badge-secondary badge-resources" ]
                            ...
                            , if diffGroupsPos /= 0 then
                                span [ class "nb-resources new" ] [ text (String.fromInt diffGroupsPos) ]

                              else
                                text ""
                            , if diffGroupsNeg /= 0 then
                                span [ class "nb-resources del" ] [ text (String.fromInt diffGroupsNeg) ]
                ...
                , if isNewRule then
                    ...
                    li [ class "nav-item" ]
                        [ button
                            ...
                            , class
                                ("nav-link"
                                    ++ (if details.tab == TechnicalLogs then
                                            " active"
                            ...
                            , onClick (UpdateRuleForm { details | tab = TechnicalLogs })
                            ]
                            [ text "Recent changes"
                            , span [ class "badge badge-secondary badge-resources" ]
