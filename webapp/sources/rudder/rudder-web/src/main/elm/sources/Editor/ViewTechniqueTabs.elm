    ...
    let
        param_var_name =
            if String.isEmpty param.name then
                canonifyString (Maybe.withDefault "" param.description)
        ...
        descriptionId =
            "description-" ++ param.id.value

        selectTypeValue =
            if param.constraints.select == Nothing then
                "Text"
        ...
        , div [ class "param" ]
            ...
            , div [ class " form-group" ]
                ...
                , div [ class "input-group" ]
                    [ input
                        ...
                        , type_ "text"
                        , class ("form-control " ++ invalidParamClass)
                        , value param.name
                        , placeholder
                            (if Maybe.Extra.isNothing param.description then
                                "Variable name"

                             else
                                canonifyString (Maybe.withDefault "" param.description)
                            )
                        , onInput (\s -> TechniqueParameterModified param.id { param | name = s })
                        , required True
                        ]
                        []
            ...
            , div [ class "form-group" ]
                ...
                , input
                    ...
                    , readonly (not model.hasWriteRights)
                    , type_ "text"
                    , class ("col-xs-8 form-control " ++ invalidNameClass)
                    , value (Maybe.withDefault "" param.description)
                    , placeholder "Parameter name"
                    , onInput
                        (\s ->
                            TechniqueParameterModified param.id
                                { param
                                    | description =
            ...
            , div [ class " form-group" ]
                ...
                , div [ class "input-group" ]
                    [ textarea
                        ...
                        , class "form-control"
                        , rows 1
                        , value (Maybe.withDefault "" param.documentation)
                        , placeholder "description"
                        , onInput (\s -> TechniqueParameterModified param.id { param | documentation = Just s })
                        , required True
                        ]
                        []
                    ]
                ...
                , div [ class "input-group" ]
                    ...
                    , label [ class "input-group-text", for checkboxId ]
                        [ input
                            [ type_ "checkbox"
                            , id checkboxId
                            , checked (not param.mayBeEmpty)
                            , onCheck
                                (\c ->
                                    let
                                        newConstraint =
                                            { constraint | allowEmpty = Just c }
                                    in
                                ...
                                li []
                                    ...
                                    , div [ class "param" ]
                                        ...
                                        , div [ class "form-group" ]
                                            ...
                                            , input
                                                [ readonly (not model.hasWriteRights)
                                                , type_ "text"
                                                , class ("col-xs-8 form-control " ++ invalidEnumNameClass)
                                                , placeholder "Name"
                                                , value (Maybe.withDefault "" currentValue.name)
                                                , placeholder "Name"
                                                , onInput
                                                    (\s ->
                                                        let
                                        ...
                                        , div [ class "form-group" ]
                                            ...
                                            , div [ class "form-group" ]
                                                [ input
                                                    [ readonly (not model.hasWriteRights)
                                                    , type_ "text"
                                                    , class ("col-xs-8 form-control " ++ invalidEnumValueClass)
                                                    ...
                                                    , onInput
                                                        (\s ->
                                                            let
                                                                newConstraint =
                                                                    { constraint | select = Just (List.Extra.updateAt index (\val -> { val | value = s }) l) }
                                                            in
                                                            TechniqueParameterModified param.id { param | constraints = newConstraint }
                                                        )
                                                    , required True
                                    ...
                                    , div
                                        ...
                                        , onClick
                                            (let
                                                newConstraint =
                                                    { constraint | select = Just (List.Extra.removeAt index l) }
                                             in
                                             TechniqueParameterModified param.id { param | constraints = newConstraint }
                                            )
                                        ]
                        ...
                        (List.concat
                            ...
                            , [ button
                                    ...
                                    , onClick
                                        (let
                                            newConstraint =
                                                { constraint | select = Just (List.concat [ l, [ SelectOption "" Nothing ] ]) }
                                         in
                                         TechniqueParameterModified param.id { param | constraints = newConstraint }
                                        )
                                    , required True
            ...
            let
                ...
                currentOption =
                    if c.name == "/" then
                        ...
                        [ option [ selected (c.path == category), value c.path ] [ text (sep ++ c.name) ] ]
                ...
                listCategories =
                    List.concatMap (buildListCategoriesWithoutRoot separator category) (getSubElems c)
            in
...
techniqueTab model technique creation ui =
    let
        searchForCategory =
            List.head (List.filter (\c -> c.path == technique.category) (allCategorieswithoutRoot model))
        ...
        classErrorInputId =
            case ui.idState of
                ...
                _ ->
                    if String.length technique.id.value > 100 then
                        " error "
        ...
        classErrorName =
            case ui.nameState of
                InvalidState errors ->
                    if List.member AlreadyTakenName errors || List.member EmptyName errors then
                        " error "
                ...
                [ div [ class "row form-group", style "margin-top" "15px" ]
                    ...
                    , div [ class "col-md-8" ]
                        [ input
                            ...
                            , id "techniqueName"
                            , name "name"
                            , class ("form-control" ++ classErrorName)
                                    ...
                                    UpdateTechnique
                                        { technique
                                            ...
                                            , id =
                                                TechniqueId
                                                    (if creation then
                                                        canonifyHelper (Value (String.toLower newName))
                            ...
                            ul []
                                (List.map
                                    (\err ->
                                        case err of
                                            AlreadyTakenName ->
                                                li [ class "text-danger col-md-8" ] [ text "Technique name must be unique" ]

                                            EmptyName ->
                                                li [ class "text-danger col-md-8" ] [ text "Technique name is required" ]
                                    )
                                    errors
                                )
                ...
                , div [ class "row form-group" ]
                    ...
                    , div [ class "col-md-8" ]
                        [ textarea
                            [ readonly (not model.hasWriteRights)
                            , name "documentation"
                            , class "form-control technique-description"
                            , id "techniqueDocumentation"
                            , rows 4
                            , value technique.documentation
                            ...
                            ul []
                                (List.map
                                    (\err ->
                                        case err of
                                            TooLongId ->
                                                li [ class "text-danger col-md-8" ] [ text "Technique IDs longer than 255 characters won't work on most filesystems." ]

                                            AlreadyTakenId ->
                                                li [ class "text-danger col-md-8" ] [ text "Technique ID must be unique." ]
                                    )
                                    errors
                                )
        ...
        Resources ->
            ...
            div [ class "tab tab-resources" ]
                ...
                , if List.any (\s -> (s.state == New) || (s.state == Deleted)) technique.resources then
                    let
                        number =
                            List.length (List.filter (\s -> (s.state == New) || (s.state == Deleted)) technique.resources)
                    in
                    div [ class "resources-uncommitted" ]
                        [ -- ng-if="getResourcesByState(selectedTechnique.resources, 'new').length>0 || getResourcesByState(selectedTechnique.resources, 'deleted').length > 0">
                          span []
                            ...
                            , text
                                (" unsaved "
                                    ...
                                    ++ ", save your changes to complete upload."
                                )
                            ]
                        ]
