...
import Html exposing (Html, div, i, li, span, table, tbody, td, th, thead, tr, ul)
...
getCategoryName model id =
    let
        cat =
            List.Extra.find (.id >> (==) id) (getAllCats model.groupsTree)
    in
    case cat of
        Just c ->
            c.name
...
searchFieldGroups g model =
    ...
    , g.name
        ...
        ++ Maybe.Extra.toList (Maybe.map (getCategoryName model) g.category)
...
getSortFunction model g1 g2 =
    let
        ...
        groupGlobalCompliance g =
            getCompliance <| Maybe.map .global (getGroupCompliance model g.id)

        groupTargetedCompliance g =
            getCompliance <| Maybe.map .targeted (getGroupCompliance model g.id)

        order =
            case model.ui.groupFilters.tableFilters.sortBy of
                Parent ->
                    let
                        categoryOrEmpty g =
                            Maybe.withDefault "" (Maybe.map (getCategoryName model) g.category)

                        o =
                            NaturalOrdering.compare (categoryOrEmpty g1) (categoryOrEmpty g2)
                    in
                    case o of
                        EQ ->
                            NaturalOrdering.compare g1.name g2.name
