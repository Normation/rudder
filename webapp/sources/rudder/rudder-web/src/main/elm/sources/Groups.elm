    ...
    case msg of
        ...
        GetGroupsTreeResult res chainInitTable ->
            case res of
                Ok r ->
                    ...
                    , Cmd.batch
                        (initTooltips ()
                            :: (if chainInitTable then
                                    [ getInitialGroupCompliance newModel ]

                                else
                                    []
                               )
                        )
                      -- reload the table each time we have a new groups tree
                    )
            ...
            case res of
                Ok r ->
                    let
                        ...
                        currentGroups =
                            if keepGroups then
                                model.groupsCompliance

                            else
                                Dict.empty

                        groupsCompliance =
                            Dict.Extra.fromListBy (.id >> .value) r |> Dict.union currentGroups
                    in
                    ( { model
        ...
        UpdateGroupFoldedFilters categoryId ->
            let
                ...
                filters =
                    foldUnfoldCategory model.ui.groupFilters categoryId

                newUi =
                    { ui | groupFilters = filters }
            in
        ...
        UpdateGroupSearchFilters search ->
            let
                ...
                newGroupFilters =
                    { groupFilters | filter = search }

                newUi =
                    { ui | groupFilters = newGroupFilters }
            in
...
handleOutMsg model groupsTable tabMsg outMsgOpt =
    case outMsgOpt of
        Just (OnHtml parentMsg) ->
            let
                ( newModel, newMsg ) =
                    update parentMsg { model | groupsTable = groupsTable }
            in
...
updateGroupsTableFilter model =
    let
        ...
        search =
            model.ui.groupFilters.filter

        predicate =
            Filters.applyString search (Filters.byValues searchFieldGroups)
    in
