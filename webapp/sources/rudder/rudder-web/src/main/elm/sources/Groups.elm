  ...
  case msg of
    ...
    UpdateGroupFilters filters ->
      let
        ...
        filter s =
            String.contains
                (filters.treeFilters.filter |> String.toLower |> String.trim)
                (s |> String.toLower)
        groupsTable = Rudder.Table.updateDataWithFilter (Groups.Init.filterPredicate filter) model.groupsTable
      in
