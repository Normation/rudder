...
viewResult result =
    let
        name =
            kindName result.header.type_ |> String.Extra.toSentenceCase
    in
                ...
                [ h4 [ class "panel-title" ]
                    [ a [ role "button", attribute "data-bs-toggle" "collapse", href ("#result" ++ name), attribute "aria-expanded" "true" ]
                        ...
                        , span [ class "description" ]
                            ...
                            , text
                                (if result.header.numbers > 10 then
                                    ", only displaying the first 10. Please refine your query."
...
filterButton model filter =
    let
        ...
        numbers =
            model.results
                |> (case filter of
                        All ->
                            List.map (.header >> .numbers) >> List.sum

                        FilterKind k ->
                            List.Extra.find (.header >> .type_ >> (==) k) >> Maybe.map (.header >> .numbers) >> Maybe.withDefault 0
                   )
    ...
    button
        ...
        , span [ class "badge pull-right" ]
            [ case model.state of
                Searching ->
                    span [ class "loading fa fa-sync-alt" ] []
