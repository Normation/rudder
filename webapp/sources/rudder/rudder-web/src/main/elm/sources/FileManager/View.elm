            ...
            [ div [ class "fm-simple-screen", onContextMenu <| None, onDragOver None ]
                [ div
                    ...
                    , if model.showBound then
                        ...
                        div [] []
                    , if model.drag then
                        renderCount model.pos2 model.selected
                        ...
                        (fs.childs
                            |> List.map
                                (\f ->
                                    let
                                        path =
                                            List.append fs.parents [ fs.name, f ] |> getDirPath
                                    in
                                    li []
                                        ...
                                        , if Dict.member path openedDir then
                                            listFolders path
...
currentDir dirList =
    case List.Extra.last dirList of
        Just f ->
            f
...
renderFile { api, thumbnailsUrl, dir, selected, clipboardDir, clipboardFiles } i file =
    div
        [ id <| "fm-file-" ++ fromInt i
        , class <|
            "fm-file"
                ++ (if member file selected then
                        " fm-selected"

                    else
                        ""
                   )
                ++ (if getDirPath dir == clipboardDir && member file clipboardFiles then
                        " fm-cut"
        ...
        , onDoubleClick <|
            if file.type_ == "dir" then
                EnvMsg <| GetLs (getDirPath (List.append dir [ file.name ]))
...
renderThumb thumbApi api dir file =
    if file.type_ == "dir" then
        div [ class "fm-thumb" ] [ folderIcon 48 ]

    else
        renderFileThumb api thumbApi <| dir ++ file.name
...
getExt name =
    withDefault "" <| head <| reverse <| split "." name
        ...
        Edit file content ->
            div [ class "fm-screen" ]
                [ div [ class "fm-modal" ]
                    ...
                    , div []
                        [ button [ class "btn btn-default", onClick CloseNameDialog ] [ text "Cancel" ]
                        , button [ class "btn btn-primary ms-2", onClick ConfirmNameDialog ] [ text "Confirm" ]
                        ]
                    ]
                ]
...
getSortFunction model f1 f2 =
    let
        order =
            case model.tableFilters.sortBy of
                FileName ->
                    N.compare f1.name f2.name

                FileSize ->
                    Basics.compare f1.size f2.size

                FileDate ->
                    N.compare f1.date f2.date

                FileRights ->
                    N.compare f1.rights f2.rights
    in
