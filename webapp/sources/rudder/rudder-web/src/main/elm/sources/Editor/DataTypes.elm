...
type alias Draft =
    { technique : Technique, origin : Maybe Technique, id : DraftId, date : Posix }


type AgentValue
    = Value String
    | Variable (List AgentValue)
...
defaultConstraint =
    Constraint Nothing Nothing Nothing Nothing Nothing Nothing Nothing
...
type MethodElem
    = Call (Maybe CallId) MethodCall
    | Block (Maybe CallId) MethodBlock


type WorstReportKind
...
allCategories t =
    let
        subElems =
            case t.subCategories of
                SubCategories l ->
                    List.concatMap allCategories l
    in
    t :: subElems
...
allCategorieswithoutRoot m =
    let
        subElems =
            case m.categories.subCategories of
                SubCategories l ->
                    List.concatMap allCategories l
    in
    subElems
...
type TechniqueState
    = Creation TechniqueId
    | Edit Technique
    | Clone Technique (Maybe TechniqueId) TechniqueId
...
type DropElement
    = StartList
    | AfterElem (Maybe CallId) MethodElem
    | InBlock MethodBlock
...
type TechniqueIdError
    = TooLongId
    | AlreadyTakenId


type MethodCallParamError
    = ConstraintError { id : ParameterId, message : String }


type MethodCallConditionError
    = ReturnCarrigeForbidden
...
type TechniqueErrorUiInfo
    = TechniqueErrorUiYamlEditing String
    | TechniqueErrorUiSaving String
...
type UiInfo
    = CallUiInfo MethodCallUiInfo MethodCall
    | BlockUiInfo MethodBlockUiInfo MethodBlock
...
techniqueCheckState { id, name } =
    { id = id, name = name }


draftCheckState : Draft -> TechniqueCheckState
draftCheckState { id, technique } =
    { id = id, name = technique.name }
