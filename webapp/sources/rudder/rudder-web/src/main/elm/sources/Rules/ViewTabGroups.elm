...
import Ui.Datatable exposing (Category, SortOrder(..), filterSearch, getSubElems)
        ...
        let
            ...
            groupTreeElem item =
                let
                                ...
                                List.concatMap
                                    (\target ->
                                        case target of
                                            ...
                                            And t ->
                                                getTargetIds t
                                     -- TODO : Need to be improved - we need to keep targets that are member of all targets list in t
                                    )
                                    listTargets
                        in
                    ...
                    includeClass =
                        if checkIncludeOrExclude includedTargets then
                            " item-selected"

                        else if checkIncludeOrExclude excludedTargets then
                            " item-selected excluded"
                    ...
                    , a [ class ("jstree-anchor" ++ includeClass), onClickPreventDefault (SelectGroup item.target True) ]
                        ...
                        , span [ class "item-name" ]
                            ...
                            , if item.dynamic then
                                small [ class "text-secondary" ] [ text "- Dynamic" ]
