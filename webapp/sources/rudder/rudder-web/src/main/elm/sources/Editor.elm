...
updateResourcesResponse model =
    case model.mode of
        TechniqueDetails _ s _ editInfo ->
            CallApi (getRessources s)
...
parseDraftsResponse json =
    case Json.Decode.decodeValue (Json.Decode.dict decodeDraft) json of
        Ok drafts ->
            let
                invalidDrafts =
                    Dict.keys (Dict.filter (\_ v -> Maybe.Extra.isNothing v) drafts)

                validDrafts =
                    Dict.Extra.filterMap (\_ v -> v) drafts
            in
...
mainInit initValues =
    let
        ...
        model =
            ...
            , recClone = []
            , policyMode = "default"
            , historyTable = initTable (ContextPath initValues.contextPath) zone
            }
    in
    ( model
    , Cmd.batch
        ...
        , getDirectives model
        , getPolicyMode model
        , Cmd.map HistoryMessage (getEventLogs Nothing (ContextPath initValues.contextPath) (Just "editorTechniques"))
        ]
    )
...
updatedStoreTechnique model =
    case model.mode of
                    ...
                    case o of
                        Edit origin ->
                            if t == origin then
                                ...
                                let
                                    draft =
                                        Draft t (Just origin) origin.id (Time.millisToPosix 0)
                                in
                        ...
                        Creation id ->
                            let
                                draft =
                                    Draft t Nothing id (Time.millisToPosix 0)
                            in
                            ( Dict.insert draft.id.value draft model.drafts, Cmd.batch [ initInputs "", clearTooltips "", storeDraft (encodeDraft draft) ] )
        ...
        _ ->
            ( model, Cmd.batch [ initInputs "", clearTooltips "" ] )
...
defaultTechniqueUi technique =
    let
        callState =
            Dict.fromList (List.map (\c -> ( c.id.value, defaultMethodUiInfo (Just c) )) (List.concatMap getAllCalls technique.elems))

        blockState =
            Dict.fromList (List.map (\c -> ( c.id.value, defaultBlockUiInfo c )) (List.concatMap getAllBlocks technique.elems))
    in
...
selectTechnique model technique =
    let
        ( effectiveTechnique, state, action ) =
            case technique of
                Left t ->
                    let
                        tech =
                            case Dict.get t.id.value model.drafts of
                                Just d ->
                                    d.technique
        ...
        validateDraftUi s =
            checkTechniqueUiState state s (List.map techniqueCheckState model.techniques) defaultUi

        ui =
            Either.unpack (\_ -> defaultUi) (draftCheckState >> validateDraftUi) technique
...
updateParameter paramId newValue x =
    case x of
        Call p c ->
            Call p { c | parameters = List.Extra.updateIf (.id >> (==) paramId) (\param -> { param | value = getAgentValue newValue }) c.parameters }

        Block p b ->
            Block p { b | calls = List.map (updateParameter paramId newValue) b.calls }
...
update msg model =
    case msg of
            ...
            case model.mode of
                TechniqueDetails t _ _ editInfo ->
                    if t.id == id then
                        ...
                        ( newModel, Cmd.map HistoryMessage (getEventLogs search (ContextPath model.contextPath) (Just "editorTechniques")) )
        ...
        NewTechnique internalId ->
            let
                ui =
                    TechniqueUiInfo General Dict.empty Dict.empty [] False Unchanged Unchanged Nothing

                t =
                    Technique (TechniqueId "") "1.0" "" "" "" "ncf_techniques" [] [] [] [] Nothing

                editInfo =
                    TechniqueEditInfo "" False

                newModel =
                    { model | mode = TechniqueDetails t (Creation internalId) ui editInfo }
            in
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails technique o ui editInfo ->
                            TechniqueDetails technique o { ui | tab = tab } editInfo
        ...
        CloneTechnique technique optDraftId internalId ->
            let
                ...
                newId =
                    canonifyHelper (Value (String.toLower copiedName))

                callState =
                    Dict.fromList (List.map (\c -> ( c.id.value, defaultMethodUiInfo (Just c) )) (List.concatMap allMethodCalls technique.elems))
        ...
        SaveTechnique (Ok ( metadata, technique )) ->
            let
                ...
                drafts =
                    Dict.remove idToClean.value model.drafts
            in
                ...
                newModel =
                    case model.mode of
                        TechniqueDetails t s ui editInfo ->
                            { model | mode = TechniqueDetails t s { ui | saving = False } editInfo }
        ...
        DeleteTechnique (Ok techniqueId) ->
            case model.mode of
                TechniqueDetails t (Edit _) _ _ ->
                    let
                        techniques =
                            List.filter (.id >> (/=) techniqueId) model.techniques

                        newMode =
                            if t.id == techniqueId then
                                Introduction
                ...
                TechniqueDetails t (Creation id) _ _ ->
                    let
                        drafts =
                            Dict.remove techniqueId.value model.drafts

                        newMode =
                            if id == techniqueId then
                                Introduction
        ...
        ClosePopup callback ->
            let
                ( nm, cmd ) =
                    update callback { model | modal = Nothing }
            in
            ( nm, Cmd.batch [ cmd ] )
                        ...
                        TechniqueDetails base s ui editInfo ->
                            let
                                ( technique, drafts ) =
                                    case s of
                                        Edit t ->
                                            ( t, Dict.remove t.id.value model.drafts )
                            ...
                            let
                                ...
                                ( updatedTechnique, needCheck ) =
                                    case findElemIf (getId >> (==) (getId call)) origin.elems of
                                        Just resetCall ->
                                            ( { technique | elems = updateElemIf (getId >> (==) (getId call)) (always resetCall) technique.elems }, Just resetCall )
                                ...
                                newUi =
                                    case needCheck of
                                        Just (Call _ realCall) ->
                                            let
                                                constraints =
                                                    case Dict.get realCall.methodName.value model.methods of
                                                        Just m ->
                                                            Dict.fromList (List.map (\p -> ( p.name.value, p.constraints )) m.parameters)
                                                    ...
                                                    \optCui ->
                                                        let
                                                            b =
                                                                case optCui of
                                                                    Nothing ->
                                                                        defaultMethodUiInfo (Just realCall)
                                            ...
                                            let
                                                updateCallUi =
                                                    \optCui ->
                                                        case optCui of
                                                            Nothing ->
                                                                Just (defaultBlockUiInfo b)

                                                            Just cui ->
                                                                Just { cui | validation = Unchanged }
                                            in
                ...
                newModel =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                parameters =
                                    List.Extra.updateIf (\c -> paramId == c.id) (always newValue) t.parameters
                ...
                newModel =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                parameters =
                                    List.Extra.filterNot (\c -> paramId == c.id) t.parameters
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                parameters =
                                    List.append t.parameters [ TechniqueParameter paramId "" Nothing Nothing False defaultConstraint ]
                            in
                        ...
                        TechniqueDetails t s _ editInfo ->
                            let
                                url =
                                    case s of
                                        Edit _ ->
                                            t.id.value ++ "/" ++ t.version ++ "/" ++ t.category

                                        Creation internalId ->
                                            "draft/" ++ internalId.value ++ "/" ++ t.version
        ...
        ToggleDoc methodId ->
            let
                ...
                newDocs =
                    if List.member methodId ui.docsOpen then
                        List.Extra.remove methodId ui.docsOpen

                    else
                        methodId :: ui.docsOpen
            in
        ...
        AddMethod method newId ->
            if model.hasWriteRights then
                let
                    ...
                    newCall =
                        MethodCall newId method.id (List.map (\p -> CallParameter p.name [ Value "" ]) method.parameters) (Condition Nothing "") "" disableReporting Nothing Nothing Nothing
                    ...
                    newModel =
                        case model.mode of
                            TechniqueDetails t o ui editInfo ->
                                let
                                    technique =
                                        { t | elems = t.elems ++ [ Call Nothing newCall ] }

                                    defaultUiMethod =
                                        defaultMethodUiInfo (Just newCall)

                                    newUi =
                                        { ui | callsUI = Dict.update newId.value (always (Just { defaultUiMethod | mode = Opened })) ui.callsUI }
                                in
        ...
        AddBlock newId ->
            if model.hasWriteRights then
                let
                    newCall =
                        MethodBlock newId "" (Condition Nothing "") WeightedReport [] Nothing Nothing Nothing
                    ...
                    newModel =
                        case model.mode of
                            TechniqueDetails t o ui editInfo ->
                                let
                                    technique =
                                        { t | elems = t.elems ++ [ Block Nothing newCall ] }

                                    newUi =
                                        { ui | blockUI = Dict.update newId.value (always (Just (MethodBlockUiInfo Opened Children (InvalidState [ EmptyComponent ]) False (ForeachUI False False (defaultNewForeach newCall.foreachName newCall.foreach))))) ui.blockUI }
                                in
                                { model | mode = TechniqueDetails technique o newUi editInfo }
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                technique =
                                    { t | elems = updateElemIf (getId >> .value >> (==) "") (setId newId) t.elems }

                                newUi =
                                    { ui | callsUI = Dict.insert newId.value (defaultMethodUiInfo Nothing) ui.callsUI }
                            in
        ...
        UpdateCallAndUi uiInfo ->
            let
                ...
                ( newModel, newMsg ) =
                    update uiAction model

                ( finalModel, finalMsg ) =
                    update callAction newModel
            in
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                newUi =
                                    { ui | callsUI = Dict.update callId.value (Maybe.map (always newMethodUi)) ui.callsUI }
                            in
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                newUi =
                                    { ui | blockUI = Dict.update callId.value (always (Just newBlockUi)) ui.blockUI }
                            in
                            TechniqueDetails t o newUi editInfo
                ...
                newMode =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                technique =
                                    { t | elems = removeElem (getId >> (==) callId) t.elems }
                            in
                            TechniqueDetails technique o ui editInfo
                        ...
                        TechniqueDetails t o ui editInfo ->
                            let
                                newMethods =
                                    case getParent clone of
                                        Nothing ->
                                            let
                                                ( heads, tails ) =
                                                    Maybe.withDefault ( t.elems, [] ) (List.Extra.splitWhen (\e -> getId e == getId call) t.elems)
                                            in
                                            List.append heads (clone :: tails)

                                        Just pid ->
                                            updateElemIf (getId >> (==) pid)
                                                (\child ->
                                                    case child of
                                                        Call _ _ ->
                                                            child
                                                        ...
                                                        Block p b ->
                                                            let
                                                                ( heads, tails ) =
                                                                    Maybe.withDefault ( b.calls, [] ) (List.Extra.splitWhen (\e -> getId e == getId call) b.calls)
                                                            in
                                ...
                                newUi =
                                    case call of
                                        Call _ c ->
                                            { ui | callsUI = Dict.update newId.value (always (Just (defaultMethodUiInfo (Just c)))) ui.callsUI }

                                        Block _ b ->
                                            { ui | blockUI = Dict.update newId.value (always (Just (defaultBlockUiInfo b))) ui.blockUI }
                                                ...
                                                (\child ->
                                                    let
                                                        elem =
                                                            case child of
                                                                Call _ c ->
                                                                    Call (Just (getId clone)) c

                                                                Block _ c ->
                                                                    Block (Just (getId clone)) c
                                                    in
            ...
            case model.mode of
                TechniqueDetails t s ui editInfo ->
                    let
                            ...
                            case ( method, uiInfo ) of
                                ( Block id block, Just (BlockUiInfo methodBlockUiInfo _) ) ->
                                    let
                                        ...
                                        updateBlockState =
                                            \originUiBlock ->
                                                case originUiBlock of
                                                    Just _ ->
                                                        Just { methodBlockUiInfo | validation = blockState }

                                                    Nothing ->
                                                        Just (MethodBlockUiInfo Closed Children blockState False (ForeachUI False False (defaultNewForeach block.foreachName block.foreach)))
                                    in
                                    ...
                                    let
                                        ...
                                        updateBlockState =
                                            \originUiBlock ->
                                                case originUiBlock of
                                                    Just uiBlock ->
                                                        Just { uiBlock | validation = blockState }
                        ...
                        newModel =
                            { model | mode = TechniqueDetails { t | elems = updateElemIf (getId >> (==) (getId method)) (always method) t.elems } s newUi editInfo }
                    in
                    updatedStoreTechnique newModel
                ...
                newModel =
                    case model.mode of
                        TechniqueDetails t o ui editInfo ->
                            let
                                calls =
                                    updateElemIf (getId >> (==) call.id) (updateParameter paramId newValue) t.elems

                                constraints =
                                    case Dict.get call.methodName.value model.methods of
                                        Just m ->
                                            Maybe.withDefault defaultConstraint (Maybe.map .constraints (List.Extra.find (.name >> (==) paramId) m.parameters))
                                    ...
                                    \optCui ->
                                        let
                                            base =
                                                case optCui of
                                                    Nothing ->
                                                        defaultMethodUiInfo (Just call)

                                                    Just cui ->
                                                        cui

                                            filterValidation =
                                                case base.validation of
                                                    InvalidState err ->
                                                        let
                                                            newErrors =
                                                                List.filter (\(ConstraintError v) -> v.id /= paramId) err
                                                        in
                                                        if List.isEmpty newErrors then
                                                            ValidState

                                                        else
                                                            InvalidState newErrors

                                                    _ ->
                                                        base.validation

                                            newValidation =
                                                accumulateErrorConstraint (CallParameter paramId (getAgentValue newValue)) constraints filterValidation
                                        in
        ...
        SetMissingIds newId ->
            case model.mode of
                TechniqueDetails t e u editInfo ->
                    let
                        newUi =
                            { u | callsUI = Dict.update newId (always (Just (defaultMethodUiInfo Nothing))) u.callsUI }
                    in
                    case setIdRec newId t.elems of
                        ...
                        ( newCalls, True ) ->
                            update (GenerateId SetMissingIds) { model | mode = TechniqueDetails { t | elems = newCalls } e newUi editInfo }
                    ...
                    let
                        ( baseCalls, newElem ) =
                            case draggedItemId of
                                Move b ->
                                    ( removeElem (getId >> (==) (getId b)) t.elems, b )

                                NewBlock ->
                                    ( t.elems, Block Nothing (MethodBlock (CallId "") "" (Condition Nothing "") WeightedReport [] Nothing Nothing Nothing) )
                        ...
                        updatedCalls =
                            case dropTarget of
                                                ...
                                                (\x ->
                                                    case x of
                                                        Block p k ->
                                                            let
                                                                calls =
                                                                    case List.Extra.splitWhen (getId >> (==) (getId call)) k.calls of
                                                                        Nothing ->
                                                                            newElem :: k.calls
                                ...
                                InBlock b ->
                                    updateElemIf (getId >> (==) b.id)
                                        (\x ->
                                            case x of
                                                Block p k ->
                                                    Block p { k | calls = newElem :: k.calls }
                        ...
                        updateTechnique =
                            { t | elems = updatedCalls }

                        newModel =
                            { model | mode = TechniqueDetails updateTechnique u e editInfo, dnd = DragDrop.initialState }
                    in
