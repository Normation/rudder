...
getClassParameter method =
    case findClassParameter method of
        ...
        Nothing ->
            MethodParameter method.classParameter "" "" defaultConstraint
...
showParam model call state methodParam params =
    let
        paramLabel =
            String.Extra.toSentenceCase (String.replace "_" " " methodParam.name.value)
        ...
        parameterInput =
            ...
            case methodParam.constraints.select of
                ...
                Just list ->
                    let
                        datalistId =
                            "param-" ++ methodParam.name.value ++ "-options"
                    in
                    [ element "input"
...
accumulateValidationState validations base =
    List.foldl
        (\c acc ->
            case ( acc, c ) of
                ( InvalidState errAcc, InvalidState err ) ->
                    InvalidState (List.concat [ err, errAcc ])
...
checkConstraintOnParameter call constraint =
    let
        checkEmpty =
            if not (constraint.allowEmpty |> Maybe.withDefault False) && isEmptyValue call.value then
                [ ConstraintError { id = call.id, message = "Parameter '" ++ call.id.value ++ "' is empty" } ]
        ...
        checkWhiteSpace =
            if constraint.allowWhiteSpace |> Maybe.withDefault False then
                ...
                case Regex.fromString "(^\\s)|(\\s$)" of
                    ...
                    Just r ->
                        if Regex.contains r (displayValue call.value) then
                            [ ConstraintError { id = call.id, message = "Parameter '" ++ call.id.value ++ "' cannot start or end with whitespace characters" } ]

                        else
                            []

        checkMax =
            if lengthValue call.value >= (constraint.maxLength |> Maybe.withDefault 16384) then
                [ ConstraintError { id = call.id, message = "Parameter '" ++ call.id.value ++ "' cannot be more than " ++ String.fromInt (constraint.maxLength |> Maybe.withDefault 16384) ++ " characters long" } ]

            else
                []

        min =
            Maybe.withDefault 0 constraint.minLength

        checkMin =
            if lengthValue call.value < min then
                [ ConstraintError { id = call.id, message = "Parameter '" ++ call.id.value ++ "' must be at least " ++ String.fromInt min ++ " characters long" } ]

            else
                []

        regexValue =
            Maybe.map Regex.fromString constraint.matchRegex |> Maybe.Extra.join
        ...
        nonRegexValue =
            Maybe.map Regex.fromString constraint.notMatchRegex |> Maybe.Extra.join

        notRegexCheck =
            case nonRegexValue of
                Nothing ->
                    []
        ...
        checkVariableUsage =
            case Regex.fromString "^\\${.*}$" of
                ...
                Just regex ->
                    Regex.contains regex (displayValue call.value)

        checkSelect =
            Maybe.map
                (\select ->
                    if List.any (.value >> (==) (displayValue call.value)) select || checkVariableUsage then
                        []
        ...
        checks =
            [ checkEmpty, checkWhiteSpace, checkMax, checkMin, checkRegex, notRegexCheck, checkSelect ] |> List.concat
    in
    ...
    case uiInfo.tab of
        ...
        CallConditions ->
            let
                ...
                ubuntuLi =
                    List.map
                        (\ubuntuMinor ->
                            let
                                updatedCall =
                                    Call parentId { call | condition = { condition | os = updateUbuntuMinor ubuntuMinor condition.os } }
                            in
                            element "li"
                ...
                updateConditonVersion =
                    \f s ->
                        let
                            updatedCall =
                                Call parentId { call | condition = { condition | os = f (String.toInt s) condition.os } }
                        in
                                ...
                                |> addAttributeList
                                    ...
                                    , onInput
                                        (\s ->
                                            let
                                                updatedCondition =
                                                    { condition | advanced = s }

                                                updatedCall =
                                                    Call parentId { call | condition = updatedCondition }
                                            in
                                            MethodCallModified updatedCall Nothing
                                        )
        ...
        Result ->
            let
                ...
                paramValue =
                    call.parameters |> List.Extra.find (\c -> c.id == classParameter.name) |> Maybe.map .value |> Maybe.withDefault [ Value "" ]

                fullParamValue =
                    method.classPrefix ++ "_" ++ canonify paramValue

                conditions =
                    if String.startsWith "condition_from" method.classPrefix then
                        [ element "label"
...
showMethodCall model ui tui parentId call =
    let
        isHovered =
            case model.isMethodHovered of
                Just methodId ->
                    if methodId.value == call.id.value then
                        "hovered"
...
callBody model ui techniqueUi call pid =
    let
        method =
            case Dict.get call.methodName.value model.methods of
                ...
                Nothing ->
                    Method call.methodName call.methodName.value "" "" (Maybe.withDefault (ParameterId "") (Maybe.map .id (List.head call.parameters))) [] [] Nothing Nothing Nothing

        deprecatedClass =
            "fa fa-info-circle method-action text-info"
        ...
        isHovered =
            case model.isMethodHovered of
                Just methodId ->
                    (methodId.value == call.id.value) && ui.mode == Closed

                Nothing ->
                    False

        ( textClass, tooltipContent ) =
            case ui.validation of
                InvalidState [ _ ] ->
                    ( "text-danger", "A parameter of this method is invalid" )

                InvalidState err ->
                    ( "text-danger", String.fromInt (List.length err) ++ " parameters of this method are invalid" )

                Unchanged ->
                    ( "", "" )

                ValidState ->
                    ( "text-primary", "This method was modified" )
        ...
        cloneIcon =
            element "i" |> addClass "fa fa-clone"
        ...
        removeIcon =
            element "i" |> addClass "fa fa-times-circle"
        ...
        resetIcon =
            element "i" |> addClass "fa fa-rotate-right"
        ...
        shoudHoveredMethod =
            case model.isMethodHovered of
                Just methodId ->
                    if (methodId.value == call.id.value) && ui.mode == Closed then
                        " hovered"
                    ...
                    element "div"
                        ...
                        |> appendChild
                            (element "span"
                                ...
                                |> appendText
                                    (if String.isEmpty call.component then
                                        method.name
    ...
    element "div"
        ...
        |> (if techniqueUi.enableDragDrop == Just call.id then
            ...
            , element "div"
                ...
                |> appendChildList
                    [ element "div"
                        ...
                        -- (call.condition.os /= Nothing || call.condition.advanced /= "")
