module Rules.JsonEncoder exposing (..)
...
encodeRuleDetails ruleDetails =
    let
        listTags =
            object (List.map (\t -> ( t.key, string t.value )) ruleDetails.tags)
    in
    object
        [ ( "id", string ruleDetails.id.value )
        , ( "displayName", string ruleDetails.name )
        , ( "category", string ruleDetails.categoryId )
        , ( "shortDescription", string ruleDetails.description )
        , ( "longDescription", string ruleDetails.documentation )
