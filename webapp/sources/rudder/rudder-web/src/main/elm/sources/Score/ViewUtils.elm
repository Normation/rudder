...
getScoreBadge id score smallSize =
    span
        [ onMouseOver (ShowScoreMessage id)
        , onMouseLeave (ShowScoreMessage Nothing)
        , class
...
scoreBreakdownList scoreDetails scoreInfo =
    scoreDetails
        |> List.sortBy .scoreId
        |> List.filter (.value >> (/=) X)
        |> List.map
            (\sD ->
                let
                    name =
                        List.Extra.find (.id >> (==) sD.scoreId) scoreInfo |> Maybe.map .name |> Maybe.withDefault (String.Extra.humanize sD.scoreId)
                in
