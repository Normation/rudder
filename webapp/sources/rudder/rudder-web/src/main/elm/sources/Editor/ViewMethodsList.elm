    ...
    let
        description =
            case method.description of
                ...
                d ->
                    "<div class='description'>" ++ htmlEscape d ++ "</div>"
        ...
        deprecation =
            case method.deprecated of
                ...
                Just m ->
                    "<div class='deprecated-info'><div>This generic method is <b>deprecated</b>.</div> <div class='deprecated-message'><b>↳</b>" ++ htmlEscape m ++ "</div></div>"
    in
    ...
    case model.mode of
        ...
        _ ->
            let
                ...
                filterMethods =
                    List.filter (filterMethod methodsUI) (Dict.values model.methods)

                methodByCategories =
                    Dict.Extra.groupBy (\m -> Maybe.withDefault m.id.value (List.head (String.split "_" m.id.value)) |> String.Extra.toTitleCase) filterMethods
                ...
                [ div [ class "sidebar-header" ]
                    ...
                    , div [ class "header-filter" ]
                        [ div [ class "input-group" ]
                            ...
                            , button [ class "btn btn-outline-secondary btn-toggle-filters", onClick ToggleFilter ]
                                [ --ng-click="ui.showMethodsFilter=!ui.showMethodsFilter">
                            ...
                            [ div [ class "form-group" ]
                                ...
                                , div [ class "btn-group space-left" ]
                                    [ button
                                        ...
                                        , onClick (UpdateMethodFilter { filter | agent = Nothing })
                                        ]
                                        [ text "All" ]
                                    , button
                                        [ class
                                            ("btn btn-default"
                                                ++ (if filter.agent == Just Cfengine then
                                                        " active"
                                        ...
                                        , onClick (UpdateMethodFilter { filter | agent = Just Cfengine })
                                        ]
                                        [ text "Linux" ]
                                    , button
                                        [ class
                                            ("btn btn-default"
                                                ++ (if filter.agent == Just Dsc then
                                                        " active"
                                        ...
                                        , onClick (UpdateMethodFilter { filter | agent = Just Dsc })
                                        ]
                                        [ text "Windows "
                                        , span [ class "dsc-icon" ] []
                                        ]
    ...
    let
        ...
        agentCheck =
            case filter.agent of
                ...
                Just ag ->
                    List.member ag method.agentSupport
        ...
        docOpenedCheck =
            List.member method.id methodsUI.docsOpen
    in
    nameCheck
        && agentCheck
        && deprecatedCheck
...
showMethodsCategories model ( category, methods ) =
    let
        ...
        methodsElem =
            List.map (\m -> showMethod model.methodsUI m model.mode model.dnd) methods
    in
    element "ul"
        |> addClass "list-unstyled"
        |> appendChildList
...
showMethod ui method mode dnd =
    let
        docOpen =
            List.member method.id ui.docsOpen
        ...
        methodUi =
            case mode of
                ...
                _ ->
                    MethodCallUiInfo Closed CallParameters Unchanged (ForeachUI False False (defaultNewForeach Nothing Nothing))
    in
    element "li"
        |> appendChild
            (element "div"
                ...
                |> appendChildList
                    [ element "div"
                        ...
                        |> addAction
                            ( "click"
                            , UIMethodAction method.id
                                { methodUi
                                    | mode =
                                        if methodUi.mode == Opened then
                                            Closed
