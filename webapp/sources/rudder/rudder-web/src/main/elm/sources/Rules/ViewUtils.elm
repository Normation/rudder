...
getCategoryName model id =
    let
        cat =
            List.Extra.find (.id >> (==) id) (getListCategories model.rulesTree)
    in
    case cat of
        Just c ->
            c.name
...
subItemOrder fun model id =
    case List.Extra.find (Tuple3.first >> (==) id) fun.rows of
        Just ( _, _, sort ) ->
            \i1 i2 -> sort (fun.data model i1) (fun.data model i2)
...
valueCompliance complianceFilters =
    ItemFun
        ...
        , ( "Message", .message >> (\v -> pre[class "font-monospace"][text v]), \d1 d2 -> N.compare d1.message d2.message )
...
nodeValueCompliance mod complianceFilters =
    ItemFun
        (\item model sortId ->
            let
                sortFunction =
                    subItemOrder (valueCompliance complianceFilters) model sortId
            in
            item.values
    ...
    ItemFun
        (\item model sortId ->
            case item of
                Block b ->
                    let
                        sortFunction =
                            subItemOrder (byComponentCompliance subFun complianceFilters) model sortId
                    in
                    b.components
                ...
                Value c ->
                    let
                        sortFunction =
                            subItemOrder subFun model sortId
                    in
                    c.values
        ...
        (Just
            (\x ->
                case x of
                    Left b ->
                        showComplianceDetails (byComponentCompliance subFun complianceFilters) b

                    Right value ->
                        showComplianceDetails subFun value
            )
        )
        (\x ->
            case x of
                Block _ ->
                    List.map Tuple3.first (byComponentCompliance subFun complianceFilters).rows

                Value _ ->
                    List.map Tuple3.first subFun.rows
        )
        (always True)
...
byNodeCompliance mod complianceFilters =
    let
        directive =
            byDirectiveCompliance mod complianceFilters (valueCompliance complianceFilters)
    in
    ItemFun
        (\item _ sortId ->
            let
                sortFunction =
                    subItemOrder directive mod sortId
            in
            item.directives
...
showComplianceDetails fun compliance parent openedRows model =
    let
        itemRows =
            List.map Tuple3.second fun.rows

        data =
            fun.data model compliance

        detailsRows =
            List.map (\row -> td [] [ row data ]) itemRows
        ...
        rowOpened =
            Dict.get rowId openedRows

        defaultSort =
            Maybe.withDefault "" (List.head (fun.subItemRows compliance))
        ...
        ( details, classes ) =
            case ( fun.childDetails, rowOpened ) of
                ( Just detailsFun, Just ( sortId, sortOrder ) ) ->
                    let
                        childrenSort =
                            fun.children compliance model sortId

                        filteredChildren =
                            childrenSort

                        ( children, order, newOrder ) =
                            case sortOrder of
                                Asc ->
                                    ( childrenSort, "asc", Desc )

                                Desc ->
                                    ( List.reverse childrenSort, "desc", Asc )
                    in
                    ( [ tr [ class "details" ]
        ...
        spanTags t =
            span [ class "tags-label" ]
                ...
                , span [ class "tag-value" ]
                    [ if String.isEmpty t.value then
                        i [ class "fa fa-asterisk" ] []
...
buildIncludeList originRule groupsTree model editMode includeBool ruleTarget =
    ...
    case List.Extra.find (\g -> g.id == id) groupsList of
        Just gr ->
            targetItem gr.name gr.target gr.enabled
...
getNodeHostname model id =
    (Maybe.withDefault (NodeInfo id id "" "") (Dict.get id model.nodes)).hostname
