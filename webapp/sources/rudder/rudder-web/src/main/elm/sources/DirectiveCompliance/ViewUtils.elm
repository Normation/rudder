...
valueCompliance complianceFilters =
  ItemFun
    ...
    , ("Messages", .reports >> List.filter (filterReports complianceFilters) >> List.map (\r -> Maybe.withDefault "" r.message) >> List.map (\t -> text t) >> List.intersperse (br [] []) >> (\ls -> div[class "value-container"] ls ), (\d1 d2 -> N.compare d1.value d2.value) )
