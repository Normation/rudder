...
byNodeCompliance mod complianceFilters =
    let
        rule =
            byRuleCompliance mod (valueCompliance complianceFilters) complianceFilters
    in
    ItemFun
        (\item model sortId ->
            let
                sortFunction =
                    subItemOrder rule mod sortId
            in
            item.rules
...
byRuleCompliance model subFun complianceFilters =
    ...
    ItemFun
        ...
        (\m i -> i)
        [ ( "Rule", \i -> span [] [ i.skippedDetails |> Maybe.map badgeSkipped |> Maybe.withDefault (badgePolicyMode model.policyMode i.policyMode), text i.name, goToBtn (getRuleLink contextPath i.ruleId) ], \r1 r2 -> N.compare r1.name r2.name )
