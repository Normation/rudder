        ...
        editForm =
            case model.editAccount of
                ...
                Just account ->
                    let
                        displayAclPlugin =
                            account.authorizationType == ACL && model.aclPluginEnabled
                                ...
                                [ label [ for "newAccount-expiration", class "mb-1" ]
                                    ...
                                    , label [ for "selectDate", class "custom-toggle toggle-secondary" ]
                                        [ input
                                            [ type_ "checkbox"
                                            , id "selectDate"
                                            , checked <| account.expirationPolicy /= NeverExpire
                                            , onCheck
                                                (\c ->
                                                    UpdateAccountForm
                                                        (account
                                                            |> (if c then
                                                                    setExpirationPolicy (ExpireAtDate selectedDate)

                                                                else
                                                                    setExpirationPolicy NeverExpire
                                                               )
                                                        )
                                                )
                                            ]
