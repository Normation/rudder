...
import Ui.Datatable exposing (Category, SubCategories(..), getAllCats, getSubElems)
...
filterRuleElemsByIds ids category =
    let
        copyCat cat =
            { cat | elems = List.filter (\e -> List.member e.id.value ids) cat.elems }

        filteredElems =
            (copyCat category).elems
    in
    case category.subElems of
        Ui.Datatable.SubCategories subCats ->
            let
                newSubCats =
                    List.map (filterRuleElemsByIds ids) subCats
            in
...
badgeIncludedExcluded model ruleId =
    let
        isIncluded =
            List.member ruleId model.rulesMeta.includedRules

        isExcluded =
            List.member ruleId model.rulesMeta.excludedRules

        msg =
            -- TODO green red
            if isIncluded then
                Just "<div style='margin-bottom:5px;'>This rule is <b style='color:#9bc832;'>included</b> by the group.</div>"

            else if isExcluded then
                Just "<div style='margin-bottom:5px;'>This rule is <b style='color:#3694d1;'>excluded</b> by the group.</div>"
    ...
    case msg of
        Just m ->
            span [ class ("treeGroupName rudder-label label-sm " ++ labelClass), attribute "data-bs-toggle" "tooltip", attribute "data-bs-placement" "bottom", title (buildTooltipContent "Included/Excluded" m) ] []
...
getCategoryName model id =
    let
        cat =
            List.Extra.find (.id >> (==) id) (getAllCats model.ruleTree)
    in
    case cat of
        Just c ->
            c.name
