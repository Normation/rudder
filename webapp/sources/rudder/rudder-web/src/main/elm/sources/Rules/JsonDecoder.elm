...
decodeCategoryGroupTarget =
    succeed
        (\id name description categories groups targets ->
            let
                elems =
                    if id == "GroupRoot" then
                        groups

                    else
                        List.append groups targets
            in
...
decodeBlockCompliance elem decoder _ =
    require "name" string <|
        \name ->
            require "compliance" float <|
                \compliance ->
                    require "complianceDetails" decodeComplianceDetails <|
                        \details ->
                            require "components" (list (decodeComponentCompliance elem decoder)) <|
                                \components ->
