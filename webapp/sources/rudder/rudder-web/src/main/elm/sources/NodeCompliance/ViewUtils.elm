    ...
    ItemFun
        ...
        (Just
            (\x ->
                case x of
                    Left value ->
                        showComplianceDetails (byComponentCompliance subFun complianceFilters) value
...
byRuleCompliance mod complianceFilters =
    let
        directive =
            byDirectiveCompliance mod (valueCompliance complianceFilters) complianceFilters
    in
    ItemFun
        (\item model sortId ->
            let
        ...
        [ ( "Rule"
          , \r ->
                span []
                    [ badgePolicyMode mod.policyMode r.policyMode
                    , text r.name
                    , if mod.onlySystem then
                        text ""

                      else
                        goToBtn (getRuleLink mod.contextPath r.ruleId)
                    ]
          , \r1 r2 -> N.compare r1.name r2.name
          )
...
byDirectiveCompliance model subFun complianceFilters =
    ...
    ItemFun
        (\item m sortId ->
            let
                sortFunction =
                    subItemOrder (byComponentCompliance subFun complianceFilters) m sortId
            in
            item.components
                |> List.filter (filterByCompliance complianceFilters)
                |> List.sortWith sortFunction
        ...
        [ ( "Directive"
          , \i ->
                span []
                    [ i.skippedDetails |> Maybe.map badgeSkipped |> Maybe.withDefault (badgePolicyMode model.policyMode i.policyMode)
                    , text i.name
                    , if model.onlySystem then
                        text ""

                      else
                        goToBtn (getDirectiveLink contextPath i.directiveId)
                    ]
          , \d1 d2 -> N.compare d1.name d2.name
          )
