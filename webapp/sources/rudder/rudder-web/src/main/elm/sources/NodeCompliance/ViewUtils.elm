...
valueCompliance complianceFilters =
  ItemFun
    ...
    , ("Messages", .reports >> List.filter (filterReports complianceFilters) >> List.map (\r -> Maybe.withDefault "" r.message) >> List.map (\t -> text t) >> List.intersperse (br [] []) >> (\ls -> pre[class "value-container font-monospace"] ls ), (\d1 d2 -> N.compare d1.value d2.value) )
