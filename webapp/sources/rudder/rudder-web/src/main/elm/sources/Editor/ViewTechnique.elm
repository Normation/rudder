...
checkTechniqueId origin technique techniques =
    case origin of
        ...
        _ ->
            if List.any (.id >> (==) technique.id) techniques then
...
checkTechniqueName technique techniques =
    ...
    else if List.any (.name >> (==) technique.name) (List.filter (.id >> (/=) technique.id) techniques) then
                    ...
                    errors
                        |> List.map
                            (\e ->
                                case e of
                                    TooLongId ->
                                        "Technique ID is too long"

                                    AlreadyTakenId ->
                                        "Technique ID is already taken"
                            )
                        |> String.join ".\n"
            in
            ( True, invalidMsg )
                        ...
                        |> List.map
                            (\e ->
                                case e of
                                    ...
                                    AlreadyTakenName ->
                                        "Technique name is already taken"
                            )
                        |> String.join ".\n"
            in
            ( True, invalidMsg )
...
checkParameter param =
    not (Maybe.Extra.isNothing param.description && String.isEmpty param.name) && not (Regex.contains ((Regex.fromString >> Maybe.withDefault Regex.never) "[^_a-zA-Z\\d]") param.name)
...
checkInvalidStates t ui =
    let
        idState =
            invalidTechniqueIdState ui.idState

        nameState =
            invalidTechniqueNameState ui.nameState
        ...
        allStates =
            [ idState, nameState, parametersState ]
    ...
    let
        errorsOnParamByCallId =
            methodCallList
                |> List.map
                    (\mCall ->
                        ( if String.isEmpty mCall.component then
                            mCall.methodName.value
                                    ...
                                    paramErrors =
                                        List.map
                                            (\param ->
                                                let
                                                    paramConstraints =
                                                        case List.head (List.filter (\p -> param.id.value == p.name.value) method.parameters) of
                                                            Just paramWithConstraints ->
                                                                paramWithConstraints.constraints

                                                            _ ->
                                                                defaultConstraint

                                                    state =
                                                        accumulateErrorConstraint param paramConstraints ValidState
                                                in
                                                state
                                            )
                                            mCall.parameters
                ...
                (\mCall ->
                    case Dict.get mCall.methodName.value libMethods of
                        Just method ->
                            let
                                conditionErrors =
                                    checkConstraintOnCondition mCall.condition
                            in
                            ( mCall.id.value
                            , conditionErrors
                            )
                        ...
                        _ ->
                            ( mCall.methodName.value, ValidState )
                )
                methodCallList
    in
    Dict.fromList
...
checkBlocksOnError methodElems =
    let
        methodsBlocks =
            List.concatMap getAllBlocks methodElems
                                    ...
                                    |> List.map
                                        (\e ->
                                            case e of
                                                ...
                                                NoFocusError ->
                                                    "A method must be selected if the reporting of the block is based on one child method report"

                                                ConditionError ->
                                                    "There are errors on block conditions"
                                        )
                                    |> String.join ".\n"
                        in
                        Just invalidMsg
...
showTechnique model technique origin ui editInfo =
    let
        fakeMetadata =
            Http.Metadata "internal-elm-call" 200 "call from elm app" Dict.empty

        errorOnBlocks =
            checkBlocksOnError technique.elems
        ...
        methodCallList =
            List.concatMap getAllCalls technique.elems

        statesByMethodIdCondition =
            listAllMethodWithErrorOnCondition methodCallList model.methods

        -- Keep the ID of the method in the UI if it contains invalid parameters value
        errorsOnMethodParameters =
            listAllMethodWithErrorOnParameters methodCallList model.methods

        areErrorOnMethodCondition =
            List.isEmpty (Dict.keys statesByMethodIdCondition)

        isMethodListEmpty =
            List.isEmpty technique.elems

        areResourceUpdated =
            List.any (.state >> (/=) Untouched) technique.resources
        ...
        deleteAction =
            case origin of
                Creation id ->
                    DeleteTechnique (Ok id)

                Clone _ _ id ->
                    DeleteTechnique (Ok id)

                Edit _ ->
                    OpenDeletionPopup { id = technique.id, name = technique.name, version = technique.version }
            ...
            element "ul"
                ...
                |> appendChildConditional
                    ...
                    (case DragDrop.currentlyDraggedObject model.dnd of
                        ...
                        Just _ ->
                            not (List.isEmpty technique.elems)
                    )
                |> appendChildList
                    (List.concatMap
                        (\call ->
                                ...
                                elem =
                                    case call of
                                        Call parentId c ->
                                            let
                                                methodUi =
                                                    Maybe.withDefault (MethodCallUiInfo Closed CallParameters Unchanged (ForeachUI False False (defaultNewForeach c.foreachName c.foreach))) (Dict.get c.id.value ui.callsUI)
                                            in
                                        ...
                                        Block parentId b ->
                                            let
                                                methodUi =
                                                    Maybe.withDefault (MethodBlockUiInfo Closed Children ValidState True (ForeachUI False False (defaultNewForeach b.foreachName b.foreach))) (Dict.get b.id.value ui.blockUI)
                                            in
        ...
        checkList =
            let
                invalidStates =
                    checkInvalidStates technique ui

                invalidTechnique =
                    not (List.isEmpty invalidStates)
            in
                ...
                , li [ class "nav-item" ]
                    [ button
                        ...
                        , span
                            ...
                            [ if List.isEmpty technique.resources || List.any (\s -> (s.state == Untouched) || (s.state == Modified)) technique.resources then
                                span [ class "nb-resources" ] [ text (String.fromInt (List.length (List.filter (\s -> s.state == Untouched || s.state == Modified) technique.resources))) ]

                              else
                                text ""
                            , if not (List.isEmpty (List.filter (.state >> (==) New) technique.resources)) then
                                span [ class "nb-resources new" ] [ text (String.fromInt (List.length (List.filter (.state >> (==) New) technique.resources))) ]

                              else
                                text ""
                            , if not (List.isEmpty (List.filter (.state >> (==) Deleted) technique.resources)) then
                                span [ class "nb-resources del" ] [ text (String.fromInt (List.length (List.filter (.state >> (==) Deleted) technique.resources))) ]
                ...
                Introduction ->
                    div [ class "main-container" ]
                        [ div [ class "col jumbotron" ]
                            ...
                            , if not model.hasWriteRights then
                                text ""
                    ...
                    , div [ class "modal-dialog" ]
                        [ div [ class "modal-content" ]
                            ...
                            , div [ class "modal-footer" ]
                                ...
                                , button [ class "btn btn-danger", onClick (ClosePopup (CallApi (deleteTechnique technique))) ]
                                    [ --ng-click="confirm()"></button>
                                      text "Delete "
                                    , i [ class "fa fa-times-circle" ] []
                                    ]
                                ]
