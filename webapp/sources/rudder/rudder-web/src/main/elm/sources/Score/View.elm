        ...
        [ div [ class "global-score d-flex" ]
            (case model.score of
                Just complianceScore ->
                    let
                        ( message, name ) =
                            case model.scoreToShow of
                                Nothing ->
                                    ( complianceScore.message, "Global" )

                                Just scoreId ->
                                    let
                                        msg =
                                            complianceScore.details |> List.Extra.find (.scoreId >> (==) scoreId) |> Maybe.map .message |> Maybe.withDefault complianceScore.message

                                        n =
                                            List.Extra.find (.id >> (==) scoreId) model.scoreInfo |> Maybe.map .name |> Maybe.withDefault (String.Extra.humanize scoreId)
                                    in
                                    ( msg, n )
                    in
                ...
                Nothing ->
                    let
                        noComplianceMsg =
                            "There is no score for this node"
                    in
