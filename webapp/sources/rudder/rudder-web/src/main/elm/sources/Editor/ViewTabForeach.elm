...
type alias CallForeach a =
    { a | foreachName : Maybe String, foreach : Maybe (List (Dict String String)) }



-- a = MethodCall OR MethodBlock
...
getUpdateMessages foreachUI call toMsg =
    let
        ...
        newItems =
            case foreach of
                Just f ->
                    List.append f [ newForeach.newItem ]
            ...
            case foreach of
                ...
                Just items ->
                    let
                        ...
                        updatedForeach =
                            items
                                |> List.Extra.updateIf (\f -> Dict.keys f |> List.any (\k -> List.Extra.notMember k newKeysList))
                                    -- If an old key is not present anymore, remove it
                                    (\f -> f |> keepOnly (Set.fromList newKeysList))
                                |> List.Extra.updateIf (\f -> newKeysList |> List.any (\k -> List.Extra.notMember k (Dict.keys f)))
                                    -- If a new key is detected, insert it
                                    (\f ->
                                        let
                                            keysList =
                                                Dict.keys f
    ...
    in
    { addForeach =
        toMsg { foreachUI | newForeach = { newForeach | newItem = newItem } }
            { call
                | foreachName =
                    Just
                        (if String.isEmpty newForeach.foreachName then
                            "item"

                         else
                            newForeach.foreachName
                        )
                , foreach = Just [ newItem ]
            }
...
displayTabForeach uiInfo =
    let
        { callId, foreachName, foreach, updateForeach, foreachUI, updateUIMsg, updateMsg } =
            case uiInfo of
        ...
        { removeKey, updateNewForeach, updateNewForeachKey, addNewKey, resetNewForeach, updateNewItem, editForeachName, editForeachKeys } =
            updateUIMsg

        { addForeach, addNewItem, saveNewForeach, saveEditKeys, removeForeach } =
            updateMsg
                                    ...
                                    keys
                                        |> List.map
                                            (\k ->
                                                let
                                                    val =
                                                        case Dict.get k newForeach.newItem of
                                                            Just v ->
                                                                v
