...
view model =
    if model.ui.hasNodeRead then
        let
            ...
            isJson =
                newProperty.format == JsonFormat

            trimmedName =
                String.trim newProperty.name

            trimmedVal =
                String.trim newProperty.value

            checkPristineName =
                not newProperty.pristineName

            checkEmptyName =
                String.isEmpty trimmedName

            checkAlreadyUsedName =
                checkUsedName trimmedName model.properties

            checkEmptyVal =
                String.isEmpty trimmedVal

            checkPristineVal =
                not newProperty.pristineValue

            checkFormatConflict =
                List.Extra.find (\p -> p.name == trimmedName && getFormat p /= newProperty.format) model.properties /= Nothing

            checkFormatVal =
                newProperty.errorFormat

            checks =
                [ checkEmptyName, checkAlreadyUsedName, checkEmptyVal, checkFormatVal ]

            filters =
                model.ui.filtersOnProperty
        in
        div []
                    ...
                    div [ class "col-lg-7 col-md-8 col-xs-12 add-prop-form" ]
                        ...
                        , div [ class "input-group has-validation align-items-start" ]
                            ...
                            , textarea
                                [ placeholder "Value"
                                , class
                                    ("form-control input-value auto-resize code"
                                        ++ (if (checkEmptyVal && checkPristineVal) || checkFormatVal then
                                                " is-invalid"
                        ...
                        , div [ class "errors" ]
                            [ if checkEmptyName && checkPristineName then
                                div [ class "text-danger" ] [ text "Name is required." ]

                              else
                                text ""
                            , if checkAlreadyUsedName then
                                div [ class "text-danger" ] [ text "This name is already used by another property." ]

                              else
                                text ""
                            , if checkEmptyVal && checkPristineVal then
                                div [ class "text-danger" ] [ text "Value is required." ]

                              else
                                text ""
                            , if checkFormatConflict then
                                div [ class "text-danger" ] [ text "The selected format is conflicting with the format of the existing property." ]
