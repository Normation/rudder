    ...
    case String.split "." class_context of
        ...
        [ _ ] ->
            case parseOs class_context of
                Nothing ->
                    Condition Nothing class_context
            ...
            \v ->
                case v of
                    "worst-case-weighted-sum" ->
                        succeed (WorstReport WorstReportWeightedSum)

                    "worst-case-weighted-one" ->
                        succeed (WorstReport WorstReportWeightedOne)

                    "focus-worst" ->
                        succeed (WorstReport FocusWorst)

                    "weighted" ->
                        succeed WeightedReport

                    "focus" ->
                        succeed (FocusReport "")

                    value ->
                        if String.startsWith "focus:" value then
                            let
                                focusValue =
                                    String.split ":" value |> List.tail |> Maybe.withDefault [] |> String.join ""
                            in
                            succeed (FocusReport focusValue)

                        else
                            fail (v ++ " is not a valid reporting logic")
    in
    string
                    ...
                    | calls =
                        List.map
                            (\x ->
                                case x of
                                    Block _ b ->
                                        Block (Just block.id) b

                                    Call _ c ->
                                        Call (Just block.id) c
                            )
                            block.calls
                }
            )
    ...
    field "source" string
        |> andThen
            (\v ->
                case v of
                    "editor" ->
                        map Just decodeTechniqueOrError
...
decoderSelectOption =
    oneOf
        [ map (\s -> SelectOption s Nothing) string
