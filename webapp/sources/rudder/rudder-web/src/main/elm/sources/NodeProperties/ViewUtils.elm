...
import SyntaxHighlight exposing (gitHub, json, toInlineHtml, useTheme)
...
checkExistingFormatConflict model propertyName =
    ...
    getPossibleFormatsFromPropertyName model propertyName
        ...
        |> List.Extra.uniqueBy valueFormatToString
        >> List.length
        >> (\l -> l > 1)
...
getParentPropertyDisplay pp =
    case pp of
        ParentGlobal { valueType } ->
            [ "<span>Value of type: <strong>" ++ valueType ++ "</strong> for global parameter</span>" ]

        ParentGroup { id, name, valueType, parent } ->
            ("<span>Value of type: <strong>" ++ valueType ++ "</strong> in group " ++ "\"" ++ name ++ "\" with id <em>" ++ id ++ "</em></span>") :: (Maybe.withDefault [] <| Maybe.map getParentPropertyDisplay parent)
...
getSortFunction model p1 p2 =
    let
        order =
            case model.ui.filtersOnProperty.sortBy of
                Name ->
                    N.compare p1.name p2.name
                ...
                Value ->
                    N.compare (Json.Encode.encode 0 p1.value) (Json.Encode.encode 0 p2.value)
    in
    ...
    properties
        |> List.any
            (\p ->
                case p.provider of
                    Just provider ->
                        provider /= "inherited" && p.name == name
...
displayPropertiesError model =
    case model.errorMessage of
        Just err ->
            -- error may contain new lines, separate them with <br/> tag
    ...
    let
        ...
        propertyRow p =
            let
                ...
                origVal =
                    Maybe.withDefault p.value p.origval

                formatConflict =
                    getFormatConflictWarning p

                inheritedPropertyError =
                    getInheritedPropertyWarning p

                defaultEditProperty =
                    EditProperty p.name (displayJsonValue origVal) format True True False
                        ...
                        Just pr ->
                            let
                                pTitle =
                                    case pr of
                                        "inherited" ->
                                            "<h4 class='tags-tooltip-title'>Inherited</h4> <div class='tooltip-inner-content'>This property is inherited " ++ Maybe.withDefault "" p.hierarchy ++ "</div>."

                                        "overridden" ->
                                            "<h4 class='tags-tooltip-title'>Overridden</h4> <div class='tooltip-inner-content'>This property is overridden on this object and was inherited " ++ Maybe.withDefault "" p.hierarchy ++ "</div>."
                        ...
                        , td [ class "property-value" ]
                            [ div []
                                [ div
                                    [ class
                                        ("value-container"
                                            ++ (if List.member p.name model.ui.showMore then
                                                    " toggle"

                                                else
                                                    ""
                                               )
                                            ++ (if isTooLong p.value then
                                                    " show-more"
                ...
                Just eP ->
                    let
                        ...
                        checkAlreadyUsedName =
                            trimmedName /= p.name && checkUsedName trimmedName model.properties

                        checkEmptyVal =
                            String.isEmpty trimmedVal

                        checkPristineVal =
                            not eP.pristineValue

                        checkFormatConflict =
                            checkExistingFormatConflict model trimmedName

                        invalidName =
                            checkAlreadyUsedName || (checkEmptyName && checkPristineName) || checkFormatConflict
                        ...
                        invalidValue =
                            checkEmptyVal && checkPristineVal
                    in
                    tr []
                        [ td [ class "is-edited" ]
                            ...
                            [ div [ class "needs-validation" ]
                                [ div [ class "value-container" ]
                                    [ textarea
                                        [ placeholder "Value"
                                        , class
                                            ("form-control input-sm input-value auto-resize code"
...
showModal model =
    case model.ui.modalState of
        ...
        Usage pName found ->
            let
                pagination =
                    model.ui.filtersOnUsage.pagination

                filters =
                    model.ui.filtersOnUsage

                page =
                    case filters.findUsageIn of
                        Directives ->
                            pagination.pageDirective

                        Techniques ->
                            pagination.pageTechnique
                ...
                start =
                    ...
                    min (start + tableSize) totalNbRow
                ...
                classNextBtn =
                    if page >= getPageMax pagination then
                        "disabled"
                ...
                nbDirective =
                    getSearchResultLength found Directives

                nbTechnique =
                    getSearchResultLength found Techniques
                ...
                hasNoUsage =
                    case filters.findUsageIn of
                        Techniques ->
                            found.techniques |> Maybe.andThen (.items >> List.head) |> Maybe.Extra.isNothing

                        Directives ->
                            found.directives |> Maybe.andThen (.items >> List.head) |> Maybe.Extra.isNothing

                messageNoUsageFound =
                    case filters.findUsageIn of
                        Techniques ->
                            "No usage found in Techniques"

                        Directives ->
                            "No usage found in Directives"
            in
