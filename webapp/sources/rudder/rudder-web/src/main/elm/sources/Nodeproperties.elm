...
update msg model =
    case msg of
        ...
        UpdateNewProperty newProperty ->
            let
                possibleFormats =
                    getPossibleFormatsFromPropertyName model newProperty.name
        ...
        UpdateProperty key newProperty ->
            let
                ...
                newProperties =
                    Dict.update key (always (Just newProperty)) ui.editedProperties
            in
        ...
        UpdateTableSize size ->
            let
                ...
                newModel =
                    { model | ui = { ui | filtersOnUsage = { filtersModelUsage | pagination = newPagination } } }
            in
            ...
            let
                ...
                newPageDir =
                    case filtersModelUsage.findUsageIn of
                        Directives ->
                            if page > getPageMax pagination || page < 1 then
                                1
                ...
                newModel =
                    { model | ui = { ui | filtersOnUsage = { filtersModelUsage | pagination = { pagination | pageDirective = newPageDir, pageTechnique = newPageTech } } } }
            in
            ( newModel, Cmd.none )
        ...
        PreviousPage ->
            let
                ...
                pageOnDir =
                    model.ui.filtersOnUsage.pagination.pageDirective

                pageOnTech =
                    model.ui.filtersOnUsage.pagination.pageTechnique
            ...
            case res of
                Ok found ->
                    let
                        ...
                        maxNbRow =
                            getSearchResultLength propertyUsage Directives

                        pagi =
                            TablePagination 1 1 10 maxNbRow
                        ...
                        newModel =
                            { model | ui = { ui | modalState = Usage pName propertyUsage, filtersOnUsage = { filtersModel | findUsageIn = Directives, pagination = pagi } } }
                    in
                    ( newModel, Cmd.batch [ initInputs "", initTooltips "" ] )
        ...
        ChangeViewUsage ->
            let
                switchView =
                    if model.ui.filtersOnUsage.findUsageIn == Directives then
                        Techniques
                ...
                nbDirectives =
                    case model.ui.modalState of
                        Usage propName findUsage ->
                            getSearchResultLength findUsage Directives
                ...
                nbTechniques =
                    case model.ui.modalState of
                        Usage propName findUsage ->
                            getSearchResultLength findUsage Techniques
                ...
                newModel =
                    { model | ui = { ui | filtersOnUsage = { filtersModelUsage | findUsageIn = switchView, pagination = { pagination | totalRow = nbRow } } } }
            in
            ( newModel, Cmd.none )
        ...
        AddProperty ->
            let
                successMsg =
                    "property '" ++ String.trim model.newProperty.name ++ "' has been added"

                ( newModel, cmd ) =
                    case model.newProperty.format of
                        StringFormat ->
                        ...
                        JsonFormat ->
                            let
                                checkJsonFormat =
                                    Json.Decode.decodeString Json.Decode.value model.newProperty.value
                            in
        ...
        ToggleEditProperty key property save ->
            if Dict.member key model.ui.editedProperties then
                -- If the property is being edited
                let
                    ui =
                        model.ui
                    ...
                    updatedModel =
                        { model | ui = { ui | editedProperties = editedProperties } }
                    ...
                    ( newModel, cmd ) =
                        if save then
                            ...
                            case property.format of
                                StringFormat ->
                                    ( updatedModel, saveProperty updatedProperty model successMsg )

                                JsonFormat ->
                                    let
                                        checkJsonFormat =
                                            Json.Decode.decodeString Json.Decode.value property.value
                                    in
                                    case checkJsonFormat of
                                        Ok s ->
                                        ...
                                        Err _ ->
                                            ( model, errorNotification "JSON check is enabled, but the value format is invalid." )
        ...
        ShowMore id ->
            let
                ...
                showMore =
                    if List.member id ui.showMore then
                        List.Extra.remove id ui.showMore
        ...
        ClosePopup callback ->
            let
                ...
                initPagination =
                    TablePagination 1 1 10 0

                ( nm, cmd ) =
                    update callback { model | ui = { ui | modalState = NoModal, filtersOnUsage = { filtersModel | pagination = initPagination, findUsageIn = Directives, filter = "", sortBy = Name, sortOrder = Asc } } }
            in
            ( nm, cmd )
