...
getAllCalls call =
    case call of
        Block _ b ->
            List.concatMap getAllCalls b.calls
...
getAllBlocks call =
    case call of
        Block _ b ->
            b :: List.concatMap getAllBlocks b.calls
...
updateElemIf predicate updateFun list =
    let
        updateLowerLevel =
            List.Extra.updateIf predicate updateFun list
    in
    List.map
        (\x ->
            case x of
                Block p b ->
                    Block p { b | calls = updateElemIf predicate updateFun b.calls }
...
removeElem predicate list =
    let
        updateLowerLevel =
            List.Extra.filterNot predicate list
    in
    List.map
        (\x ->
            case x of
                Block p b ->
                    Block p { b | calls = removeElem predicate b.calls }
...
setId newId x =
    case x of
        Call parent c ->
            Call parent { c | id = newId }

        Block parent b ->
            Block parent { b | id = newId }
...
checkBlockConstraint block =
    let
        ...
        fold =
            \acc h ->
                case ( acc, h ) of
                    ( InvalidState err1, InvalidState err2 ) ->
                        InvalidState (List.concat [ err1, err2 ])
