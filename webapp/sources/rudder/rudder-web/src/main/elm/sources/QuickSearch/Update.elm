module QuickSearch.Update exposing (Effect(..), Msg(..), update, update_)
...
update msg model =
    ...
    case effect of
        ...
        DebouncePush search ->
            let
                ( debounce, cmd ) =
                    Debounce.push debounceConfig search updatedModel.debounceSearch
            in
            ( updatedModel |> setDebounce debounce, cmd )
                        ...
                        (Debounce.takeLast
                            (\s ->
                                if String.length s > 2 then
                                    getSearchResult model s GetResults
