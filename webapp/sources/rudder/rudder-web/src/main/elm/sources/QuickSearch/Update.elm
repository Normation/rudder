    ...
    case msg of
        UpdateFilter filter ->
            case filter of
                ...
                FilterKind k ->
                    let
                        updatedFilter =
                            if List.member k model.selectedFilter then
                                List.Extra.remove k model.selectedFilter

                            else
                                k :: model.selectedFilter
                    in
        ...
        UpdateSearch search ->
            let
                ...
                newModel =
                    { model | search = search, state = state, debounceSearch = debounce }
            in
            ( newModel, cmd )
