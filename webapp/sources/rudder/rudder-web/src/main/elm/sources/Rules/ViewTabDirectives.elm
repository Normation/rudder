...
directivesTab model details =
    let
        ...
        fun =
            byDirectiveCompliance model complianceFilters (nodeValueCompliance model complianceFilters)

        directiveRows =
            List.map Tuple3.first fun.rows

        rowId =
            "byDirectives/"

        ( sortId, sortOrder ) =
            Dict.get rowId ui.openedRows |> Maybe.withDefault ( "Directive", Asc )

        sort =
            case List.Extra.find (Tuple3.first >> (==) sortId) fun.rows of
                Just ( _, _, sortFun ) ->
                    \i1 i2 -> sortFun (fun.data model i1) (fun.data model i2)

                Nothing ->
                    \_ _ -> EQ

        filter =
            model.ui.directiveFilters.tableFilters.filter

        childs =
            Maybe.withDefault [] (Maybe.map .directives details.compliance)
        ...
        ( directivesChildren, order, newOrder ) =
            case sortOrder of
                Asc ->
        ...
        noDirectives =
            List.isEmpty ruleDirectives

        hasDirectivesButNoCompliance =
            List.isEmpty childs && not noDirectives

        noCompliance =
            not rule.enabled || hasDirectivesButNoCompliance
        ...
        noNodes =
            Maybe.withDefault 1 (getRuleNbNodes details) <= 0
        ...
        directiveComplianceTable =
            if Maybe.Extra.isNothing details.compliance then
                -- Compliance is not loaded yet
                    ...
                    [ let
                        ...
                        sortedDirectives =
                            case tableFilters.sortOrder of
                                ...
                                Desc ->
                                    List.reverse filteredDirectives
                      ...
                      if noCompliance then
                        table [ class "dataTable" ]
                            [ thead []
                                [ tr [ class "head" ]
                                    [ th
                                        [ onClick (UpdateDirectiveFilters { directiveFilters | tableFilters = { tableFilters | sortOrder = toggleSortOrder tableFilters.sortOrder } })
                                        , class
                                            ("sorting_"
                                                ++ (if tableFilters.sortOrder == Asc then
                                                        "asc"
