...
import Editor.ViewTabForeach exposing (displayTabForeach, foreachLabel)
    ...
    let
        isHovered =
            case model.isMethodHovered of
                Just methodId ->
                    if methodId.value == block.id.value then
                        "hovered"
...
showBlockTab model parentId block uiInfo techniqueUi =
    case uiInfo.tab of
            ...
            let
                ...
                ubuntuLi =
                    List.map
                        (\ubuntuMinor ->
                            let
                                updatedCall =
                                    Block parentId { block | condition = { condition | os = updateUbuntuMinor ubuntuMinor condition.os } }
                            in
                ...
                updateConditionVersion =
                    \f s ->
                        let
                            updatedCall =
                                Block parentId { block | condition = { condition | os = f (String.toInt s) condition.os } }
                        in
                                    ...
                                    (element "div"
                                        ...
                                        |> appendNodeConditional
                                            (input
                                                ...
                                                , onInput (updateConditionVersion updateMinorVersion)
                                                , type_ "number"
                                                , style "display" "inline-block"
                                                , style "width" "auto"
                                                , class "form-control"
                                        ...
                                        |> appendNodeConditional
                                            (input
                                                ...
                                                , onInput (updateConditionVersion updateVersion)
                                                , type_ "number"
                                                , style "display" "inline-block"
                                                , style "width" "auto"
                                                , class "form-control"
                                        ...
                                        |> appendNodeConditional
                                            (input
                                                ...
                                                , onInput (updateConditionVersion updateSP)
                                                , type_ "number"
                                                , style "display" "inline-block"
                                                , style "width" "auto"
                                                , class "form-control"
                            ...
                            , onInput
                                (\s ->
                                    let
                                        ...
                                        updatedCall =
                                            Block parentId { block | condition = updatedCondition }
                                    in
                                    MethodCallModified updatedCall Nothing
                                )
                            ]
        ...
        Children ->
            showChildren model block { uiInfo | showChildDetails = True } techniqueUi parentId
        ...
        BlockReporting ->
            let
                ...
                compositionText =
                    \reportingLogic ->
                        case reportingLogic of
                            ...
                            WeightedReport ->
                                "Weighted sum of reports"

                            FocusReport _ ->
                                "Focus on one child method report"
                ...
                availableComposition =
                    List.map liCompositionRule [ WeightedReport, FocusReport "", WorstReport WorstReportWeightedSum ]
                            ...
                            component =
                                if componentValue == "" then
                                    case child of
                                        ...
                                        Call _ c ->
                                            Maybe.withDefault c.methodName.value (Maybe.map .name (Dict.get c.methodName.value model.methods))
                    ...
                    \weight ->
                        case weight of
                            WorstReportWeightedOne ->
                                "Use a weight of '1' for component"

                            WorstReportWeightedSum ->
                                "Use sum of sub-components for weight"

                            FocusWorst ->
                                "Focus on the child with worst compliance"
                ...
                availableWorst =
                    List.map liWorst [ FocusWorst, WorstReportWeightedOne, WorstReportWeightedSum ]
            in
            element "div"
                |> appendChildList
                ...
                |> appendChild
                    (case block.reportingLogic of
                        FocusReport value ->
                            let
                                methodElem =
                                    findElemIf (\e -> (getId e).value == value) block.calls
...
blockBody model parentId block ui techniqueUi =
    let
        ( textClass, tooltipContent ) =
            case ui.validation of
                InvalidState _ ->
                    ( "text-danger", "<div class='tooltip-inner-content'>This block is invalid</div>" )

                Unchanged ->
                    ( "", "" )

                ValidState ->
                    ( "text-primary", "<div class='tooltip-inner-content'>This method was modified</div>" )
    ...
    element "div"
        ...
        |> (if techniqueUi.enableDragDrop == Just block.id then
            ...
            , element "div"
                ...
                |> appendChildList
                    ...
                    , element "div"
                        ...
                        -- (block.condition.os /= Nothing || block.condition.advanced /= "")
            ...
            (element "ul"
                        ...
                        |> appendChild
                            (element "button"
                                ...
                                |> appendChild
                                    (element "span"
                                        |> addClass
                                            ("expandBlockChild fas fa-chevron-"
                                                ++ (if ui.showChildDetails then
                                                        "up"
                ...
                |> appendChildConditional
                    (element "li"
                        ...
                        |> addClass
                            (if DragDrop.isCurrentDropTarget model.dnd (InBlock block) then
                                " drop-target"
                ...
                |> appendChildConditional
                    ...
                    (case ( List.isEmpty block.calls, DragDrop.currentlyDraggedObject model.dnd ) of
                        ...
                        ( _, Just (Move x) ) ->
                            Maybe.withDefault True (Maybe.map (\c -> getId x /= getId c) (List.head block.calls))

                        ( _, Just _ ) ->
                            not (List.isEmpty block.calls)
                    )
                |> appendChildList
                    (List.concatMap
                        (\call ->
                            (case call of
                                Call _ c ->
                                    let
                                        methodUi =
                                            Maybe.withDefault (MethodCallUiInfo Closed CallParameters Unchanged (ForeachUI False False (defaultNewForeach c.foreachName c.foreach))) (Dict.get c.id.value techniqueUi.callsUI)
                                        ...
                                        base =
                                            [ showMethodCall model methodUi techniqueUi (Just block.id) c ]

                                        dropElem =
                                            AfterElem (Just block.id) (Call parentId c)
                                ...
                                Block _ b ->
                                    let
                                        methodUi =
                                            Maybe.withDefault (MethodBlockUiInfo Closed Children ValidState True (ForeachUI False False (defaultNewForeach b.foreachName b.foreach))) (Dict.get b.id.value techniqueUi.blockUI)
                                    in
