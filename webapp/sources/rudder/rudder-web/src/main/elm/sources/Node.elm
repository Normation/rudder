...
import Node.Init exposing (errorNotification, init)
...
subscriptions _ =
    receiveDetails (\d -> ReceiveDetails d.name d.html)
                        ...
                        filterScript elem =
                            case elem of
                                ...
                                Element "i" attributes children ->
                                    Element "i" (attributes ++ [ ( "class", "icon-state" ) ]) (children |> List.map filterScript)

                                Element tag attributes children ->
                                    Element tag attributes (children |> List.map filterScript)

                                s ->
                                    s

                        html =
                            Html.Parser.Util.toVirtualDom (htmlString |> List.map filterScript)
                    in
