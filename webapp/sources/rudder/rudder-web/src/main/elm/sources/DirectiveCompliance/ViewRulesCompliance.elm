...
displayRulesComplianceTable model =
    let
        ...
        fun =
            byRuleCompliance model (nodeValueCompliance model complianceFilters) complianceFilters

        col =
            "Rule"

        childs =
            case model.directiveCompliance of
                Just dc ->
                    dc.rules
        ...
        ( sortId, sortOrder ) =
            Dict.get rowId filters.openedRows |> Maybe.withDefault ( col, Asc )
            ...
            [ div [ class "table-header extra-filters" ]
                [ div [ class "main-filters" ]
                    ...
                    , div [ class "ms-auto my-auto" ]
                        [ csvExportDropdownAllEntries (CallApi (\m -> Task.perform (ExportDirectiveComplianceByRule m.directiveId) Date.today)) "me-2"
                        , button [ class "btn btn-default btn-refresh", onClick (CallApi getDirectiveCompliance) ]
