...
import Compliance.Utils exposing (badgePolicyMode, defaultComplianceFilter, getAllComplianceValues)
import Html exposing (Html, div, i, span, td, text, tr)
import Html.Attributes exposing (attribute, class, colspan, title)
...
getSortFunction model r1 r2 =
    let
        order =
            case model.ui.ruleFilters.tableFilters.sortBy of
                RuleChanges ->
                    let
                        getChanges =
                            \r -> countRecentChanges r.id model.changes
                ...
                Parent ->
                    let
                        o =
                            NaturalOrdering.compare (getCategoryName model r1.categoryId) (getCategoryName model r2.categoryId)
                    in
                    case o of
                        EQ ->
                            NaturalOrdering.compare r1.name r2.name

                        _ ->
                            o

                Status ->
                    NaturalOrdering.compare r1.status.value r2.status.value
                ...
                Compliance ->
                    let
                        ...
                        getCompliance rc =
                            case rc of
                                Just c ->
                                    let
                                        allComplianceValues =
                                            getAllComplianceValues c.complianceDetails
                                    in
                        ...
                        r1Compliance =
                            getCompliance (getRuleCompliance model r1.id)

                        r2Compliance =
                            getCompliance (getRuleCompliance model r2.id)
                    in
...
buildRulesTable model rules =
    let
        ...
        rowTable r =
            let
                categoryName =
                    text (getCategoryName model r.categoryId)
                ...
                changes =
                    text (String.fromFloat (countRecentChanges r.id model.changes))
