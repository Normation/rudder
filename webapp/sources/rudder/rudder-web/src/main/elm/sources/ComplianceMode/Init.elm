...
init flags =
    let
        ...
        initUi =
            UI flags.hasWriteRights

        currentMode =
            getComplianceMode flags.complianceMode.name

        initModel =
            Model flags.contextPath initUi currentMode currentMode (getComplianceMode flags.globalMode.name)
    in
    ( initModel, Cmd.none )
