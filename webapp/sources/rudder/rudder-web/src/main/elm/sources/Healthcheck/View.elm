...
import Healthcheck.DataTypes exposing (Check, Model, Msg(..), SeverityLevel(..))
...
compareCheck a b =
    let
        orderLevel =
            compareSeverityLevel a.level b.level
    in
    case orderLevel of
        EQ ->
...
chooseHigherSecurityLevel checks =
    let
        higherPriority =
            minimumWith (\c1 c2 -> compareSeverityLevel c1.level c2.level) checks
    in
...
severityLevelToIcon level =
    case level of
        Critical ->
            i [ class "fa fa-times-circle critical-icon icon-info" ] []

        Warning ->
            i [ class "fa fa-exclamation-circle warning-icon icon-info" ] []

        CheckPassed ->
            i [ class "fa fa-check-circle ok-icon icon-info" ] []
...
displayBigMessage checks =
    ...
    div [ class "global-msg" ]
        [ div [ class (levelStr ++ "-info") ]
            ...
            , case level of
                Critical ->
                    text "Some elements require your attention"
        ...
        content =
            map
                (\c ->
                    let
                        classNameCircle =
                            severityLevelToString c.level ++ "-light circle "

                        msgCheck =
                            intersperse (br [] []) (map text (lines c.msg))
                    in
                    div [ class "check" ]
