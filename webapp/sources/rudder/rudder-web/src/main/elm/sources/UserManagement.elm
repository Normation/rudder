    ...
    case msg of
        ...
        RemoveRole user provider r ->
            let
                ...
                newRoles =
                    Dict.get provider user.providersInfo |> Maybe.map .roles |> Maybe.withDefault [] |> List.filter (\x -> r /= x)
                ...
                newAuthz =
                    ...
                    let
                        allAuthz =
                            Dict.toList model.roles
                                |> List.concatMap
                                    (\( role, authz ) ->
                                        if List.member role newRoles then
                                            authz
                ...
                newPanelMode =
                    EditMode { user | authz = newAuthz, roles = newRoles }
        ...
        Password newPassword ->
            let
                ...
                newUserForm =
                    { userForm | password = newPassword, isValidInput = ValidInputs }
            in
        ...
        Login newLogin ->
            let
                ...
                newUserForm =
                    { userForm | login = newLogin, isValidInput = ValidInputs }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        NewUserInfoFieldKey key idx ->
            let
                ...
                newUserForm =
                    { userForm
                        | newUserInfoFields = newUserInfoFields
                        , isValidInput =
                            if key == "" then
        ...
        RemoveNewUserInfoField idx ->
            let
                ...
                newUserForm =
                    { userForm | newUserInfoFields = newUserInfoFields }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        ModifyUserInfoField key value ->
            let
                ...
                newUserInfoFields =
                    Dict.insert key value userInfoForm.info

                newUserInfoForm =
                    { userInfoForm | info = newUserInfoFields }

                newUserForm =
                    { userForm | userInfoForm = newUserInfoForm }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        AddUserInfoField ->
            let
                ...
                newUserForm =
                    { userForm | newUserInfoFields = userForm.newUserInfoFields ++ [ ( "", "" ) ] }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        RemoveUserOtherInfoField key ->
            let
                ...
                newUserInfoForm =
                    { userInfoForm | otherInfo = Dict.remove key userInfoForm.otherInfo }
        ...
        RemoveUserInfoField key ->
            let
                ...
                newUserInfoForm =
                    { userInfoForm | info = Dict.remove key userInfoForm.info }
        ...
        UserInfoName name ->
            let
                ...
                newUserForm =
                    { userForm | userInfoForm = newUserInfoForm, isValidInput = ValidInputs }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        SubmitUpdateUser u ->
            let
                newModel =
                    resetFormPanel model model.ui.panelMode
            in
        ...
        SubmitNewUser u ->
            if isEmpty u.login then
                let
                    ...
                    newUserForm =
                        { userForm | isValidInput = InvalidUsername }
                in
                ( { model | userForm = newUserForm }, Cmd.none )
        ...
        PreHashedPasswd bool ->
            let
                ...
                newUserForm =
                    { userForm | password = "", isHashedPasswd = bool }
            in
            ( { model | userForm = newUserForm }, Cmd.none )
        ...
        AddPasswdAnyway ->
            if model.userForm.userForcePasswdInput then
                let
                    ...
                    newUserForm =
                        { userForm | password = "", userForcePasswdInput = False }
                in
                ( { model | userForm = newUserForm }, Cmd.none )
                ...
                let
                    ...
                    newUserForm =
                        { userForm | userForcePasswdInput = True }
                in
                ( { model | userForm = newUserForm }, Cmd.none )
        ...
        OpenDeleteModal username ->
            let
                ...
                newUI =
                    { ui | openDeleteModal = True }

                userForm =
                    model.userForm

                newUserForm =
                    { userForm | login = username }
            in
    ...
    let
        ...
        newFields =
            case panelMode of
                ...
                _ ->
                    { name = "", email = "", info = Dict.empty, otherInfo = Dict.empty }

        newUI =
            { panelMode = panelMode, openDeleteModal = False, tableFilters = model.ui.tableFilters }
