...
import Ui.Datatable exposing (Category, getAllElems)
    ...
    let
                        ...
                        [ Composition (Or include) (Or exclude) ] ->
                            let
                                ...
                                checkExistingTarget target =
                                    case target of
                                        NodeGroupId id ->
                                            isJust (find (\g -> g.id == id) groupsList)
        ...
        ( method, url ) =
            if creation then
                ...
                ( "POST", [ "rules", ruleDetails.id.value ] )
    ...
    let
        ( method, url ) =
            if creation then
                ...
                ( "POST", [ "rules", "categories", category.id ] )
