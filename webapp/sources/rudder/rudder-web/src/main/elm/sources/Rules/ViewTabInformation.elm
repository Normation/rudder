...
import Rules.ViewUtils exposing (buildListCategories, getRuleCompliance)
import Ui.Datatable exposing (Category, SortOrder(..), getSubElems)
    ...
    let
        ...
        getCategoryName cId =
            let
                ...
                findCategory =
                    List.Extra.find (\c -> c.id == cId) (concatCategories model.rulesTree)
            in
        ...
        allMissingCategoriesRulesId =
            List.map (\r -> r.id.value) (getAllMissingCatsRules model.rulesTree)

        originRuleMissingCatId =
        ...
        isMissingCatRule =
            List.member rule.id.value allMissingCategoriesRulesId
                ...
                Just settings ->
                    if settings.enableChangeRequest && not (List.isEmpty settings.pendingChangeRequests) then
                        div [ class "col-sm-12" ]
                            [ div
                                [ class
                                    ("callout-fade callout-info callout-cr"
                                        ++ (if settings.collapsePending then
                                                " list-hidden"
                                        ...
                                        (settings.pendingChangeRequests
                                            |> List.map
                                                (\cr ->
                                                    let
                                                        statusBadge =
                                                            span [ class "badge" ] [ text (toLabel cr.status) ]
