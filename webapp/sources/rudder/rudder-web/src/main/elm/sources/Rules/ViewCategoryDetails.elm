...
import Html exposing (Html, a, button, div, form, h1, i, input, label, li, p, select, span, table, tbody, text, textarea, thead, ul)
import Html.Attributes exposing (attribute, class, for, id, placeholder, style, type_, value)
...
import Rules.ViewUtils exposing (btnSave, buildListCategories, getListRules, rulesTableHeader)
import String
import Ui.Datatable exposing (getAllCats, getSubElems)
...
editionTemplateCat model details =
    let
        ...
        allMissingCategories =
            List.filter (\sub -> sub.id == missingCategoryId) (getSubElems model.rulesTree)

        listOfCat =
            List.concatMap getAllCats allMissingCategories

        listCatIdMissing =
            List.map (\r -> r.id) listOfCat

        writeRights =
            model.ui.hasWriteRights && (not (category.id == missingCategoryId) && not (List.member category.id listCatIdMissing))

        -- Logic to get all rule above a category, it take the sub categories rules too
        categoryInfos =
            List.filter (\c -> c.id == details.category.id) (getAllCats model.rulesTree)
            ...
            [ ul [ class "nav nav-underline" ]
                [ li [ class "nav-item" ]
                    [ button
                        ...
                        , onClick (UpdateCategoryForm { details | tab = Information })
                        ]
                        [ text "Information" ]
                    ]
                , li [ class "nav-item" ]
                    [ button
                        ...
                        , onClick (UpdateCategoryForm { details | tab = Rules })
                        ]
                        [ text "Rules"
