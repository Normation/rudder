...
type alias ParentGlobalProperty =
    { valueType : String }


type alias ParentGroupProperty =
    { id : String, name : String, valueType : String, parent : Maybe ParentProperty }


type alias ParentNodeProperty =
    { id : String, name : String, valueType : String, parent : Maybe ParentProperty }
...
getSearchResultLength result kind =
    case kind of
        Techniques ->
            result.techniques |> Maybe.map (\d -> d.header.numbers) |> Maybe.withDefault 0

        Directives ->
            result.directives |> Maybe.map (\d -> d.header.numbers) |> Maybe.withDefault 0
