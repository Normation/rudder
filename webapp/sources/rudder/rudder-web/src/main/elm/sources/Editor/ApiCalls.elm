...
checkTechnique mode model =
    let
        techniqueDecoder =
            Json.Decode.at [ "data", "techniques" ] (headList (Json.Decode.list decodeTechnique))

        yamlDecoder =
            Json.Decode.at [ "data", "techniques" ] (headList (Json.Decode.list (Json.Decode.at [ "output" ] Json.Decode.string)))

        ( input, output, ( expect, body ) ) =
            case mode of
                Import content ->
                    ( "yaml", "json", ( Detailed.expectJson (CheckOutJson mode) techniqueDecoder, stringBody "application/x-yml" content ) )
                ...
                CheckJson technique ->
                    ( "json", "yaml", ( Detailed.expectJson (CheckOutYaml mode) yamlDecoder, jsonBody (encodeTechnique technique) ) )
    ...
    let
        encoder =
            case internalId of
                Just value ->
                    if creation then
                        encodeNewTechnique technique value
        ...
        req =
            request
                ...
                , headers = [ header "X-Requested-With" "XMLHttpRequest" ]
                , url =
                    getUrl model "techniques"
                        ++ (if creation then
                                ""

                            else
                                "/" ++ technique.id.value ++ "/" ++ technique.version
                           )
...
getRessources state model =
    let
        url =
            case state of
                Edit t ->
                    "techniques/" ++ t.id.value ++ "/" ++ t.version ++ "/resources"

                Creation id ->
                    "drafts/" ++ id.value ++ "/" ++ "1.0/resources"

                Clone t _ id ->
                    "drafts/" ++ id.value ++ "/" ++ t.version ++ "/resources"
...
copyResourcesToDraft draftId technique optId model =
    let
        id =
            Maybe.withDefault technique.id optId

        url =
            getUrlNew model [ "drafts", draftId, technique.version, "resources", "clone" ] [ Url.Builder.string "techniqueId" id.value, Url.Builder.string "category" technique.category ]
