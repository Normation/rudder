...
class RuleGrid(
  ...
  private val getFullDirectiveLib      = () =>
    RudderConfig.roDirectiveRepository.getFullDirectiveLibrary()(using QueryContext.systemQC)
      ...
      (trackerVariables, targetsInfo) match {
        ...
        case (x, y) =>
          if (rule.isEnabledStatus) {
            ...
            (for {
              r <- roRuleRepository.get(rule.id)(using QueryContext.systemQC)
