...
class TechniqueLibraryManagement extends DispatchSnippet with Loggable {
  ...
  def userLibraryAction(): NodeSeq = {
    SHtml.ajaxButton(
      "Add category",
      () => showCreateActiveTechniqueCategoryPopup(),
      ("class", "btn btn-success btn-outline new-icon pull-right")
    )
  }
      ...
      (for {
        ...
        case (sourceactiveTechniqueId, destCatId) :: Nil =>
          (for {
            ...
            case Full(res) =>
              refreshTree() & JsRaw(
                """setTimeout(function() { $("[activeTechniqueId=%s]").effect("highlight", {}, 2000)}, 100)""".format(
                  sourceactiveTechniqueId
                )
        ...
        case (sourceCatId, destCatId) :: Nil =>
          (for {
            ...
            case Full(res) =>
              ...
              OnLoad(
                JsRaw(
                  """setTimeout(function() { $("[catid=%s]").effect("highlight", {}, 2000);}, 100)"""
                    .format(sourceCatId)
                )
              )
            case f: Failure => Alert(f.messageChain + "\nPlease reload the page")
