      ...
      updated <- bestEffort(techLib.allDirectives.values.toSeq) {
                   case (inActiveTechnique, directive) =>
                     techniqueIds.get(inActiveTechnique.techniqueName) match {
                       case Some(mods) =>
                         for {
                           paramEditor <- directiveEditorService.get(
                                            TechniqueId(inActiveTechnique.techniqueName, directive.techniqueVersion),
                                            directive.id.uid,
                           ...
                           logger.debug(
                             s"Technique ${inActiveTechnique.techniqueName.value} changed => saving directive '${directive.name}' [${directive.id.uid.value}]"
                           )
                         }
                       case None       =>
                         Full("Nothing to do")
