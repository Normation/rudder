...
class SearchNodeComponent(
  ...
  def buildQuery(isGroupsPage: Boolean)(implicit qc: QueryContext): NodeSeq = {
    ...
    def displayQueryLine(cl: CriterionLine, index: Int, addRemove: Boolean): NodeSeq = {
      ...
      val input           = inputNoClass.copy(attributes =
        inputNoClass.attributes.append(Attribute.apply(pre = null, key = "class", value = inputClass, next = Null))
      )
...
object SearchNodeComponent {
  ...
  def updateCompAndValue(
    ...
    val newForm        = newFormNoClass.copy(attributes =
      newFormNoClass.attributes.append(Attribute.apply(pre = null, key = "class", value = inputClass, next = Null))
    )
