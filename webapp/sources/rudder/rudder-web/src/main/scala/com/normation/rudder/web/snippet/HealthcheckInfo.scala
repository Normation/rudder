...
class HealthcheckInfo(
  ...
  def getLevel(checks: List[HealthcheckResult]): NotificationLevel = {
    checks.foldLeft(Info:NotificationLevel) { (previous, checkResult) => (previous, checkResult) match {
  ...
  def healthcheckInfo(html: NodeSeq) ={
    ...
    (for {
      ...
      val sorted = checks.filter(_ match {
        case _:Ok => false
        case _    => true
  ...
  def healthcheckInfoIcon(html: NodeSeq) = {
    def displayPluginIcon(iconClass: String, notifTitle: String): NodeSeq = {
      val tooltipContent = "<h4 class='"+ iconClass +"' > <i class='fa fa-exclamation-triangle'></i> "+ notifTitle +"</h4><div>More details on <b>Health Check</b> page</div>"
