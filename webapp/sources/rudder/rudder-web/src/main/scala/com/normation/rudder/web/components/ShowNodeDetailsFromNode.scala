...
class ShowNodeDetailsFromNode(
  ...
  private def saveNodeState(nodeId: NodeId)(nodeState: NodeState)(implicit qc: QueryContext): Box[NodeState] = {
    ...
    for {
      ...
      result  <- nodeFactRepo
                   .save(newNode)(ChangeContext(modId, qc.actor, DateTime.now(DateTimeZone.UTC), None, None, qc.nodePerms))
                   .toBox
    } yield {
  ...
  def saveSchedule(nodeFact: CoreNodeFact)(schedule: AgentRunInterval): Box[Unit] = {
    ...
    val modId       = ModificationId(uuidGen.newUuid)
    val cc          = ChangeContext(modId, CurrentUser.actor, DateTime.now(DateTimeZone.UTC), None, None, CurrentUser.nodePerms)
