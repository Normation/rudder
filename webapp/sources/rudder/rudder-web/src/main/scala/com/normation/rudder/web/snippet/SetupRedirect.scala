...
class SetupRedirect extends DispatchSnippet with Loggable {
  ...
  def dispatch = {
    case "display" => _ => Script(display())
  }

  def display() = {

    configService.rudder_setup_done().toBox match {
      case Full(false) if CurrentUser.checkRights(Administration.Write) && !CurrentReq.value.request.url.contains("administration/setup") =>
