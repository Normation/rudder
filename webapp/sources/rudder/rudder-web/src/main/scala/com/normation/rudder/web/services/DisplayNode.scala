...
object DisplayNode extends Loggable {
      ...
      case Some(agent) =>
        ...
        <div class="security-info" id={"security-" + node.main.id.value}>{
          ...
          s"$$('#${publicKeyId}').toggle(300); $$(this).toggleClass('opened'); return false;"
            ...
            <div id={publicKeyId} class="display-keys" style="display:none;">
              {
          if (CurrentUser.checkRights(AuthorizationType.Administration.Write) && node.main.keyStatus == CertifiedKey) {
            SHtml.ajaxButton(
              ...
              <pre><div>{agent.securityToken.key}</div></pre>{
  ...
  private def resetKeyStatus(node: NodeInventory, st: SecurityToken): JsCmd = {
    ...
    nodeRepo
      ...
      .map { _ =>
        val js: JsCmd =
          SetHtml(s"security-${node.main.id.value}", displaySecutiryInfo(node.modify(_.main.keyStatus).setTo(UndefinedKey)))
        js
      }
      .catchAll { err =>
        val js: JsCmd = JsRaw(
