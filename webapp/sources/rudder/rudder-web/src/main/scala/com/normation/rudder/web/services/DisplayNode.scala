  ...
  private def displaySecurityInfo(nodeFact: NodeFact)(implicit qc: QueryContext): NodeSeq = {
    ...
    val securityTokenStatus = (nodeFact.rudderSettings.status, nodeFact.rudderSettings.keyStatus) match {
      ...
      case (AcceptedInventory, UndefinedKey) =>
        <span>
          Undefined
          <span class="fa fa-exclamation-triangle text-warning" data-bs-toggle="tooltip" title="The certificate for this node has been reset, the next inventory will be trusted automatically"></span>
        </span>
    ...
    <div class="security-info" id={"security-" + nodeFact.id.value}>
      ...
      <div><label>Security token kind: </label><span> {tokenKind} </span></div>
      ...
      if (CurrentUser.checkRights(AuthorizationType.Node.Write) && nodeFact.rudderSettings.keyStatus == CertifiedKey) {
        <div>
            <p>
              ...
              <br/>
              You can <b>reset</b> the current token by clicking the button below :
              <br/>
            </p>
            ...
            <div class="row">
              <div class="col-xl-12">
                <h4 class="text-center">
                  Are you sure you want to reset this node's security token ?
                </h4>
                <p>The next inventory update will be trusted automatically, and subsequent inventory updates and reports will need to be signed with the same token in order to be accepted.</p>
              </div>
            </div>
          </div>
