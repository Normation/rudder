...
class NodeGroupForm(
  ...
  private def showRelatedRulesTree(target: RuleTarget): NodeSeq = {
    ...
    val (includingGroup, excludingGroup) = {
      ZIO
        .foreach(relatedRules)(r => ruleRepository.get(r)(using QueryContext.systemQC))
  ...
  private[components] def hasDynamicSubgroups(query: Option[Query])(using qc: QueryContext): IOResult[Option[String]] = {
    query match {
  ...
  private[components] def getDependingGroups(id: NodeGroupId, onlyStatic: Boolean)(using
      qc: QueryContext
