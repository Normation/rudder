...
class CategoryHierarchyDisplayer() {
  def getCategoriesHierarchy(
      ...
      exclude:      Option[FullNodeGroupCategory => Boolean]
    ...
    rootCategory.getSortedCategories((a, b) => a.name > b.name, ex).map {
      case (list, nodeGroupCategory) =>
        if (list.size <= 1) {
          (nodeGroupCategory.id, nodeGroupCategory.name)
        } else {
          (
            nodeGroupCategory.id,
            ("\u00a0\u00a0\u00a0\u00a0\u00a0" * (list.size - 2) + "\u2514" + "\u2500 " + nodeGroupCategory.name)
          )
        }
    }
  }

  def getRuleCategoryHierarchy(
      rootCategory: RuleCategory,
      exclude:      Option[RuleCategory => Boolean],
      size:         Int = 0
  ): List[(RuleCategoryId, String)] = {
    (if (size == 0) {
