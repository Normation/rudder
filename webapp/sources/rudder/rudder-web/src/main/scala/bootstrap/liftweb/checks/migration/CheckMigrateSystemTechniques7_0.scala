...
class CheckMigratedSystemTechniques(
  ...
  val migrateConfigs6_x_7_0  =
    new MigrateTechniques6_x_7_0(ldap, gitRepo, techniqueRepo, techniqueLibUpdater, uuidGen, directiveRepo, ruleRepo)

  def DN(child: String, parentDN: DN) = new DN(child + "," + parentDN.toString)

  override def description: String =
    "Check if migration of system configuration object and allowed networks to Rudder 7.0 format is needed"

  override def checks(): Unit = {
    ZIO
      .whenM(checkMigrationNeeded())(
        for {
      ...
      .catchAll(err => {
        MigrationLoggerPure.error(
          s"Error during migration of policy server system configuration to Rudder V7.0 data. It is likely that Rudder won't " +
   ...
   */
  def getPolicyServerIds: IOResult[List[NodeId]] = nodeInfoService
    .getAll()
    .map(_.collect {
...
object SystemConfig {
  ...
  val systemTechniqueBaseDn = new DN(
...
class MigrateAllowedNetworks6_x_7_0(
  ...
  def getAllowedNetworks6_2(policyServerId: NodeId): IOResult[Seq[String]] = {
    for {
      ...
      entry <-
        con
          .get(dir.dn, RudderLDAPConstants.A_DIRECTIVE_VARIABLES)
          .notOptional(
            s"Entry for directive '${dir.id}' was not found, can not migrate allowed network for '${dir.policyServerId.value}'."
          )
  ...
  def createNewAllowedNetworks(relayServers: List[NodeId]): IOResult[Unit] = {
    def getOldAllowedNetFor(nodeId: NodeId) = {
      getAllowedNetworks6_2(nodeId).map(ips => PolicyServer(nodeId, ips.map(ip => AllowedNetwork(ip, ip)).toList))
    }
...
class MigrateTechniques6_x_7_0(
  ...
  def testNewTechniquesLoaded(): IOResult[Unit] = {
    ...
    def touchTechniques() = {
      ...
      val touchAll = ZIO.foreach(PolicyServerConfigurationObjects.rootTechniques) { t =>
        ...
        IOResult.effect {
          ...
          s"Error when preparing reload of new system technique '${t}'. Please check that new system" +
      ...
      gitRepo.semaphore.withPermit(for {
        ...
        _ <- IOResult.effect(
               gitRepo.git
                 .commit()
                 .setMessage(s"Force reload of system technique v7.0")
                 .setCommitter(new PersonIdent("Rudder", "email not set"))
                 .call()
             )
      } yield ())
    }

    val reloadTechLib = techniqueLibUpdater
      .update(
    ...
    val checkActiveTechnique = PolicyServerConfigurationObjects.rootTechniques
      .accumulate(t => {
    ...
    touchTechniques() *>
    reloadTechLib *>
    checkActiveTechnique.chainError(
      s"Some system techniques needed to migration" +
  ...
  def createNewConfigsOne(nodeId: NodeId) = {
    ...
    for {
      ...
      _    <- ZIO.foreach(configs.directives.toList) {
                case (t, d) =>
                  directiveRepo.saveSystemDirective(
                    ActiveTechniqueId(t.value),
                    d,
                    modId,
                    RudderEventActor,
                    Some(s"Create system directive for rudder v7.0 for policy server '${nodeId.value}'")
                  )
              }
      ...
      _    <- ZIO.foreach(rules)(r => {
                ruleRepo.create(
                  r,
                  modId,
                  RudderEventActor,
                  Some(s"Update system rules for rudder v7.0 for policy server '${nodeId.value}'")
                )
              })
  ...
  def migrateServerRolesRuleTarget(): IOResult[Unit] = {
    ...
    def updateRuleWithTarget(oldTarget: String, newTarget: String) = {
      for {
        ...
        rules  <-
          con.search(
            SystemConfig.ruleBaseDn,
            One,
            BuildFilter.SUB(RudderLDAPConstants.A_RULE_TARGET, null, Array(oldTarget.getBytes(StandardCharsets.UTF_8)), null)
          )
        ...
        saved  <- ZIO.foreach(updated) { r =>
                    con
                      .save(r)
                      .catchAll(err => {
                        MigrationLoggerPure.warn(
                          s"Error when migrating target '${oldTarget}' to new target '${newTarget}' in " +
  ...
  def finalMoveAndCleanOne(nodeId: NodeId): IOResult[Unit] = {
    ...
    def rollback(
        ...
        oldSaved: Ref[List[SystemConfig.R]],
        migrated: Ref[List[SystemConfig.R]],
        err:      RudderError
    ): IOResult[Nothing] = {
      MigrationLoggerPure.error(
        s"Error when trying to migrate system technique, rollbacking to 6.2 configuration: ${err.fullMsg}"
      ) *>
      (for {
        toDelete   <- migrated.get
        _          <- ZIO.foreach(toDelete)(r => con.delete(r.dnWithPrefix(NEW_RULE_PREFIX)))
        toMoveBack <- oldSaved.get
        _          <- ZIO.foreach(toMoveBack)(r => con.move(r.dnWithPrefix(OLD_RULE_PREFIX), r.dn.getParent, Some(r.dn.getRDN)))
      } yield ()) *>
      Inconsistency(
        s"An error happened while migrating system technique to rudder 7.0. You " +
        s"will likely need to restart Rudder and report that error: ${err.fullMsg}"
      ).fail
    }
    ...
    val migrateOne = for {
      ...
      _ <- (ZIO
             .foreach(List(SystemConfig.v6_2.ruleHasPolicyServer(nodeId), SystemConfig.v6_2.ruleDistributePolicy(nodeId))) { r =>
      ...
      _ <- (ZIO
             .foreach(List(SystemConfig.v7_x.ruleHasPolicyServer(nodeId), SystemConfig.v7_x.rulePolicyServer(nodeId))) { r =>
