...
class CheckAddSpecialNodeGroupsDescription(
  ...
  val all_nodeGroupDN             = new DN(
    s"nodeGroupId=all-nodes-with-cfengine-agent,groupCategoryId=SystemGroups,groupCategoryId=GroupRoot,ou=Rudder,cn=rudder-configuration"
  )
  val all_nodeGroupPolicyServerDN = new DN(
    s"nodeGroupId=hasPolicyServer-root,groupCategoryId=SystemGroups,groupCategoryId=GroupRoot,ou=Rudder,cn=rudder-configuration"
  )
  ...
  val allNodeGroupPolicyServerNewDescription =
    "All Linux Nodes known by Rudder directly connected to the root server. This group exists only as internal purpose and should not be used to configure nodes."

  override def description: String =
    "Check if system groups hasPolicyServer-root and all-nodes-with-cfengine-agent have the correct description and name"
  ...
  def checkMigrationNeeded(): IOResult[Boolean] = {
    for {
      ...
      allLinux                       <- con
                                          .get(all_nodeGroupDN)
                                          .chainError(s"Error when trying to get entry for ${all_nodeGroupDN} to modified name and description")
      allLinuxPolicyServerModified   <-
        con
          .get(all_nodeGroupPolicyServerDN)
          .chainError(s"Error when trying to get entry for ${all_nodeGroupPolicyServerDN} to modified name and description")
      isAllLinuxModified             <-
        IOResult.effect(
          s"Error when trying to get attributes `${A_NAME}` and `${A_DESCRIPTION}` for ${all_nodeGroupDN.toString}"
        ) {
          allLinux match {
            case Some(entry) =>
      ...
      isAllLinuxPolicyServerModified <-
        IOResult.effect(
          s"Error when trying to get attributes `${A_NAME}` and `${A_DESCRIPTION}` for ${all_nodeGroupPolicyServerDN.toString}"
        ) {
  ...
  def createSpecialTarget(): IOResult[Unit] = {
    for {
      ...
      _   <- con.modify(
               all_nodeGroupPolicyServerDN,
               new Modification(ModificationType.REPLACE, A_NAME, allNodeGroupPolicyServerNewName)
             )
      _   <- con.modify(
               all_nodeGroupPolicyServerDN,
               new Modification(ModificationType.REPLACE, A_DESCRIPTION, allNodeGroupPolicyServerNewDescription)
             )
    } yield ()
  }
}
