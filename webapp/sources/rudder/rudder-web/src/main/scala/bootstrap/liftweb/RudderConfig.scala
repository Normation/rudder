...
object RudderConfigInit {
  ...
  def init(): RudderServiceApi = {
    // modularization of the method needed to avoid "platform restriction a parameter list's length cannot exceed 254"
    // see https://issues.rudder.io/issues/26416 and related issues
      ...
      lazy val eventLogApi         = {
        ...
        new EventLogAPI(
          new RestEventLogService(
            ...
            personIdentService,
            new ItemRollbackServiceImpl(gitModificationRepository, itemRollbackRepository)
          ),
          eventLogCoreService,
          eventLogDetailsGenerator
        )
