    ...
    case x         => // so, it should be a full path, check it
      ...
      if (config.exists && config.canRead) {
        ApplicationLogger.info(
          "Use configuration file defined by JVM property -D%s : %s".format(JVM_CONFIG_FILE_KEY, config.getPath)
        )
        FileSystemResource(config)
      } else {
        ApplicationLogger.error(
          "Can not find configuration file specified by JVM property %s: %s ; abort".format(JVM_CONFIG_FILE_KEY, config.getPath)
        )
...
object RudderConfig extends Loggable {
  ...
  val WATCHER_WAIT_FOR_SIG = {
    try {
      Duration.fromScala(
        scala.concurrent.duration.Duration.apply(config.getString("inventories.watcher.waitForSignatureDuration"))
      )
    } catch {
      case ex: Exception =>
        try {
  ...
  ZioRuntime.internal
    ...
    .foreach(err => throw new InitError("An error occured when testing for LDAP connection, please check it: " + err.fullMsg))
  ...
  val techniqueCompiler  =
    new RudderCRunner("/opt/rudder/etc/rudderc.conf", "/opt/rudder/bin/rudderc", RUDDER_GIT_ROOT_CONFIG_REPO)
    ...
    val maxParallel = {
      try {
        ...
        case ex: Exception =>
          ...
          println(
            s"ERROR Error when parsing configuration properties for the parallelisation of inventory processing. " +
