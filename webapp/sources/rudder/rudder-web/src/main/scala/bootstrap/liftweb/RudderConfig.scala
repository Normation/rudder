...
object RudderConfig extends Loggable {
  ...
  private[this] lazy val licenseRepository = new LicenseRepositoryXML("/opt/rudder/etc/cfengine-enterprise-licenses/licenses.xml")
  ...
  val executionService = {
    val maxCatchupTime = {
      val temp = RUDDER_REPORTS_EXECUTION_MAX_DAYS.day + RUDDER_REPORTS_EXECUTION_MAX_MINUTES.minutes
        if (temp.toMillis == 0) {
    ...
    val maxCatchupBatch = RUDDER_REPORTS_EXECUTION_MAX_SIZE.minutes
