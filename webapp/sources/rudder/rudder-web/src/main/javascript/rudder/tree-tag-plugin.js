...
$.jstree.plugins.searchtag = function (options, parent) {
  ...
  this.searchtag = function (str, tags, toggleEmptyTechniques, filteringTagsOptions, skip_async, show_only_matches, inside, append, show_only_matches_children) {
    this.show_all();
    if((!Array.isArray(tags) || tags.length<=0)&&(str === false || $.trim(str.toString()) === "")&&(toggleEmptyTechniques===true)) {
    ...
    $.each(m[inside ? inside : $.jstree.root].children_d, function (ii, i) {
      ...
      if(v.text && !v.state.hidden && (!s.search_leaves_only || (v.state.loaded && v.children.length === 0)) && (!s.search_callback && f.search(v.text).isMatch) ) {
        ...
        }else if (directiveTags && toggleEmptyTechniques===false){
          r.push(i);
          p = p.concat(v.parents);
        }
      }
