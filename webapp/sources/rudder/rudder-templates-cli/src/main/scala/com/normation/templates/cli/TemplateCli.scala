...
object TemplateCli {
  ...
  def readStdin(): IOResult[String] = {
    for {
      ...
      ready   <-
        if (in.ready) UIO.unit else Inconsistency("Can not get template content from stdin and no template file given").fail
  ...
  def fill(variables: Seq[STVariable], outDir: File, inputExtension: String, outputExtension: String, timer: FillTemplateTimer)(
    ...
    for {
      ok      <- if (template.getName.endsWith(inputExtension)) { UIO.unit }
                 else {
                   Inconsistency(s"Ignoring file ${template.getName} because it does not have extension '${inputExtension}'").fail
                 }
      content <- IOResult.effect(s"Error when reading variables from ${template.getAbsolutePath}")(
      ...
      writed  <- IOResult.effect(s"Error when writting filled template into ${out.getAbsolutePath}")(
...
object ParseVariables extends Loggable {
  ...
  def fromFile(file: File): IOResult[Set[STVariable]] = {
    for {
      jsonString <-
        IOResult.effect(s"Error when trying to read file ${file.getAbsolutePath}")(FileUtils.readFileToString(file, "UTF-8"))
