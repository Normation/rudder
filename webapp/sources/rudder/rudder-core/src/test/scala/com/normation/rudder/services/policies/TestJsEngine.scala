...
class TestJsEngine extends Specification {
  ...
  def contextEnabled[T](script: JsEngine => IOResult[T]):  Either[RudderError, T] =
    JsEngineProvider.withNewEngine[T](FeatureSwitch.Enabled, 1, 1.second)(script).either.runNow
  def contextDisabled[T](script: JsEngine => IOResult[T]): Either[RudderError, T] =
    JsEngineProvider.withNewEngine[T](FeatureSwitch.Disabled, 1, 1.second)(script).either.runNow
  ...
  "Getting the scripting engine" should {
    ...
    "let test have direct access to the engine" in {
      val res =
        JsEngine.SandboxedJsEngine.getJsEngine(1).flatMap(_.buildContext.use(_.eval("js", "1 + 1").asInt().succeed)).runNow
      res === 2
    }
  }
