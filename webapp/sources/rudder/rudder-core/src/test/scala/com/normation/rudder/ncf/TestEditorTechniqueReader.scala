...
class TestEditorTechniqueReader extends ZIOSpecDefault {
  ...
  private val gitTechniquesLength = 6

  override def spec: Spec[TestEnvironment & Scope, Any] = {
    suite("EditorTechniqueReader")(
      test("read success techniques")(
        techniqueReader.readTechniquesMetadataFile.map(t => {
          assertTrue(t.techniques.length == gitTechniquesLength, t.allErrors.isEmpty)
        })
      ) @@ TestAspect.before(initMethods),
      test("read parsing error")(
        techniqueReader.readTechniquesMetadataFile.map(t => {
          assertTrue(t.parsingErrors.length == 1, t.allErrors.length == 1, t.techniques.length == gitTechniquesLength)
        })
      ...
      test("read I/O error")(
        techniqueReader.readTechniquesMetadataFile.map(t => {
          assertTrue(
            t.errors.length == 1,
            t.parsingErrors.length == 1,
            t.allErrors.length == 2,
            t.techniques.length == gitTechniquesLength
          )
        })
