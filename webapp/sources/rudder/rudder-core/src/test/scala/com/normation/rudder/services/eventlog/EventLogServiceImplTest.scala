...
import com.normation.eventlog.{EventActor, EventLogRequest}
import com.normation.rudder.EventLogRepositoryMock
import com.normation.rudder.repository.EventLogRepository
import com.normation.rudder.tenants.{QueryContext, TenantAccessGrant}
...
class EventLogServiceImplTest extends Specification {
  ...
  "event log service" should {
    ...
    "apply some filter in the event log coming from the repository" in {
      ...
      val filter = Some(EventLogRequest(0, 10, id = Some(EventLogRequest.Id(123)), search = None, startDate = None, endDate = None, principal = None, order = None, typeFilter = None))
