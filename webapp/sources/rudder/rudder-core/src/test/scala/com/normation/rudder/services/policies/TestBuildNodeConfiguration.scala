...
class TestBuildNodeConfiguration extends Specification {
  ...
  def newNode(i: Int): NodeInfo =
    node1.copy(node = node1Node.copy(id = NodeId("node" + i), name = "node" + i), hostname = s"node$i.localhost")

  val allNodes:     Map[NodeId, NodeInfo]       = ((1 to 1000).map(newNode) :+ root).map(n => (n.id, n)).toMap
  // only one group with all nodes
  val group:        NodeGroup                   = NodeGroup(NodeGroupId(NodeGroupUid("allnodes")), "allnodes", "", Nil, None, false, allNodes.keySet, true)
  ...
  val allNodeModes:     Map[NodeId, NodeModeConfig]        = allNodes.map { case (id, _) => (id, defaultModesConfig) }
