...
class GitGCTest extends ZIOSpecDefault {
  ...
  val gc2       = new GitGC(mock.gitRepo, schedule, () => testMonitor)

  override def spec: Spec[TestEnvironment & Scope, Any] = suite("Test collision of GC") {
    test("A failed GC should not start again immediately, it must wait at least until the next second") {
      for {
        _  <- TestClock.setTime(Instant.parse("2026-02-25T01:00:00.000Z"))
        _  <- gc.prog.forkDaemon
        _  <- gc2.prog.forkDaemon
        _  <- TestClock.adjust(1.minute)
//        _  <- IOResult.attempt((mock.gitRepo.rootDirectory / ".git/gc.pid").createFileIfNotExists())
        c1 <- count.get
        //    _  <- TestClock.setTime(Instant.parse("2026-02-25T01:03:41.000Z"))
        _  <- TestClock.adjust(161.minutes)
//        t  <- Clock.currentDateTime
//        _  <- errors.effectUioUnit(println(s"**** ${t}"))
