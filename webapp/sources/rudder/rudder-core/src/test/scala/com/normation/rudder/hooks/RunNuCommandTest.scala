...
class RunNuCommandTest() extends Specification {
  ...
  "A command" should {
    ...
    "has only the environment variable explicitly defined" in {
      ...
      prog.provideManaged(ZioRuntime.environment).runNow must beMatching("return code=0\nPATH=.*\nfoo=bar\n".r)
    }
    ...
    "has only the 3 stdin, stdout, stderr file descriptors" in {
      ...
      prog.provideManaged(ZioRuntime.environment).runNow must beEqualTo((0, 4))
    }
    ...
    "can actually modify the file system" in {
      ...
      (prog.provideManaged(ZioRuntime.environment).runNow must beEqualTo(0)) and
