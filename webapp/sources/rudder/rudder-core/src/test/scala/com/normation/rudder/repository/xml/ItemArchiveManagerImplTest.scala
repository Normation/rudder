...
class ItemArchiveManagerRollbackItemTest extends ZIOSpecDefault {

  import ItemArchiveManagerRollbackItemTest.*
  import ItemArchiveManagerRollbackItemTest.given
  ...
  override def spec: Spec[Any, Any] = suite("rollbackOneItem")(
    suite("directive")(
      test("reverting an addition deletes the directive") {
        for {
          ...
          _      <- itemArchiveManager(parseActiveTechniqueLibrary = directiveArchive(directiveUid))
    ...
    suite("node group")(
      test("reverting an addition deletes the group") {
        for {
          ...
          _      <- itemArchiveManager(parseGroupLibrary = groupArchive(groupId))
    ...
    suite("global parameter")(
      test("reverting an addition deletes the parameter") {
        for {
          ...
          _      <- itemArchiveManager(parseGlobalParameters = parameterArchive(parameterName))
    ...
    suite("rule")(
      test("reverting an addition deletes the rule") {
        for {
          ...
          _      <- itemArchiveManager(parseRules = ruleArchive(ruleId))
    ...
    suite("unsupported events")(
      test("an event type that can not be rolled back item by item is ignored") {
        ...
        itemArchiveManager().rollbackOneItem(commitId, event).as(assertCompletes)
      },
      test("details without an id fail with an explicit error") {
        for {
          res <- itemArchiveManager()
                   .rollbackOneItem(
                     commitId,
      ...
      test("restoring an item absent from the archive fails with an error") {
        for {
          _   <- givenNoDirective
          res <- itemArchiveManager(parseActiveTechniqueLibrary = emptyDirectiveArchive)
                   .rollbackOneItem(commitId, ModifyDirective(eventDetails(directiveXml(directiveUid))))
                   .either
        } yield assertTrue(res.left.exists(_.fullMsg.contains("was not found in the archive")))
      }
...
private object ItemArchiveManagerRollbackItemTest {
  ...
  def editorTechniqueManager(writer: TechniqueWriter): ItemArchiveManagerImpl = itemArchiveManager(
  ...
  /*
   * Helper constructor to avoid the null pollution in tests: ItemArchiveManager needs
   * the configuration repos.
   * What the rollback never touches is left null (a lot).
   */
  def itemArchiveManager(
  ...
  ): ItemArchiveManagerImpl = new ItemArchiveManagerImpl(
    roRuleRepository = ruleRepo,
    woRuleRepository = ruleRepo,
    roRuleCategoryeRepository = null,
    roDirectiveRepository = directiveRepo,
    ...
    gitRepo = gitRepo,
    gitRuleArchiver = null,
    gitRuleCategoryArchiver = null,
    gitActiveTechniqueCategoryArchiver = null,
    gitActiveTechniqueArchiver = null,
    gitNodeGroupArchiver = null,
    gitParameterArchiver = null,
    parseRules = parseRules,
    parseActiveTechniqueLibrary = parseActiveTechniqueLibrary,
    parseGlobalParameters = parseGlobalParameters,
    parseRuleCategories = null,
    importTechniqueLibrary = null,
    parseGroupLibrary = parseGroupLibrary,
    importGroupLibrary = null,
    importRuleCategoryLibrary = null,
    eventLogger = null,
    asyncDeploymentAgent = null,
    gitModificationRepo = null,
    updateDynamicGroups = null,
