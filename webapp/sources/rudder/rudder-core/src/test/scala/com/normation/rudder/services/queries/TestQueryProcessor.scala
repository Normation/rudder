...
class TestQueryProcessor extends Loggable {
  ...
  @Test def basicQueriesOnOneNodeParameter(): Unit = {
    ...
    val q2_0_ = TestQuery("q2_0_", query = q2_0.query match { case q : OldQuery => q.copy(composition = Or); case q : NewQuery => q.copy(composition = Or)}, q2_0.awaited)
    ...
    val q2_1_ = TestQuery("q2_1_", query = q2_1.query match { case q : OldQuery => q.copy(composition = Or); case q : NewQuery => q.copy(composition = Or)}, q2_1.awaited)
    ...
    val q2_2_ = TestQuery("q2_2_", query = q2_2.query match { case q : OldQuery => q.copy(composition = Or); case q : NewQuery => q.copy(composition = Or)}, q2_2.awaited)
  ...
  @Test def regexQueries(): Unit = {
    ...
    val q1_ = TestQuery("q1_", query = q1.query match { case q : OldQuery => q.copy(composition = Or); case q : NewQuery => q.copy(composition = Or)}, s(0) :: s(1) :: Nil)
    ...
    val q2_ = TestQuery("q2_", query = q2.query match { case q : OldQuery => q.copy(composition = Or); case q : NewQuery => q.copy(composition = Or)},
        (s(0) :: s(1) :: s(7) :: //nodeId
        s(2) :: s(7) :: //software
...
//      s.filterNot(n => n == s(2)) )
  ...
  @Test def nodePropertiesFailingReq(): Unit = {
    ...
    results.foreach { r =>
      assertTrue(s"Regex OldQuery with wrong data for node properties should fail: ${r._1}", r._2.isInstanceOf[Failure])
    }
  }
