...
final case class MigrationTestLog(
  ...
  def insertSql(c: Connection): Long = {
    ...
    val INSERT_SQL =
      "insert into EventLog (creationDate, principal, eventType, severity, data%s) values (?, ?, ?, ?, ?)".format(row, qmark)
    ...
    cause.foreach(id => ps.setInt(6, id))
    ps.executeUpdate
