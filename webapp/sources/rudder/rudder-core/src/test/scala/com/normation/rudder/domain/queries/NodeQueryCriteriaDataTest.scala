...
class NodeQueryCriteriaDataTest extends Specification {
  ...
  "Date comparator" should {
    val matcher  = NodeCriterionMatcherDate((n: CoreNodeFact) => Chunk.from(n.lastInventoryDate))
    ...
    "match with equality" in {
      "on date only" in {
        matcher.matches(n, Equals, dateOnly(date)).runNow === true
        matcher.matches(n, Equals, dateOnly(date.plusDays(2))).runNow === false
      }

      "on datetime" in {
        matcher.matches(n, Equals, dateTime(date)).runNow === true
        matcher.matches(n, Equals, dateTime(date.plusDays(2))).runNow === false
      }
    }

    "match with inequality" in {
      "on date only" in {
        matcher.matches(n, NotEquals, dateOnly(date)).runNow === false
        matcher.matches(n, NotEquals, dateOnly(date.plusDays(2))).runNow === true
      }

      "on datetime" in {
        matcher.matches(n, NotEquals, dateTime(date)).runNow === false
        matcher.matches(n, NotEquals, dateTime(date.plusDays(2))).runNow === true
      }
    }

    "match with lt" in {
      "on date only" in {
        matcher.matches(n, LesserEq, dateOnly(date.plusDays(2))).runNow === true
        matcher.matches(n, LesserEq, dateOnly(date)).runNow === true
        matcher.matches(n, LesserEq, dateOnly(date.minusDays(2))).runNow === false
      }

      "on datetime" in {
        matcher.matches(n, LesserEq, dateTime(date.plusDays(2))).runNow === true
        matcher.matches(n, LesserEq, dateTime(date)).runNow === true
        matcher.matches(n, LesserEq, dateTime(date.minusDays(2))).runNow === false
      }
    }
  }
}
