...
class GenericPropertiesTest extends Specification with Loggable with BoxSpecMatcher {
  ...
  "parsing" should {
    ...
    "correctly accept comments in json-like, wherever they are" in {
      val s = """//here, a comment
          ...
          | # even on new lines
          |""".stripMargin
      (firstNonCommentChar(s, InheritMode.Default) must_=== ((Some('{'), s, InheritMode.Default))) and
  ...
  "serialization / deserialisation" should {
    val check = (s: String) => GenericProperty.parseValue(s).map(x => GenericProperty.serializeToHocon(x._1)) must beRight(s)
    ...
    val checkPrimitive = (s: AnyVal) => GenericProperty.parseValue(s.toString).map(x => GenericProperty.serializeToHocon(x._1)) must beRight(s.toString)
    ...
    "hocon does not keep comment out of value and remove/reorder spaces/comments" in {
      ...
      val t = """{# comments!
        |"a":"b"}""".stripMargin
       GenericProperty.parseValue(s).map(x => GenericProperty.serializeToHocon(x._1)) must beRight(t)
    }
  }
