...
class PolicyWriterServiceImpl(
      ...
      .foldM(
        ...
        suc => {
          suc match {
            case None      => // timeout
              Accumulated(
                NonEmptyList.one(Unexpected(s"Execution of computation timed out after '${timeout.asJava.toString}'"))
              ).fail
            case Some(seq) =>
              seq.succeed
          }
  ...
  def writeTemplatePure(
    ...
    val logNodeConfigurations = ZIO.when(logNodeConfig.isDebugEnabled) {
      IOResult
        .effect(logNodeConfig.log(interestingNodeConfigs))
        .foldM(
          err =>
    ...
    for {
      ...
      _                    <- for {
                                pair                          <- for {
                                                                   ...
                                                                   preparedTemplates     <- parrallelSequence(configAndPaths) {
                                                                     ...
                                                                     timingLogger.debug(s"Policy templates prepared in ${preparedTemplatesTime - readTemplateTime2} ms") *> {
                                                                       ZIO.when(timingLogger.logEffect.isTraceEnabled) {
                                                                         (prepareTimer.buildBundleSeq.get <*> prepareTimer.buildAgentVars.get <*> prepareTimer.prepareTemplate.get)
                                                                           .flatMap(t => {
                                                                             timingLogger.trace(
                                                                               s" -> bundle sequence built in ${t._1._1} ms | agent system variables built in ${t._1._2} ms | policy template prepared in ${t._2} ms"
                                                                             )
                                                                           })
                                                                       }
                                                                     }
      ...
      propertiesWritten    <- parrallelSequence(configAndPaths) {
                                case agentNodeConfig =>
      ...
      parametersWritten <- parrallelSequence(configAndPaths) {
                             case agentNodeConfig =>
  ...
  private[this] def writePolicies(
    ...
    parrallelSequence(byTemplate.toSeq) {
      case (content, seqInfos) =>
        for {
          t0     <- currentTimeNanos
          parsed <-
            IOResult.effect(s"Error when trying to parse template '${seqInfos.head.destination}'") { // head ok because of groupBy
  ...
  private[this] def writeOtherResources(
    ...
    parrallelSequence(preparedTemplates) { prepared =>
      ...
      val writeResources = ZIO.foreach_(prepared.preparedTechniques) { preparedTechnique =>
        ZIO.foreach_(preparedTechnique.filesToCopy) { file =>
          for {
            t0 <- currentTimeNanos
            r  <- copyResourceFile(
                    file,
      ...
      val writeAgent = for {
        ...
        t0 <- currentTimeNanos
        _  <- IOResult
                .effect(writeAllAgentSpecificFiles.write(prepared))
  ...
  private[this] def readTemplateFromFileSystem(
    ...
    for {
      ...
      res <-
        (parrallelSequence(templatesToRead) {
                  ...
                  optInputStream match {
                    ...
                    case Some(inputStream) =>
                      for {
                        ...
                        content <- IOResult.effect(s"Error when copying technique template '${templateId.displayPath}'")(
  ...
  private[this] def readResourcesFromFileSystem(
    ...
    for {
      t0  <- currentTimeMillis
      res <-
        (parrallelSequence(staticResourceToRead) {
          case ToRead(templateId, agentType, templateOutPath) =>
            for {
  ...
  private[this] def movePromisesToFinalPosition(folders: Seq[NodePoliciesPaths], maxParallelism: Int): IOResult[Unit] = {
    ...
    if (sortedFolder.isEmpty) {
      ...
      for {
        ...
        _                      <-
          (ZIO
            .foreachParN_(maxParallelism)(sortedFolder) {
