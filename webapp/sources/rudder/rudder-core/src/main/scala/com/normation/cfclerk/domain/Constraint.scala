...
final case class IntegerVType(regex: Option[RegexConstraint] = None) extends VTypeConstraint with VTypeWithRegex {
  ...
  override def getFormatedValidated(value: String, forField: String, escapeString: String => String): PureResult[String] = {
    super
      .getFormatedValidated(value, forField, escapeString)
      .flatMap(escaped => {
        VTypeConstraint
          .checkIsProperty(value, forField) { v =>
            try {
              v.toInt
              Right(())
            } catch {
              case _: NumberFormatException =>
                Left(Inconsistency(s"Wrong value ${v} for field '${forField}': expecting an integer."))
            }
          }
          .map(_ => escaped)
      })
        ...
        VTypeConstraint
          .checkIsProperty(value, forField) { v =>
            try {
              ...
              case _: Exception =>
                Left(Inconsistency(s"Wrong value ${v} for field '${forField}': expecting a datetime in ISO 8601 standard."))
            }
          }
          .map(_ => escaped)
      })
...
final case class DateVType(regex: Option[RegexConstraint] = None)     extends VTypeConstraint with VTypeWithRegex {
  override val name = "date"
}
final case class TimeVType(regex: Option[RegexConstraint] = None)     extends VTypeConstraint with VTypeWithRegex {
...
sealed trait AbstactPassword extends VTypeConstraint with STString {
  final override def getFormatedValidated(value: String, forField: String, escapeString: String => String): PureResult[String] = {
...
case object AixDerivedPasswordVType   extends DerivedPasswordVType {
  override val tpe: HashAlgoConstraint.DerivedPasswordType = HashAlgoConstraint.DerivedPasswordType.AIX
}
case object LinuxDerivedPasswordVType extends DerivedPasswordVType {
  override val tpe: HashAlgoConstraint.DerivedPasswordType = HashAlgoConstraint.DerivedPasswordType.Linux
}
