...
trait SecretEventLogService {
  ...
  def saveModifySecretLog(modId: ModificationId, principal: EventActor, oldSec: Secret, newSec: Secret, reason: Option[String]): Box[EventLog]

  def saveDeleteSecretLog(modId: ModificationId, principal: EventActor, oldSec: Secret, newSec: Secret, reason: Option[String]): Box[EventLog]
}
