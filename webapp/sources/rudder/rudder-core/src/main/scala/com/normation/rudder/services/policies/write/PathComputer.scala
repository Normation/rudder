...
class PathComputerImpl(
   ...
   *
   */
  private def recurseComputePath(
      fromNodeId:    NodeId,
      toNodeId:      NodeId,
      path:          String,
    ...
    if (fromNodeId == toNodeId) {
      ...
      if (chain.size >= chainDepthLimit) {
        Left(
          Unexpected(
            s"We reach the maximum number of relay depth (${chainDepthLimit}), which is likely to denote a cycle in the chain of policy parents. " +
          ...
          result <- parent match {
                      ...
                      case policyParent =>
                        recurseComputePath(
                          fromNodeId,
                          policyParent.id,
                          policyParent.id.value + "/" + relativeShareFolder + "/" + path,
                          allNodeConfig,
                          policyParent.id :: chain
                        )
                    }
        } yield {
          result
