...
trait NodeFactRepository {
   ...
   */
  def setState(nodeId: NodeId, state: NodeState)(implicit cc: ChangeContext): IOResult[NodeFactChangeEventCC]
...
//  } yield ()).runNow
  ...
  override def setState(nodeId: NodeId, state: NodeState)(implicit cc: ChangeContext): IOResult[NodeFactChangeEventCC] = {
    get(nodeId)(using cc.toQuery)
