...
trait RudderJsonDecoders {
  ...
  implicit val tagsDecoder:       JsonDecoder[Tags]               = JsonDecoder[List[Map[String, String]]].map { list =>
    val res = list.flatMap(_.map { case (k, v) => com.normation.rudder.domain.policies.Tag(TagName(k), TagValue(v)) })
    Tags(res.toSet)
  }
  // PolicyMode
  ...
  implicit val ruddercTargetDecoder:  JsonDecoder[RuddercTarget]    = JsonDecoder[String].mapOrFail(s => RuddercTarget.parse(s))
  implicit val ruddercTargetsDecoder: JsonDecoder[JQRuddercTargets] = JsonDecoder[List[String]].mapOrFail(list => {
    list
      .accumulatePure(s => ruddercTargetDecoder.decodeJson(s).left.map(Inconsistency))
      .left
      .map(_.fullMsg)
      .map(x => JQRuddercTargets(x.toSet))
  })
}
