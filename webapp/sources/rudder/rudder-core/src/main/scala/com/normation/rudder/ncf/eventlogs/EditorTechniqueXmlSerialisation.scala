...
import com.normation.rudder.ncf.{Constraints, EditorTechnique, MethodBlock, MethodCall, MethodElem, ParameterId, ResourceFile, SelectOption, TechniqueParameter}
...
class EditorTechniqueXmlSerialisationImpl(xmlVersion: String) extends EditorTechniqueXmlSerialisation {
  ...
  def serialiseDiff(
    ...
    createTrimedElem(XML_TAG_EDITOR_TECHNIQUE, xmlVersion, "modify")(
      ...
      modifyDiff.modVersion.map(x => SimpleDiff.toXml[Version](<version/>, x)(v => Text(v.value))).getOrElse(NodeSeq.Empty) ++
      ...
      modifyDiff.modResources.map(x => SimpleDiff.toXml(<resources/>, x)(resources => resources.map(serialiseResource))).getOrElse(NodeSeq.Empty) ++
  ...
  def serialiseConstraintsDiff(
    ...
    createTrimedElem("constraints", xmlVersion)(
      ...
      modifyDiff.modSelect.map(x => {SimpleDiff.optionToXml(<select/>, x)(options => options.map(serialiseSelectOption))}).getOrElse(NodeSeq.Empty)
    )
  }

  def serialiseMethodElemDiff(
                              diff: TechniqueBlockDiff | TechniqueCallDiff
                            ): Elem = {
    diff match {
        ...
        createTrimedElem("block", xmlVersion, "modify")(
          <id>
            ...
            Nil ++
              ...
              modifyDiff.modeReportingLogic.map(x => SimpleDiff.toXml(<reportingLogic/>, x)(rl => Text(rl.value))).getOrElse(NodeSeq.Empty) ++
              modifyDiff.modPolicyMode.map(x => SimpleDiff.toXml(<policyMode/>, x)(rl => Text(rl.map(_.name).getOrElse(PolicyMode.defaultValue)))).getOrElse(NodeSeq.Empty) ++
              (if (modifyDiff.modCalls.isEmpty) NodeSeq.Empty else <calls>{modifyDiff.modCalls.map(serialiseMethodElemDiff) }</calls>) ++
        ...
        createTrimedElem("call", xmlVersion, "modify")(
          <id>
            ...
            Nil ++
              ...
              modifyDiff.modPolicyMode.map(x => SimpleDiff.toXml(<policyMode/>, x)(rl => Text(rl.map(_.name).getOrElse(PolicyMode.defaultValue)))).getOrElse(NodeSeq.Empty) ++
              modifyDiff.modParameters.map(x => SimpleDiff.toXml(<parameters/>, x)(_.toList.map(serializeMethodParameter.tupled))).getOrElse(NodeSeq.Empty) ++
              modifyDiff.modForeachName.map(x => SimpleDiff.optionToXml(<foreachName/>, x)(Text(_))).getOrElse(NodeSeq.Empty) ++
              modifyDiff.modForeach.map(x => SimpleDiff.optionToXml(<foreach/>, x)(serializeForeach)).getOrElse(NodeSeq.Empty)
        )
    }
