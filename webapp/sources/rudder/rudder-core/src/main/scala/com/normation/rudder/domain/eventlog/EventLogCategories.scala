...
object EventTypeFactory {
  val extraEventTypeMap: Map["NodeAgentRunPeriodModified" | "NodePropertiesModified" | "NodeHeartbeatModified", EventLogType] = {
    Map(
      // See ModifyNodeEventType
      "NodeAgentRunPeriodModified" -> ModifyNodeEventType,
      "NodePropertiesModified"     -> ModifyNodeEventType,
      "NodeHeartbeatModified"      -> ModifyNodeEventType
    )
  }
  ...
  private val eventTypesMap: Map[String, EventLogType] = extraEventTypeMap.toMap ++ eventTypes.map(e => e.serialize -> e).toMap
...
object EventLogTypeTranslate {
  private val extraNames: Map[String, String] = EventTypeFactory.extraEventTypeMap.map {
    ...
    case (s: "NodeHeartbeatModified", _)      => s -> "Node heartbeat modified"
  }
