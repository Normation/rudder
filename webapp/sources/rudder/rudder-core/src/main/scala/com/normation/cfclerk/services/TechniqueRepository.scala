  ...
  final def getTechniqueCategoriesBreadCrump(id: TechniqueId): IOResult[Seq[TechniqueCategory]] = {
    for {
      ...
      path <- zio.ZIO.foreach(cat.id.getIdPathFromRoot) { currentCatId =>
                getTechniqueCategory(currentCatId).chainError(
                  s"'${currentCatId.toString}' category was not found but should be a parent of '${cat.id.name.value}'. This is likely an error, please report it."
                )
              }
    } yield {
      path
