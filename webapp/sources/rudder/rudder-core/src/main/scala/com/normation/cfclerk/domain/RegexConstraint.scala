    ...
    if (compiled.matcher(varValue).matches) {
      ...
      PropertyParser.parse(varValue) match {
        case Left(err)     => // most likely the user wanted to use a property but made a typo
          Left(
            Inconsistency(s"Please modify ${varName}: error while parsing property: ${varValue}")
          ) // parser error are not hyper user friendly
        case Right(tokens) => // check that we don't have a pure string
          if (PropertyParserTokens.containsVariable(tokens)) {
            ...
            Left(
              Inconsistency(
                s"Please modify ${varName} to match the requested format ${if (errorMsg != "") " : " + errorMsg else ""}"
              )
            )
          }
      }
...
object MailRegex
    extends RegexConstraint(RegexConstraint.mail, "invalid format for the given email (valid example : pat@example.com)")

object Ipv4Regex extends RegexConstraint(
      RegexConstraint.ipv4,
      "invalid format for the given IPv4 (it should be xxx.xxx.xxx.xxx, where xxx is a number from 0-255)"
    )
object Ipv6Regex extends RegexConstraint(
      RegexConstraint.ipv6,
      "invalid format for the given IPv6 (typically looks like 2001:db8::1:0:0:1, " +
      "see https://tools.ietf.org/html/rfc5952 for more information)"
    )
object IpRegex   extends RegexConstraint(
      RegexConstraint.ipv4or6,
      "invalid format for the given IP v4 or v6 (IP v4: xxx.xxx.xxx.xxx, where xxx is a number from 0-255); " +
