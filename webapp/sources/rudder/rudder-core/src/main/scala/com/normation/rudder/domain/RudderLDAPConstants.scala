...
object RudderLDAPConstants extends Loggable {
  ...
  OC.createObjectClass(OC_RUDDER_NODE,
  ...
  OC.createObjectClass(OC_POLICY_SERVER_NODE, sup = OC(OC_RUDDER_NODE),
  ...
  OC.createObjectClass(OC_DIRECTIVE,
  ...
  OC.createObjectClass(OC_ACTIVE_TECHNIQUE_LIB_VERSION,
    may = Set(A_INIT_DATETIME))

  OC.createObjectClass(OC_API_ACCOUNT
