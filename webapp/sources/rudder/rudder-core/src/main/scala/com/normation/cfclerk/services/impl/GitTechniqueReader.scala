...
class GitTechniqueReader(
    ...
    (for {
      ...
      mods   <- if (nextId == cached._1) modifiedTechniquesCache.get
                  ...
                  for {
                    ...
                    diffPathEntries    <- managedDiffFmt.use(diffFmt => {
                                            IOResult.effect {
                                              diffFmt
  ...
  private[this] def extractMaybeCategory(
    ...
    def parse(db: Repository, parseDesc: Boolean, catId: TechniqueCategoryId): IOResult[(String, String, Boolean)] = {
      if (parseDesc) {
        val managedStream =
          ZManaged.make(IOResult.effect(db.open(descriptorObjectId).openStream))(is => effectUioUnit(is.close()))
        for {
  ...
  private[this] def loadDescriptorFile(
      managedStream: ZManaged[Any, RudderError, InputStream],
      filePath:      String
  ): IOResult[Elem] = {
