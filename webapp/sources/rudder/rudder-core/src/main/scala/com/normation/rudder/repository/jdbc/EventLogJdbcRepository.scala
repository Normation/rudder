...
class EventLogJdbcRepository(
  ...
  def getEventLogCount(
    ...
    val q    = sql"SELECT count(*) ${from} where " ++ principal.getOrElse(excludeRudderPrincipal) ++ criteria.getOrElse(
      Fragment.const(" 1 = 1 ")
    )
  ...
  def getEventLogByCriteria(
    ...
    val where = criteria match {
      case Some(value) => fr" where " ++ principal.getOrElse(excludeRudderPrincipal) ++ value
  ...
  /**
   * The default behavior is to exclude logs generated by the application, i.e. logs where actors = 'rudder'.
   */
  val excludeRudderPrincipal = fr" principal != 'rudder' and "
