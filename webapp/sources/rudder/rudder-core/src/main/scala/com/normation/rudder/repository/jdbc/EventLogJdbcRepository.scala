...
import com.normation.rudder.domain.eventlog.criteria.EventLogCriteriaFilter.Direction.Desc
...
class EventLogJdbcRepository(
  ...
  def getEventLogByCriteria(filter: Option[EventLogCriteriaFilter]): IOResult[Seq[EventLog]] = {
...
object EventLogJdbcRepository {
  ...
  private def filterToFromAndWhere(filter: Option[EventLogCriteriaFilter]) = {
    ...
    val search = filter.flatMap(f => f.search).flatMap(_.toFragment)
  ...
  def getEventLogCountSQL(filter: Option[EventLogCriteriaFilter]): Query0[Long] = {
  ...
  def getEventLogByCriteriaSQL(filter: Option[EventLogCriteriaFilter]): Query0[(String, EventLogDetails)] = {
    val order   = filter.flatMap(_.order.map(_.toFragment))
    val orderBy = order
      .map(fr"ORDER BY" ++ _)
  ...
  implicit class OrderOps(order: EventLogCriteriaFilter.Order) {
  ...
  implicit class SearchOps(search: EventLogCriteriaFilter.Search) {
