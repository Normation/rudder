...
trait EventLogRepository {
  ...
  // Tenant filtering: these user-facing reads take a QueryContext and only return events the actor's
  // tenant grant can see (event tag `None`/NULL => admin-only, fail closed). See the multi-tenants
  // object-tenant-tag-lifecycle ADR. The deprecated Fragment-based reader above is for system/deployment
  // reads that run with a full (system) context and is intentionally left unfiltered.
  ...
  // CR-scoped reads: gated by change-request visibility (change-validation), not by a per-event tenant tag.
