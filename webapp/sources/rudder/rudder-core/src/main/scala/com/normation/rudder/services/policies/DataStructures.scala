...
/*
 * A bundle exec condition, translated into an `ifvarclass`
 */
opaque type IfVarClass = String

object IfVarClass {
  // ifvarclass uses the same escape as BundleName
  def apply(s: String): IfVarClass = BundleName.escape(s)

  extension (x: IfVarClass) {
    def value: String = x
  }
