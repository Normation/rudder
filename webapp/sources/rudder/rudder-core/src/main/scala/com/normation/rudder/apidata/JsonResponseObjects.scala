...
object JsonResponseObjects {
  ...
  object JRPropertyHierarchyStatus {
    def fromInherited(
      ...
      val parentProperties = inheritedPropertyStatus.property.hierarchy
    ...
    def fromInheritedPropertyStatus(
      ...
      inheritedPropertyStatus match {
        case s: SuccessInheritedPropertyStatus =>
          JRProperty(
            propertyId,
            s.property.prop.value,
            s.nonEmptyDescription,
            s.property.prop.inheritMode,
            s.property.prop.provider,
            parents,
  ...
  sealed trait InheritedPropertyStatus extends PropertyStatus {
    def property:   NodePropertyHierarchy
    def propertyId: JRPropertyId = property.prop.name.transformInto[JRPropertyId]

    def inheritMode: Option[InheritMode] = property.prop.inheritMode

    def description:           String                   = property.prop.description
    def provider:              Option[PropertyProvider] = property.prop.provider
  ...
  final case class SuccessInheritedPropertyStatus private[apidata] (
      property: NodePropertyHierarchy
     ...
     */
    def mainHierarchy: ParentProperty[?] = property.hierarchy
  }
  ...
  sealed abstract class ErrorInheritedPropertyStatus(
      override val property:              NodePropertyHierarchy,
  ...
  object InheritedPropertyStatus {
     ...
     */
    def fromChildren(parent: NodePropertyHierarchy, children: Option[ParentProperty[?]]): InheritedPropertyStatus = {
      children match {
        case None                 =>
          SuccessInheritedPropertyStatus(parent)
        case Some(childHierarchy) =>
          val hasConflicts = MergeNodeProperties
            .checkValueTypes(List(NodePropertyHierarchy(childHierarchy)))
            .isLeft
