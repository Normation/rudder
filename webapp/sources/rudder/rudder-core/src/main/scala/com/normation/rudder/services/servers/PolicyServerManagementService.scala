...
object RelaySynchronizationMethod       extends Enum[RelaySynchronizationMethod] {
  ...
  def parse(value: String): PureResult[RelaySynchronizationMethod] = {
    value match {
      case null | "" => Left(Unexpected("An empty or null string can not be parsed as a relay synchronization method"))
      case s         =>
...
object PolicyServerConfigurationObjects {
  ...
  def directiveServerCommon(nodeId: NodeId): (TechniqueName, Directive) = {
