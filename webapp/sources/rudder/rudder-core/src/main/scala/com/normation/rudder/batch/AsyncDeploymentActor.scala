      ...
      if (techniqueStatus != status) {
        ...
        status match {
          case err: EditorTechniqueStatus.Errors =>
            StatusLoggerPure.Techniques.logEffect.warn(s"Status of technique compilation has ${err.size} errors")
          case EditorTechniqueStatus.AllSuccess =>
            StatusLoggerPure.Techniques.logEffect.debug(
              s"Status of technique compilation has changed to success from previous : ${techniqueStatus}"
            )
        }
        updateListeners()
      }
