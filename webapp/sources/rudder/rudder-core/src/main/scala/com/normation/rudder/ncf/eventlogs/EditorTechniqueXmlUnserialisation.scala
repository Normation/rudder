...
import com.normation.errors.{AccumulateErrors, BoxToEither, Chained, PureResult}
...
class EditorTechniqueXmlUnserialisationImpl extends EditorTechniqueXmlUnserialisation {
  ...
  def unserialiseForeach(entry : Node): PureResult[List[Map[String, String]]] = {
      (entry \ "iterator").accumulatePure(it =>
  ...
  def unserialiseBlock(entry : Node) : PureResult[MethodBlock] = {
    ...
    for {
      ...
      // category <- getAndTransformChild(entry, "category", _.text)
  ...
  def unserialiseDiff (entry : Node)  : Box[ModifyEditorTechniqueDiff] =  {
    ...
    for {
      ...
      version <- getFromTo[Version]((editorTechnique \ "version").headOption, x => Full(Version(x.text)))
      ...
      resources <- getFromTo[Seq[ResourceFile]]((editorTechnique \ "resources").headOption, x => (x \ "resource").accumulatePure(unserialiseResource).toBox)
      tags <- getFromTo[Map[String,Json]]((editorTechnique \ "tags").headOption, x => (x \ "tag").accumulatePure(unserialiseTag).toBox.map(_.toMap))
      parameters <- (editorTechnique \ "parameters").accumulatePure(unserialiseTechniqueParameterDiff).toBox
      elems <- (editorTechnique \ "calls").accumulatePure(unserialiseTechniqueElemDiff).toBox
    } yield {
      ModifyEditorTechniqueDiff(
        id, displayName, name, version, category, description, documentation, elems, parameters, resources, tags
      )
    }
  }
  ...
  def unserialiseTechniqueParameterModifyDiff (techniqueParameter : Node)  : Box[ModifyTechniqueParameterDiff] =  {
    ...
    for {
      ...
      ModifyTechniqueParameterDiff(
        id, displayName, name, description, documentation, mayBeEmpty, constraints
      )
    }
  }
  ...
  def unserialiseTechniqueBlockModifyDiff (entry : Node)  : Box[ModifyTechniqueBlockDiff] =  {
    import com.normation.rudder.services.eventlog.EventLogDetailsService.*
    for {
      block <- Box((entry \ "block").headOption) ?~! ("Entry type is not a technique block : " + entry.toString())
      changeTypeAddOk <- {
        if (block.attribute("changeType").map(_.text).contains("modify"))
          Full("OK")
        else
          Failure("Editor technique attribute does not have changeType=modify: " + entry.toString())
      }
      fileFormatOk <- TestFileFormat(block)
      ...
      ModifyTechniqueBlockDiff(
        id, displayName, component, condition, reportingLogic, Nil, policyMode, foreach, foreachName
      )
    }
  }

  def unserialiseTechniqueCallModifyDiff (entry : Node)  : Box[ModifyTechniqueCallDiff] =  {
    import com.normation.rudder.services.eventlog.EventLogDetailsService.*
    for {
      call <- Box((entry \ "call").headOption) ?~! ("Entry type is not a technique call : " + entry.toString())
      changeTypeAddOk <- {
        if (call.attribute("changeType").map(_.text).contains("modify"))
          Full("OK")
        else
          Failure("Editor technique attribute does not have changeType=modify: " + entry.toString())
      }
      fileFormatOk <- TestFileFormat(call)
      id <- Box((call \ "id").headOption.map(_.text)) ?~!
        ("Missing attribute 'id' in entry type editorTechnique : " + entry.toString())
      method <- Box((call \ "method").headOption.map(s => BundleName(s.text))) ?~!
        ("Missing attribute 'method' in entry type editorTechnique : " + entry.toString())
      ...
      calls <- getFromTo((call \ "parameters").headOption, s => (s \ "parameter").accumulatePure(unserialiseCallParameters).toBox.map(_.toMap))
    } yield {
      ModifyTechniqueCallDiff(
        method, id, displayName, component, condition, disableReporting, calls, policyMode, foreach, foreachName
      )
    }
  }
  ...
  def unserialiseTechniqueParameterDiff(entry : Node) : PureResult[TechniqueParameterDiff] = {
    ...
    if (entry.label ==  "techniqueParameter") {
      entry.attribute("changeType").map(_.text) match {
          ...
          case s => Left(Inconsistency(s"Technique parameter attribute does not have a valid changeType: ${s.getOrElse("<no changeType attribute>")}"))
        }
      } else {
        Left(Inconsistency("Entry type is not a technique call : " + entry.toString()))
     }
  }
    ...
    entry.label match {
      case "block" =>
        entry.attribute("changeType").map(_.text) match {
          ...
          case s => Left(Inconsistency(s"Technique block attribute does not have a valid changeType: ${s.getOrElse("<no changeType attribute>")}"))
      ...
      case "call" =>
        entry.attribute("changeType").map(_.text) match {
          ...
          case s => Left(Inconsistency(s"Technique call attribute does not have a valid changeType: ${s.getOrElse("<no changeType attribute>")}"))
