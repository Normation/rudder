    ...
    (for {
      ...
      if (cond) {
        (for {
          ...
          groupsOk      <- (changeRequest.nodeGroups.values.toSeq).accumulate { changes =>
      ...
      .chainError("An error occurred while checking the change request acceptance")
      .either
      .runNow match {
