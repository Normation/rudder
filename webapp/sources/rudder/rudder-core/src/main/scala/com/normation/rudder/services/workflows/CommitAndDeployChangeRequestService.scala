  ...
  private[this] def isMergeableConfigurationChangeRequest(changeRequest: ConfigurationChangeRequest): Boolean = {
        ...
        for {
          ...
          if (currVal != initVal) {
            debugLog(s"Directive ID ${initialFixed.id.uid.value} parameter $key has changed : original state from CR: ${initVal
                .getOrElse("value is mising")}, current value: ${currVal.getOrElse("value is mising")}")
          }
        }
    ...
    def compareGroups(initial: NodeGroup, current: NodeGroup): Boolean = {
      ...
      if (initialFixed == currentFixed) {
        ...
        if (initialFixed.name != currentFixed.name) {
          debugLog(
            s"Group ID ${initialFixed.id.value} name has changed: original state from CR: ${initialFixed.name}, current value: ${currentFixed.name}"
          )
        }

        if (initialFixed.description != currentFixed.description) {
          debugLog(
            s"Group ID ${initialFixed.id.value} description has changed: original state from CR: ${initialFixed.description}, current value: ${currentFixed.description}"
          )
        }

        if (initialFixed.query != currentFixed.query) {
          debugLog(
            s"Group ID ${initialFixed.id.value} query has changed: original state from CR: ${initialFixed.query}, current value: ${currentFixed.query}"
          )
        }

        if (initialFixed.isDynamic != currentFixed.isDynamic) {
          debugLog(
            s"Group ID ${initialFixed.id.value} dynamic status has changed: original state from CR: ${initialFixed.isDynamic}, current value: ${currentFixed.isDynamic}"
          )
        }

        if (initialFixed.isEnabled != currentFixed.isEnabled) {
          debugLog(
            s"Group ID ${initialFixed.id.value} enable status has changed: original state from CR: ${initialFixed.isEnabled}, current value: ${currentFixed.isEnabled}"
          )
        }
        ...
        if (!(initialFixed.isDynamic && currentFixed.isDynamic) && initialFixed.serverList != currentFixed.serverList) {
          debugLog(s"Group ID ${initialFixed.id.value} nodes list has changed: original state from CR: ${initialFixed.serverList
              .map(_.value)
              .mkString("[ ", ", ", " ]")}, current value: ${currentFixed.serverList.map(_.value).mkString("[ ", ", ", " ]")}")
        }

        if (initialFixed.properties != currentFixed.properties) {
          debugLog(s"Group ID ${initialFixed.id.value} properties changed: original state from CR: ${initialFixed.properties
              .map(_.toData)
