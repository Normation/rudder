...
class CommitAndDeployChangeRequestServiceImpl(
  ...
  private[this] def isMergeableConfigurationChangeRequest(changeRequest: ConfigurationChangeRequest): Boolean = {
    ...
    final case object CheckRule extends CheckChanges[Rule] {
      ...
      def xmlSerialize(rule: Rule): Box[Elem] = Full(xmlSerializer.rule.serialise(rule))
    ...
    case object CheckGroup extends CheckChanges[NodeGroup] {
      ...
      def xmlSerialize(group: NodeGroup): Box[Elem] = Full(xmlSerializer.group.serialise(group))
