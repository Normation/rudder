...
class ApiAccountUnserialisationImpl extends ApiAccountUnserialisation {
  ...
  def unserialise(entry: XNode): Box[ApiAccount] = {
    for {
      ...
      lastAuthentication <- (apiAccount \ "lastAuthentication").headOption match {
                              case None                                                       => Full(AccountLastAuthentication.Unknown)
                              case Some(s) if s.text == AccountLastAuthentication.Never.value =>
                                Full(AccountLastAuthentication.Never)
                              case Some(s)                                                    =>
                                Either
                                  .catchNonFatal(Instant.parse(s.text))
                                  .bimap(
                                    _ => "Bad date format for field 'lastAuthentication' in entry type API Account : " + entry,
                                    AccountLastAuthentication.AtDate(_)
                                  )
                                  .toBox
                            }
