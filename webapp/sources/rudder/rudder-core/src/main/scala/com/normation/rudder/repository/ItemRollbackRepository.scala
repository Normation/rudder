 ...
 *
 * This is deliberately separate from `ItemArchiveManager`: restoring the whole archive replaces every
 * configuration object at once and is therefore reserved to users with a grant on all tenants, while
 * restoring one item only touches that item and runs under the grant of the user asking for it.
 */
