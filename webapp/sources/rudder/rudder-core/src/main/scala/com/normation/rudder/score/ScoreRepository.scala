...
class InMemoryScoreRepository extends ScoreRepository {
  ...
  override def getScore(nodeId: NodeId, scoreId: Option[String]): IOResult[List[Score]] = cache.get.map{
    c =>
  ...
  override def getOneScore(nodeId: NodeId, scoreId: String): IOResult[Score] = getScore(nodeId,Some(scoreId)).flatMap(_.headOption.notOptional(""))
  override def saveScore(nodeId:   NodeId, score:   Score):          IOResult[Unit] = cache.update(c => c.updatedWith(nodeId) {
    case None => Some(score :: Nil)
    case Some(values) => Some(score :: values.filterNot( _.scoreId == score.scoreId))
  })
  override def deleteScore(nodeId: NodeId, scoreId: Option[String]): IOResult[Unit] = cache.update(c => c.updatedWith(nodeId) {
    case None => None
    case Some(values) => scoreId match {
      case None => None
