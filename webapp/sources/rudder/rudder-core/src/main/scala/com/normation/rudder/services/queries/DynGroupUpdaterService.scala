  ...
  override def computeDynGroup(group: NodeGroup): Box[NodeGroup] = {
    for {
      ...
      newMembers      <- queryProcessor.processOnlyId(
                           query
                         ) ?~! s"Error when processing request for updating dynamic group '${group.name}' (${group.id.value})"
      ...
      result    <- com.normation.utils.Control.sequence(dynGroups) { group =>
                     for {
                       ...
                       savedGroup <- woNodeGroupRepository
                                       ...
                                       .toBox ?~! s"Error when saving update for dynamic group '${group.name}' (${group.id.value})"
                     } yield {
