  ...
  def getSGEntry(con: RoLDAPConnection, id: NodeGroupId, attributes: String*): IOResult[Option[LDAPEntry]] = {
    con.searchSub(rudderDit.GROUP.dn, EQ(A_NODE_GROUP_UUID, id.value), attributes: _*).flatMap { srvEntries =>
      srvEntries.size match {
        ...
        case _ =>
          Inconsistency(
            s"Error, the directory contains multiple occurrence of the server group with ID '${id.value}'. DNs involved: ${srvEntries.map(_.dn).mkString("; ")}"
          ).fail
      }
    }
  }
  ...
  def getNodeGroupCategory(id: NodeGroupId): IOResult[NodeGroupCategory] = {
    groupLibMutex.readLock {
      for {
        ...
        parentCategoryEntry <-
          con.get(groupEntry.dn.getParent).notOptional(s"Parent category of entry with ID '${id.value}' was not found")
        parentCategory      <-
          mapper
            .entry2NodeGroupCategory(parentCategoryEntry)
            .toIO
...
class WoLDAPNodeGroupRepository(
  ...
  private[this] def checkNodeGroupExists(con: RoLDAPConnection, group: NodeGroup): IOResult[Boolean] = {
    groupLibMutex.readLock(
      con
        .searchSub(
          rudderDit.GROUP.dn,
          AND(IS(OC_RUDDER_NODE_GROUP), OR(EQ(A_NODE_GROUP_UUID, group.id.value), EQ(A_NAME, group.name))),
          A_NODE_GROUP_UUID
        )
        .flatMap(_.size match {
          case 0 => false.succeed
          ...
          case _ =>
            logPure
              .error(s"There is more than one node group with id '${group.id.value}' or name '${group.name}'") *> true.succeed
        })
    )
  }
  ...
  private[this] def checkNameAlreadyInUse(con: RoLDAPConnection, name: String, id: NodeGroupId): IOResult[Boolean] = {
    groupLibMutex.readLock(
      con
        .searchSub(
          rudderDit.GROUP.dn,
          AND(NOT(EQ(A_NODE_GROUP_UUID, id.value)), AND(EQ(A_OC, OC_RUDDER_NODE_GROUP), EQ(A_NAME, name))),
          A_NODE_GROUP_UUID
        )
        .flatMap(_.size match {
          case 0 => false.succeed
  ...
  override def create(
    ...
    groupLibMutex.writeLock(for {
      ...
      exists        <- if (exists) {
                         Inconsistency(
                           s"Cannot create a group '${nodeGroup.name}': a group with the same id (${nodeGroup.id.value}) or name already exists"
                         ).fail
                       } else UIO.unit
    ...
    groupLibMutex.writeLock(
      for {
        ...
        oldGroup <- mapper
                      ...
                      .toIO
                      .chainError("Error when trying to check for the group %s".format(nodeGroup.id.value))
                                         ...
                                         (oldGroup.isSystem, systemCall) match {
                                           case (true, false) =>
                                             "System group '%s' (%s) can not be modified"
                                               .format(oldGroup.name, oldGroup.id.value)
                                               .fail
                                           case (false, true) =>
    ...
    groupLibMutex.writeLock(
      for {
        ...
        existing <- getSGEntry(con, nodeGroupId).notOptional(
                      s"Error when trying to check for existence of group with id '${nodeGroupId.value}'. Can not update"
                    )
        oldGroup <-
          mapper.entry2NodeGroup(existing).toIO.chainError(s"Error when trying to check for the group '${nodeGroupId.value}'")
  ...
  override def move(
    ...
    groupLibMutex.writeLock(for {
      ...
      existing    <- getSGEntry(con, nodeGroupId).notOptional(
                       "Error when trying to check for existence of group with id %s. Can not update".format(nodeGroupId.value)
                     )
      oldGroup    <- mapper
                       .entry2NodeGroup(existing)
                       .toIO
                       .chainError("Error when trying to get the existing group with id %s".format(nodeGroupId.value))
