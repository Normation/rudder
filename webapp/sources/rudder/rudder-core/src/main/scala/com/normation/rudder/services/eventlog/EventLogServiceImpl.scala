...
class EventLogServiceImpl(val repository: EventLogRepository) extends EventLogService {
        ...
        .map(node => {
          node.label match {
            ...
            case Constants.XML_TAG_EDITOR_TECHNIQUE =>
              ...
              id.equals(i.value)
            case other                              =>
              EventLogsLoggerPure.debug(s"Filtering by id on event logs we should not have this object type here ${other}")
              false
          }
        })
        .forall(identity)
   ...
   * They don't want to see automatically generated logs also.
  ...
  override def getUserEventLogs(filter: Option[EventLogRequest])(implicit qc: QueryContext): IOResult[Seq[EventLog]] = {
    (for {
      ...
      eventsFiltered <- ZIO.succeed(events.filter(e => filterOnElementIdOrName(e.details, filter.flatMap(_.id))))
    } yield {
      eventsFiltered
    }).catchSystemErrors
  }
