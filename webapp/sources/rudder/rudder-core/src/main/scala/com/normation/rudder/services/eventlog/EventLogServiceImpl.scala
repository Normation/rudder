...
class EventLogServiceImpl(val repository: EventLogRepository) extends EventLogService {
   ...
   */
  override def getUserEventLogs(filter: Option[EventLogCriteriaFilter]): IOResult[Seq[EventLog]] = {
    (for {
      events <-
        repository
  ...
  private def excludeRudderEventLogs(filter: Option[EventLogCriteriaFilter]) = {
