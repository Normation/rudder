...
import com.normation.rudder.ncf.eventlogs.{AddEditorTechniqueDiff, ModifyEditorTechniqueDiff}
...
class TechniqueWriterImpl(
  ...
  def writeTechniqueAndUpdateLib(
    ...
    for {
      ...
      _                    <-
        techLibUpdate
          .update(changeContext.modId, changeContext.actor, Some(s"Update Technique library after creating files for ncf Technique ${technique.name}"))
          .toIO
