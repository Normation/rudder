...
class RudderCRunner(
   ...
   */
  def compileForTarget[T <: RuddercTarget](
      techniquePath:                  String
  )(implicit ruddercOptionsForTarget: RuddercOptionsForTarget[T]) = {
    for {
      ...
      _      <-
        TimingDebugLoggerPure.trace(
          s"compileTechnique: compiling technique '${techniquePath}' into ${ruddercOptionsForTarget.targetName} took ${time_2 - time_1}ms"
        )
    } yield ()
  }

  // returns whether the fallback was applied
  def writeOne[T <: RuddercTarget](
      target:         T,
      ruddercTargets: Set[RuddercTarget],
      technique:      EditorTechnique,
      methods:        Map[BundleName, GenericMethod],
      fallback:       AgentSpecificTechniqueWriter,
      outputPath:     String,
      configFilePath: String
  ): ZIO[Any, RudderError, Boolean] = {
    if (ruddercTargets.contains(target)) {
      ...
      for {
        _ <-
          TechniqueWriterLoggerPure.debug(
            s"Using fallback technique generation in place of rudderc for technique '${technique.path}' because target '${target.name}' not enabled in settings"
          )
  ...
  def compileTechnique(
      technique:        EditorTechnique,
      targets:          Set[RuddercTarget],
      methods:          Map[BundleName, GenericMethod],
      cfengineFallback: AgentSpecificTechniqueWriter,
      dscFallback:      AgentSpecificTechniqueWriter
  ) = {
    ...
    for {
      ...
      writtenByRudderWebapp <-
        writeOne(RuddercTarget.CFEngine, targets, technique, methods, cfengineFallback, outputPath, configFilePath)
...
class TechniqueWriter(
  ...
  def deleteTechnique(
    ...
    def removeTechnique(techniqueId: TechniqueId, technique: domain.Technique): IOResult[Unit] = {
      for {
                          ...
                          if (deleteDirective) {
                            ...
                            for {
                              cr <- directives
                                      ...
                                      .notOptional(
                                        s"Could not create a change request to delete ${technique.name}/${techniqueVersion} directives"
                                      )
                              _  <- wf.startWorkflow(cr, committer, Some(s"Deleting technique ${technique.name}/${techniqueVersion}")).toIO
                            } yield ()
                          } else {
                            Unexpected(
                              s"${directives.size} directives are defined for ${technique.name}/${techniqueVersion} please delete them, or force deletion"
                            ).fail
                          }
                      }
        ...
        _          <- activeTech match {
                        ...
                        case Some(activeTechnique) =>
                          writeDirectives.deleteActiveTechnique(
                            ...
                            modId,
                            committer,
                            Some(s"Deleting active technique ${techniqueName}")
                          )
                      }
        _          <- archiver.deleteTechnique(
                        ...
                        modId,
                        committer,
                        s"Deleting technique ${technique.name}/${techniqueVersion}"
                      )
        _          <- techLibUpdate
    ...
    def removeInvalidTechnique(basePath: String, techniqueName: String): IOResult[Unit] = {
      val unknownTechniquesDir =
        File(s"${basePath}/techniques/").listRecursively.filter(_.isDirectory).filter(_.name == techniqueName).toList
          ...
          for {
            _ <- ZIO.foreach(unknownTechniquesDir) { f =>
                   val cat = f.pathAsString
                     .substring(s"${basePath}/techniques/".length)
                     .split("/")
  ...
  def techniqueMetadataContent(
      technique:             EditorTechnique,
      methods:               Map[BundleName, GenericMethod],
      writtenByRudderWebapp: Boolean
  ): PureResult[XmlNode] = {
  ...
  def writeTechnique(
    ...
    for {
      ...
      time_1                <- currentTimeMillis
      _                     <-
        TimingDebugLoggerPure.trace(s"writeTechnique: writing json for technique '${technique.name}' took ${time_1 - time_0}ms")
      ...
      writtenByRudderWebapp <- if (targets.nonEmpty) {
                                 ...
                                 compiler
                                   .compileTechnique(
                                     ...
                                     targets,
                                     methods,
                                     cfengineFallbackTechniqueWriter,
                                     dscFallbackTechniqueWriter
                                   )
                                   .catchAll { e =>
                                     val errorPath: File =
                                       root / errorLogPath / "rudderc" / "failures" / s"${DateTime.now()}_${technique.bundleName.value}.log"
                                     for {
                                       _ <-
                                         TechniqueWriterLoggerPure.error(
                                           s"An error occurred when compiling technique '${technique.name}' (id : '${technique.bundleName}') with rudderc, error details in ${errorPath}, falling back to old saving process"
                                         )
                                       _ <- IOResult.effect {
                                              ...
                                              errorPath.write(
                                                s"""
                                                   ...
                                                   |  ${e.fullMsg}
                                                   |technique data =>
                                                   |  ${net.liftweb.json.prettyRender(
                                                    techniqueSerializer.serializeTechniqueMetadata(technique, methods)
                                                  )}""".stripMargin
                                              )
                                            }.catchAll(e2 => {
                                              TechniqueWriterLoggerPure.error(
                                                s"Error when writing error log of '${technique.name}' (id : '${technique.bundleName}') in in ${errorPath}: ${e2.fullMsg}"
                                              ) *>
                                              TechniqueWriterLoggerPure.error(
                                                s"Error when compiling '${technique.name}' (id : '${technique.bundleName}') with rudderc was: ${e.fullMsg}"
                                              )
                                            })
        ...
        case c: MethodCall =>
          ...
          if (call.methodId.value.startsWith("_")) {
            ...
            val disableReporting = {
              s"""-Report:$$${if (call.disabledReporting) { "false" }
                else { "true" }}"""
            }
            ...
            (for {
                ...
                ((call.parameters.toList).traverse {
                  case (id, arg) =>
                    translater.translateToAgent(arg, AgentType.Dsc) match {
                      ...
                      case eb: EmptyBox =>
                        val fail =
                          eb ?~! s"Error when translating parameter '${arg}' of technique of method ${call.methodId} of technique ${technique.name}"
              ...
              condition <- translater.translateToAgent(canonifyCondition(call), AgentType.Dsc) match {
                             ...
                             case eb: EmptyBox =>
                               val fail =
                                 eb ?~! s"Error when translating condition '${call.condition}' of technique of method ${call.methodId} of technique ${technique.name}"
                               Left(IOError(fail.messageChain, None))
                           }
              ...
              method         <- methods.get(call.methodId) match {
                                  ...
                                  case None         =>
                                    Left(
                                      MethodNotFound(
                                        s"Method '${call.methodId.value}' not found when writing dsc Technique '${technique.name}' methods calls",
                                        None
                                      )
                                    )
                                }
              ...
              classParameter <- params.get(method.classParameter) match {
                                  ...
                                  case None                 =>
                                    Left(
                                      MethodNotFound(
                                        s"Parameter '${method.classParameter.value}' for method '${method.id.value}' not found when writing dsc Technique '${technique.name}' methods calls",
                                        None
                                      )
                                    )
                                }
...
class TechniqueArchiverImpl(
  ...
  def deleteTechnique(
    ...
    (for {
      ...
      rm           <-
        IOResult.effect(gitRepo.git.rm.addFilepattern(s"techniques/${categoryPath}/${techniqueName}/${techniqueVersion}").call())
  ...
  def commitTechnique(technique: EditorTechnique, modId: ModificationId, commiter: EventActor, msg: String): IOResult[Unit] = {
    ...
    (for {
      ...
      added   <- ZIO.foreach(filesToAdd)(f => IOResult.effect(gitRepo.git.add.addFilepattern(f).call()))
      removed <- ZIO.foreach(filesToDelete)(f => IOResult.effect(gitRepo.git.rm.addFilepattern(f).call()))
      commit  <- IOResult.effect(gitRepo.git.commit.setCommitter(ident).setMessage(msg).call())
