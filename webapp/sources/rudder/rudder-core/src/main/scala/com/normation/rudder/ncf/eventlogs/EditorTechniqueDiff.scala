...
import com.normation.rudder.domain.policies.{PolicyMode, SimpleDiff, TriggerDeploymentDiff}
...
final case class ModifyEditorTechniqueDiff(
    ...
    modVersion: Option[SimpleDiff[Version]],
    ...
    modParameters:         Option[Seq[TechniqueParameterDiff]],
  ...
  def needDeployment: Boolean = {
    modParameters.isDefined || modVersion.isDefined || modCalls.nonEmpty || modResources.isDefined
  }
}

trait TechniqueBlockDiff
...
final case class ModifyTechniqueBlockDiff(
  ...
  modeDisableReporting:  Option[SimpleDiff[ReportingLogic]],
  modCalls:             List[TechniqueBlockDiff|TechniqueCallDiff],
  ...
  def needDeployment: Boolean = {
    modCalls.nonEmpty || modPolicyMode.isDefined || modeDisableReporting.isDefined
  }
}
...
object TechniqueElemDiff {
  def buildParametersDiff(oldElems: Seq[MethodElem], updateElems: Seq[MethodElem]): List[TechniqueCallDiff | TechniqueBlockDiff] = {
...
object TechniqueParameterDiff {
  def buildParametersDiff(oldParams: Seq[TechniqueParameter], updateParams: Seq[TechniqueParameter]): Option[Seq[TechniqueParameterDiff]] = {
    if (oldParams == updateParams) {
      ...
      for {
        ...
        (oldValue, updateValue) match {
          case (None,None) => None
          case(Some(old), None) => DeleteTechniqueParameterDiff(old)
          case(None,Some(update)) => AddTechniqueParameterDiff(update)
          case(Some(old),Some(update)) =>
            ModifyTechniqueParameterDiff(old,update)
        }
...
object ModifyEditorTechniqueDiff {
  ...
  def apply(old: EditorTechnique, update: EditorTechnique): ModifyEditorTechniqueDiff = {
    ModifyEditorTechniqueDiff(
      ...
      old.name,
      SimpleDiff.createDiff(old, update)(_.name),
      SimpleDiff.createDiff(old, update)(_.version),
