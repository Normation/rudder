...
class InMemoryGlobalScoreRepository extends GlobalScoreRepository {
  ...
  override def save(nodeId: NodeId, globalScore: GlobalScore): IOResult[(NodeId, GlobalScore)] = cache.update(_.updated(nodeId,globalScore)).map(_ => (nodeId,globalScore))
}
