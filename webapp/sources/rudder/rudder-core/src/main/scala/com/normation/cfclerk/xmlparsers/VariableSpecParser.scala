...
object Utils {
  ...
  def getUniqueNode(root: Node, nodeName: String, subtree: Boolean = false): Either[LoadTechniqueError, Node] = {
    def checkCardinality(nodes: NodeSeq): Either[LoadTechniqueError, Node] = {
      if (nodes.isEmpty) {
        Left(
          LoadTechniqueError.Consistancy(
            s"No node found for name ${nodeName} in ${root} children with scope ${if (subtree) "subtree" else "one level"}"
          )
        )
      } else if (nodes.size > 1) {
        Left(
          LoadTechniqueError.Consistancy(
            s"More than one node found for name ${nodeName} in ${root} children with scope ${if (subtree) "subtree"
              else "one level"}"
          )
        )
...
class VariableSpecParser extends Loggable {
  ...
  def parseSectionVariableSpec(
      parentSectionName: String,
      elt:               Node
  ): Either[LoadTechniqueError, (SectionVariableSpec, List[SectionVariableSpec])] = {
    ...
    if (!SectionVariableSpec.isVariable(markerName)) {
      Left(
        LoadTechniqueError.Consistancy(
          s"The node '${markerName}' is not a variable specification node: it should be one of ${SectionVariableSpec.markerNames
              .mkString("<", ">, <", ">")}"
        )
      )
      ...
      (getUniqueNodeText(elt, VAR_NAME, ""), getUniqueNodeText(elt, VAR_DESCRIPTION, "")) match {
        case ("", _) => Left(LoadTechniqueError.Consistancy("Name is mandatory but wasn't found for variable spec elt: " + elt))
        case (_, "") =>
          Left(LoadTechniqueError.Consistancy("Description is mandatory but wasn't found for variable spec elt: " + elt))
        ...
        case (name, desc) =>
          for {
            _              <- reservedVariableName.find(reserved => name == reserved) match {
                                case None    => Right("ok")
                                case Some(x) =>
                                  Left(LoadTechniqueError.Consistancy(s"Name '${x}' is reserved and can not be used for a variable"))
                              }
            ...
            _               = { // we use to have a "uniqueVariable" field that is not supported anymore in Rudder 4.3
              ...
              if (getUniqueNode(elt, "UNIQUEVARIABLE").isRight) {
                logger.warn(
                  s"Bad tag in variable '${parentSectionName}': since Rudder 4.3, a variable can not be marked as 'UNIQUEVARIABLE' anymore and that attribute will be ignored. In place, " +
  ...
  def parseConstraint(varName: String, elt: Node): Either[LoadTechniqueError, (Constraint, List[SectionVariableSpec])] = {
    ...
    def regexError(vt: VTypeConstraint) = Left(
      LoadTechniqueError.Constraint(
        s"type '${vt.name}' already has a predifined regex (you can't define a regex with these types : ${VTypeConstraint.regexTypes.map(_.name).mkString(",")})."
      )
    )

    val resTypeName: Either[LoadTechniqueError, VTypeConstraint] = {
      VTypeConstraint.fromString(e, regexConstraint, parseAlgoList(passwordHashes)) match {
        case None                     =>
          Left(
            LoadTechniqueError.Constraint(
              s"'${e}' is an invalid type.\n A type may be one of the next list : ${VTypeConstraint.allTypeNames}"
            )
          )
    ...
    for {
      ...
      val derivedVars = typeName match {
        case MasterPasswordVType(_) =>
          getUniqueNodeText(elt, CONSTRAINT_PWD_AUTOSUBVARIABLES, "")
            .split(",")
            .flatMap { s =>
