                                   ...
                                   if (acc.isDefinedAt(pId)) Right(Nil)
                                     ...
                                     allGroups.get(pId) match {
                                       case None    =>
                                         Left(
                                           Inconsistency(
                                             s"Parent group with id '${parentId}' is missing for group '${g.nodeGroup.name}'(${g.nodeGroup.id.value})"
                                           )
                                         )
                 ...
                 case ex: Exception => // At least NPE, IllegalArgEx, UnsupportedException...
                   Left(
                     SystemError(
                       ...
                       s"(${child.groupId.value}) for parent '${p.groupName}' (${p.groupId.value})",
                       ex
                     )
                   )
               }
      ...
      _      <- groupList.traverse { group =>
                  try {
                    ...
                    case ex: Exception => // At least NPE, IllegalArgEx, UnsupportedException...
                      Left(
                        SystemError(
                          s"Error when trying to build group hierarchy of group '${group.groupName}' ${group.groupId.value})",
                          ex
                        )
                      )
                  }
