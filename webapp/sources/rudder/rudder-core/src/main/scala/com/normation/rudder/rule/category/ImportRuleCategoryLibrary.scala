...
class ImportRuleCategoryLibraryImpl(
  ...
  def swapRuleCategory(
    ...
    def atomicSwap(rootCategory: RuleCategory): IOResult[RuleCategoryArchiveId] = {
      ...
      def saveUserLib(con: RwLDAPConnection): IOResult[Unit] = {
        def recSaveUserLib(parentDN: DN, content: RuleCategory): IOResult[Unit] = {
          ...
          for {
            category      <-
              con.save(categoryEntry).chainError(s"Error when persisting category with DN '${categoryEntry.dn}' in LDAP")
            subCategories <- ZIO.foreach(content.childs)(cat => recSaveUserLib(categoryEntry.dn, cat))
          } yield {
            () // unit is expected
          }
        }
        ...
        finished <- {
          (for {
            ...
            restoreArchive(con, rudderDit.GROUP.dn, targetArchiveDN).fold(
              _ =>
                Chained(s"Error when trying to restore archive with ID '${archiveId.value}' for the active technique library", e),
              _ =>
                Chained("Error when trying to load archived Rule category library. A rollback to previous state was executed", e)
            )
          }
        }
      } yield {
    ...
    def checkUserLibConsistance(rootCategory: RuleCategory): IOResult[RuleCategory] = {
        ...
        if (!isRoot && cat.isSystem && includeSystem == false) None
          ...
          parentCategories match {
            ...
            case Some(list) if list.contains(parent.id) =>
              logEffect.error(
                s"Ignoring Rule category with ID '${cat.id.value}' because its name is '${cat.name}' already referenced in a child of category with '${parent.name}' of ID '${parent.id.value}'"
              )
              None
      ...
      IOResult
        .effect(recSanitizeCategory(rootCategory, rootCategory, true))
    ...
    for {
      ...
      _        <- (for {
                    ...
                    ()
                  }).catchAll(e => logPure.warn("Error when deleting archived library in LDAP with DN '%s'".format(dn)))
    } yield {
      ()
    }
  }
