...
case class InstanceIdComparator(instanceId: InstanceId) extends NonLdapCriterionType with TStringComparator {
  ...
  private def continueFilter = HAS(A_OC)
  private def breakFilter    = NOT(HAS(A_OC))

  override def buildFilter(attributeName: String, comparator: CriterionComparator, value: String): Filter = comparator match {
    // The filter is done here, not in LDAP, and we can short-circuit the LDAP query in case of no match
    case Equals    => if (matches(value)) continueFilter else breakFilter
    case NotEquals => if (!matches(value)) continueFilter else breakFilter
    case _         => continueFilter
  }

  def matches(value: String): Boolean = {
    value.equalsIgnoreCase(instanceId.value)
  }
}
