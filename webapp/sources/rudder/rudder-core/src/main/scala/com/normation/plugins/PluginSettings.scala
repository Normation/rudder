...
class FilePluginSettingsService(
  ...
  def readPluginSettings() = {
    ...
    for {
      ...
      userName       <-
        IOResult.effect(s"Getting user name for plugin download in ${pluginConfFile.pathAsString}")(p.getProperty("username"))
      pass           <-
        IOResult.effect(s"Getting password for plugin download in ${pluginConfFile.pathAsString}")(p.getProperty("password"))
