...
Avoid the usage of the `CurrentUser` to obtain the `QueryContext`, unless within safety nets guaranteeing that it is defined, namely **snippets strictly initialized with the `QueryContext` of the authenticated user**.

Safe snippet are introduced in [this issue](https://issues.rudder.io/issues/28404), here is the implementation that is proposed:
```scala 3
