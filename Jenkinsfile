        ...
        stage('Tests') {
            parallel {
                ...
                stage('python-lib') {
                    ...
                    steps {
                        dir("policies/lib") {
                            sh script: '~/.local/bin/avocado run --disable-sysinfo tests/quick', label: 'quick method tests'
                        }
                    }
                    post {
                        failure {
                            ...
                            script {
                                docker.image('postgres:11-bullseye').withRun('-u 0:0 -e POSTGRES_USER=${POSTGRES_USER} -e POSTGRES_PASSWORD=${POSTGRES_PASSWORD} -e POSTGRES_DB=${POSTGRES_DB}', '-c listen_addresses="*"') { c ->
                                    docker.build('relayd', "-f relay/sources/relayd/Dockerfile --build-arg USER_ID=${env.JENKINS_UID} --pull .")
                                          .inside("-u 0:0 -v /srv/cache/cargo:/usr/local/cargo/registry/cache -v /srv/cache/sccache:/root/.cache/sccache --link=${c.id}:postgres") {
                ...
                stage('rudder-package') {
                    agent {
                        dockerfile {
                            ...
                            // mount cache
                            args '-u 0:0 -v /srv/cache/cargo:/usr/local/cargo/registry/cache -v /srv/cache/sccache:/root/.cache/sccache'
                        }
                    }
                    steps {
