...
class TestJsEngine extends Specification {
  ...
  val variableSpec = InputVariableSpec("test", "")
  ...
  val getFooVariable = variableSpec.toVariable(Seq("$evaljs foo"))
...
//    major >= 8 // else rhino, because we don't support java 1.5 and before in all case
  ...
  def beVariableValue[T](cond: String => Boolean): Matcher[Box[Variable]] = { b: Box[Variable] =>
    ...
    , s"${b} is not a Full(InputVariable) that matches condition but a '${b}'"
    )
  }
  ...
  "When feature is disabled, one " should {
    ...
    "failed with a message when the variable is a script" in {
      context( _.eval(get4scriptVariable, JsRudderLibBinding.Crypt)) must beFailure(".*starts with.*evaljs.*".r)
    }
  }
  ...
  "When feature is enabled, one " should {
    ...
    "get the correct STRING value for simple expression" in {
      context( _.eval(get4scriptVariable, JsRudderLibBinding.Crypt)) must beVariableValue(s => Integer.parseInt(s) == 4)
    }
    ...
    "not be able to access the content of a previously setted var" in {
      ...
      (res2 must beFailure(".*ReferenceError.*foo.*".r))
    }
  ...
  "When using the Rudder JS Library, one" should {
    ...
    val sha512Variable = variableSpec.toVariable(Seq("$evaljs rudder.password.sha512('secret', '01234567')"))
    ...
    "get the correct hashed value for Aix sha256" in {
      ...
      } must beVariableValue( _.startsWith("{ssha256}") )
    }
    ...
    "get the correct hashed value for Aix sha512" in {
      ...
      } must beVariableValue( _.startsWith("{ssha512}") )
    }
  }
