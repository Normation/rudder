...
sealed trait PolicyInstanceDiff

final case class DeletePolicyInstanceDiff(
    policyTemplateName: PolicyPackageName
  , pi                : PolicyInstance
) extends PolicyInstanceDiff with HashcodeCaching

// add and modify are put together
sealed trait PolicyInstanceSaveDiff extends PolicyInstanceDiff 

final case class AddPolicyInstanceDiff(
    policyTemplateName : PolicyPackageName
  , pi                 : PolicyInstance
) extends PolicyInstanceSaveDiff with HashcodeCaching


final case class ModifyPolicyInstanceDiff(
    policyTemplateName      : PolicyPackageName
  , id                      : PolicyInstanceId
  , name                    : String //keep the name around to be able to display it as it was at that time
  , modName                 : Option[SimpleDiff[String]]        = None
  , modPolicyTemplateVersion: Option[SimpleDiff[PolicyVersion]] = None
