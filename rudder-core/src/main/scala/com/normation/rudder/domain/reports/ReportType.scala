...
object ReportType {
  ...
  def apply(report : Reports) : ReportType = {
    report match {
      case _ : ResultSuccessReport       => EnforceSuccess
      case _ : ResultErrorReport         => EnforceError
      case _ : ResultRepairedReport      => EnforceRepaired
      case _ : ResultNotApplicableReport => EnforceNotApplicable
      case _                             => Unexpected
    }
  }
}
