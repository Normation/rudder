...
case class PolicyInstanceId(value:String) extends HashcodeCaching

/**
 * That class represent a Policy Instance.
 * 
 * A policy instance is a Policy Template bounded to
 * values for:
 * - its parameter (group of parameters and their value(s) )
 * - some other policy parameters (priority of that instance compared 
 *   to other one, name, description, etc)
 *
 */
case class PolicyInstance(
  /**
   * Policy instance are uniquely identified in
   * universe by an UUID
   */
  id:PolicyInstanceId,
  
  /**
   * They reference one and only one PolicyTemplate version
   */
  policyTemplateVersion:PolicyVersion,
  
  /**
   * The list or parameters with their values.
   * Parameters must be the same as in UserPolicyTemplate pointed
   * by UserPolicyTemplateUuid
   * TODO: I really would like to be able to not allow to set bad parameter here,
   *       what mean parameter that are not in the referenced policy template.
   *       For now, say it's done by construction.
   */
  parameters:Map[String, Seq[String]],  
  
  /**
   * A human readable name for that policy instance,
...
object SectionVal {
  ...
  def piValToSectionVal(rootSection:SectionSpec, allValues:Map[String,Seq[String]]) : SectionVal = {
    /*
