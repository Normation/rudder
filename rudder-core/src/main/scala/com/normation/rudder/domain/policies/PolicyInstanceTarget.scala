 ...
 * - a specific node 
 */
sealed abstract class PolicyInstanceTarget {
  def target:String
}

object GroupTarget { def r = "group:(.+)".r }
case class GroupTarget(groupId:NodeGroupId) extends PolicyInstanceTarget with HashcodeCaching {
  override def target = "group:"+groupId.value
}

//object NodeTarget { def r = "node:(.+)".r }
//case class NodeTarget(nodeId:NodeId) extends PolicyInstanceTarget {
//  override def target = "node:"+nodeId.value
//}

object PolicyServerTarget { def r = "policyServer:(.+)".r }
case class PolicyServerTarget(nodeId:NodeId) extends PolicyInstanceTarget with HashcodeCaching {
  override def target = "policyServer:"+nodeId.value
}

case object AllTarget extends PolicyInstanceTarget {
  override def target = "special:all"
  def r = "special:all".r
}

case object AllTargetExceptPolicyServers extends PolicyInstanceTarget {
  override def target = "special:all_exceptPolicyServers"
  def r = "special:all_exceptPolicyServers".r
}


object PolicyInstanceTarget {
...
case class PolicyInstanceTargetInfo(
