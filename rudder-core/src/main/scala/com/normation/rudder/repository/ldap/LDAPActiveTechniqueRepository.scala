...
class LDAPActiveTechniqueRepository(
  ...
  def move(uactiveTechniqueId:ActiveTechniqueId, newCategoryId:ActiveTechniqueCategoryId, actor: EventActor, reason: Option[String]) : Box[ActiveTechniqueId] = {
     for {
      ...
      // Like categories, we should only use the object active Technique not the entry
      ...
      autoArchive <- (if(autoExportOnModify && !moved.isInstanceOf[LDIFNoopChangeRecord] && !activeTechnique.isSystem) {
                       for {
                         parents  <- activeTechniqueBreadCrump(uactiveTechniqueId)
                         commiter <- personIdentService.getPersonIdentOrDefault(actor.name)
                         moved    <- gitArchiver.moveActiveTechnique(activeTechnique, oldParents.map( _.id), parents.map( _.id), Some(commiter, reason))
                       } yield {
                         moved
                       }
  ...
  def changeStatus(uactiveTechniqueId:ActiveTechniqueId, status:Boolean, actor: EventActor, reason: Option[String]) : Box[ActiveTechniqueId] = {
    for {
      ...
      activeTechnique      <- mapper.entry2ActiveTechnique(activeTechniqueEntry)
      saved       <- {
