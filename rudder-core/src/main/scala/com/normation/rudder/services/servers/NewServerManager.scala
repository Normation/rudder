 ...
 */
trait NewServerManager {

  /**
   * List all pending servers
   */
  def listNewServers : Box[Seq[Srv]]
 ...
 */
class NewServerManagerImpl(
    override val ldap:LDAPConnectionProvider, 
    override val pendingServersDit:InventoryDit,
    override val acceptedServersDit:InventoryDit,
    override val serverSummaryService:ServerSummaryServiceImpl,
...
) extends NewServerManager with ListNewServer with ComposedNewServerManager


///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


trait ListNewServer extends NewServerManager with Loggable {
  def ldap:LDAPConnectionProvider
  def serverSummaryService:ServerSummaryServiceImpl
  def pendingServersDit:InventoryDit
  
  override def listNewServers : Box[Seq[Srv]] = {
    ldap map { con => 
      con.searchOne(pendingServersDit.NODES.dn,ALL,Srv.ldapAttributes:_*).map { e => 
...
trait ComposedNewServerManager extends NewServerManager with Loggable {

  def ldap:LDAPConnectionProvider
  def pendingServersDit:InventoryDit
  def acceptedServersDit:InventoryDit
  def smRepo:LDAPFullInventoryRepository
  def serverSummaryService:ServerSummaryService
  ...
  override def refuse(ids:Seq[NodeId], actor:EventActor) : Seq[Box[Srv]] = {
    ids.flatMap { id => serverSummaryService.find(pendingServersDit, id).openOr(Seq()) }.map { srv => 
      refuseOne(srv, actor)
    }
  }
...
class AcceptInventory(
    ...
    acceptedServersDit:InventoryDit,
 ...
 */
class AcceptServerConfigurationRule(
    override val name:String,
