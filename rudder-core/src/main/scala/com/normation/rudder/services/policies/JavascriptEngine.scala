...
final object JsEngine {
  final val EVAL = "$evaljs "
  final val EVAL_SIZE = EVAL.size
  ...
  final object DisabledEngine extends JsEngine {
     ...
     */
    def eval(variable: Variable, lib: JsRudderLibBinding): Box[Variable] = {
      variable.values.find( _.startsWith(EVAL) ) match {
        ...
        case Some(v) =>
           ...
           */
          Failure(s"Value '${v}' starts with the ${EVAL} keyword, but the 'parameter evaluation feature' "
      ...
      for {
        values <- sequence(variable.values) { value =>
                    if(value.startsWith(EVAL)) {
                      val script = value.substring(EVAL_SIZE)
    ...
    override def checkPermission(permission: Permission): Unit = {
      if(isSandboxed.get) {
        permission match {
          ...
          case x: FilePermission if(x.getName.startsWith("/home/fanf") ) =>
          case _ =>
            println(s"Forbid '${Thread.currentThread().getName}' access to ${permission}")
