...
for template in templates:
    ...
    if ".png" in intro:
        print(
            "Error: images in API doc break links on Firefox: https://issues.rudder.io/issues/28289"
        )
        exit(1)
    ...
    # Download JS file
    if subprocess.call(["curl", "--silent", "--fail", url, "--output", redoc_js_file]):
        print("Could not download redoc lib from %s into %s" % (url, redoc_js_file))
        exit(1)
    ...
    if subprocess.call(
        ...
        exit(1)
    # Remove the script hash as it is invalid now, and not relevant as we serve it ourselved
    if subprocess.call(
        [
            "sed",
            "-i",
            f's/integrity=".*" cross/cross/',
            html_file,
        ]
    ):
        print("Could not remove script hash from %s" % redoc_js_file)
        exit(1)
