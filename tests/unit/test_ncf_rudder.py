...
class TestNcfRudder(unittest.TestCase):
  ...
  def test_rudder_reporting_content(self):
    ...
    expected_result.append('      "dummy_report" usebundle => log_rudder("Service start apache2 if debian", "service_start_apache2");')
    expected_result.append('')
    ...
    expected_result.append('      "dummy_report" usebundle => log_rudder("Package install openssh-server if service_start_apache2_repaired", "package_install_openssh_server");')
    expected_result.append('')
    ...
    expected_result.append('      "dummy_report" usebundle => log_rudder("Command execution /bin/date if !service_start_apache2_repaired.debian", "command_execution__bin_date");')
    expected_result.append('}')
  ...
  def test_rudder_reporting_with_variable_content(self):
    ...
    expected_result.append('      "dummy_report" usebundle => log_rudder("File create ${sys.workdir}/module/env if directory_create_${sys.workdir}_module_repaired", "file_create_${sys.workdir}_module_env"),')
    expected_result.append('                    ifvarclass => concat("!(directory_create_",canonify("${sys.workdir}"),"_module_repaired)");')
    expected_result.append('}')
