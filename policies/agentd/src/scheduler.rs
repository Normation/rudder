...
impl Schedule {
    ...
    /// Passing now as an argument simplifies testing and make calculation more precise
...
impl CommandBuilder {
    ...
    /// Spawn the command now (works on any OS)
...
impl Scheduler {
    /// Result::Err should be a fatal error, we try to  avoid it
    ...
    pub fn get_next_run(&self, job_name: &str) -> Result<DateTime<Local>> {
        let jobs = self.schedules.iter().find(|s| s.name == job_name);
        let job = match jobs {
            Some(j) => j,
