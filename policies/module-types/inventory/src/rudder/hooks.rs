...
/// Where an administrator drops the scripts we run.
pub fn dir() -> PathBuf {
    crate::rudder::state_dir().join("hooks.d")
}
...
impl Hook {
    ...
    /// A hook has to be:
    ///
    /// * a regular file, and not a symlink, as the target of a symlink can be replaced
    ///   between the moment we check it and the moment we run it,
    /// * executable,
    /// * owned by `root` or by the user we run as,
    /// * writable by its owner only, as group or world write access would let another user
    ///   change what we execute.
    ///
    ...
    fn validate(path: &Path) -> Result<Self> {
        ...
        let current_user = nix::unistd::getuid().as_raw();
...
pub fn custom_properties(dir: &Path) -> Option<String> {
    ...
    for path in entries(dir) {
        match Hook::validate(&path).and_then(|h| h.run()) {
            // Reserializing normalizes the formatting, and guarantees we only ever insert
            // one value into the array we build below.
            Ok(value) => properties.push(value.to_string()),
...
/// The entries of a hook directory, in file name order, ignoring hidden files.
///
/// An unreadable directory yields no hook, like an empty one.
fn entries(dir: &Path) -> Vec<PathBuf> {
    let Ok(read_dir) = fs::read_dir(dir) else {
        warn!("Could not read the hook directory '{}'", dir.display());
        return vec![];
    };
...
mod tests {
    ...
    /// Writes a hook with the given shell body and permissions.
    ///
    /// The file has to be closed before we execute it, see `crate::no_concurrent_fork`.
