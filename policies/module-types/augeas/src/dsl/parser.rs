use crate::dsl::comparator::{Number, NumericComparison, StrComparator, StrValidation};
use crate::dsl::{
...
fn parse_check_command(pair: Pair<Rule>) -> Result<CheckExpr> {
    Ok(match pair.as_rule() {
        ...
        Rule::values_equal => {
            ...
            CheckExpr::ValuesEqual(values)
        }
        Rule::values_not_equal => {
            let mut inner_rules = pair.into_inner();
            let values = parse_array(inner_rules.next().unwrap());
            CheckExpr::ValuesNotEqual(values)
        }
        Rule::len => {
            let mut inner_rules = pair.into_inner();
...
mod tests {
    ...
    fn pest_parse_script() {
        let input = r#"
            ...
            check /path/to/node has_type ipv4

            check /pat/to match_size < 5
        "#;
        let expected = vec![
            ...
            Expr::Check(
                ...
                CheckExpr::MatchSize(LessThan, 5),
            ),
        ];
