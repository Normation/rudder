...
fn nustache_render(s: &str) -> String {
    format!(
        "[Nustache.Core.Render]::StringToString('{{{{' + {} + '}}}}', $data, $mustacheOptions)",
        s
    )
}
...
impl Expression {
    pub fn simplify_using_context(
        ...
        match self.clone() {
            ...
            Expression::Const(e) => Ok(Expression::Const(Box::new(e.simplify_using_context(
                target,
                t_id,
                parameter_calls_shortcut,
            )?))),
            Expression::GlobalParameter(e) => Ok(Expression::GlobalParameter(Box::new(
                e.simplify_using_context(target, t_id, parameter_calls_shortcut)?,
            ))),
            ...
            Expression::Sys(v) => Ok(Expression::Sys(
                v.iter()
                    .map(|e| {
                        e.clone().simplify_using_context(
                            target,
                            t_id,
                    ...
                    match v[0].clone() {
                        Expression::Scalar(_) => {
                                ...
                                .any(|s| s.name == v[0].fmt(target))
                            {
                                ...
                                v[0].clone().simplify_using_context(
                                    target,
                                    t_id,
                                    parameter_calls_shortcut,
                                )
                        ...
                        _ => v[0].clone().simplify_using_context(
                            target,
                            t_id,
                            parameter_calls_shortcut,
                        ),
    ...
    pub fn fmt(&self, target: Target) -> String {
        match self {
            ...
            //    let a = s.split("\n").collect::<Vec<_>>();
            //    if a.len() > 1 {
            //        let mut v = vec![Expression::Scalar(a[0].to_string())];
            //        for e in a.iter() {
            //          v.push(Expression::NcfConst(Box::new(Expression::Scalar("n".to_string()))));
            //          v.push(Expression::Scalar(e.to_string()));
            //        }
            //        debug!("{:?}", v);
            //        Expression::Sequence(v).fmt(target)
            //    } else {
            //        s.to_string().replace('\'', "`\'")
            //    }
            //}
