...
fn it_should_succeed_if_directory_exists_in_audit_mode() {
    ...
    let tested_method = &method("directory_present", &[target_str]).audit();
...
fn it_should_succeed_if_directory_exists_in_enforce_mode() {
    ...
    let tested_method = &method("directory_present", &[target_str]).enforce();
...
#[test]
fn it_should_repair_if_directory_missing_in_enforce_mode() {
    let workdir = init_test();
