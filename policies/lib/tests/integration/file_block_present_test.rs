...
fn it_should_edit_the_file_if_needed() {
    ...
    let tested_method = &method("file_block_present", &[&file_path, "A block to add\non multiple lines"]).enforce();
...
fn it_should_be_idempotent() {
    ...
    let tested_method = &method("file_block_present", &[&file_path, "A block to add\non multiple lines"]).enforce();
    let tested_method2 = &method("file_block_present", &[&file_path, "A block to add\non multiple lines"]).enforce();
    let r = MethodTestSuite::new()
        .given(Given::file_present(&file_path, "Hello world!"))
        .given(Given::method_call(tested_method2))
        .when(tested_method)
        .execute(get_lib_path(), workdir.path().to_path_buf());
    r.assert_legacy_result_conditions(tested_method, vec![MethodStatus::Repaired, MethodStatus::Success]);
