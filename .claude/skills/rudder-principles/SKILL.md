...
**This is not only about the *domain*.** Technical data deserves the same treatment:
instrumentation, cache keys, internal protocol messages, anything you invent to make the
code work. They are where the rule is silently skipped — "it's just a couple of strings,
it never leaves this file" — and where the cost shows up as readers decoding `_._3` and
as two call sites that assemble the same string slightly differently.

Two triggers, mechanical enough to apply without judgement:

- **A tuple that travels.** If two or more values are passed together through more than
  one function, or if you find yourself writing `_._1` / `_._3`, give them a record with
  named fields.
- **A variant assembled by the caller.** If a call site builds a discriminator by
  concatenation (`"plugins:" + name`, `kind + ":" + id`), that is a sum type with a
  payload, and the name belongs on the case — not at each call site.

Declare such a type *before* the code that uses it: the reader meets the model, then the
procedure. That ordering is principle 1 made visible in the file.
