...
7. **Braces, and let spotless do the layout.** Significant indentation does not build
   here — scalafmt's dialect rejects it. Write braces, write `${value}` in
   interpolations, and run `spotless:apply` instead of hand-formatting anything
   (see `800`).
