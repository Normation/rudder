...
- **No inline styles** — **blocking**. No `style="..."` attribute, no `<style>` block, no
  `Html.Attributes.style`. Use classes. The goal is a Content-Security-Policy without
  `style-src 'unsafe-inline'`: one inline style keeps the exception open for the whole app.
...
- Tooltips: do **not** escape content with `htmlEscape` — Bootstrap sanitizes it automatically.
- Remove `Debug.log`.
