...
description: >-
  ...
  the `rudder-plugins` / `rudder-plugins-private` plugin UIs alike (same Elm + gulp stack).
...
Rudder's UI is a set of **Elm single-page apps**, one per screen, each compiled to a
...
same toolchain (Elm, `elm-format`, `elm-review`, gulp — versions in
[`400`](400-build-format-review.md)) and the same app shape. The
