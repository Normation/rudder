...
Write idiomatic Elm. The two mechanical gates ([`400`](400-build-format-review.md)) settle
...
- **Tooltips:** don't escape content with `htmlEscape` — Bootstrap sanitizes it automatically.
- Add **unit tests** and **acceptance tests** for new UI logic.
- **Styling: inline styles are forbidden.** No `style="…"` attribute, no `<style>` block, no
  `Html.Attributes.style` — use Bootstrap utility classes and Rudder's SCSS variables
  (`_rudder-variables.scss`), never hardcoded colors. This is not a taste rule: we want to serve
  a Content-Security-Policy that does not need `style-src 'unsafe-inline'`, and one inline style
  anywhere in a page forces us to keep that exception for the whole application. If a utility
  class is missing, add the class — details in [`900`](900-pr-review-checklist.md).
